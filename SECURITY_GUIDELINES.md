...
export const ROLE_PERMISSIONS = {
  ...
  ADMIN: [
    ...
    'warehouse:read', 'warehouse:write', 'warehouse:delete', 'warehouse:manage',
    'po:read', 'po:write', 'po:approve', 'po:delete',
    'user:read', 'user:write', 'user:delete', 'user:invite',
    'admin:read', 'admin:write', 'admin:billing', 'admin:audit'
  ],
  MANAGER: [
...
export async function uploadToS3(file: Express.Multer.File, organizationId: string) {
...
export class IncidentResponse {
  ...
  private static async initiateSecurityLockdown(): Promise<void> {
    ...
    await AuditLogger.logSecurityEvent(
      'SECURITY_LOCKDOWN',
      'CRITICAL',
      { initiatedAt: new Date(), reason: 'Suspected security incident' }
    );
  }
}
```
