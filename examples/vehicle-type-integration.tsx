...
export async function getVehicleForOrders(orderIds: string[]) {
  ...
  const totalVolume = orders.reduce((sum, order) => {
    return sum + order.items.reduce((itemSum, item) => {
      return itemSum + (item.lengthInches * item.widthInches * item.heightInches / 1728); // Convert to cu ft
    }, 0);
  }, 0);
  ...
  const palletCount = orders.reduce((sum, order) => sum + (order.palletCount || 0), 0);
  ...
  return {
    ...
    loadMetrics: {
      ...
      totalWeight,
      palletCount,
      volumeUtilization: (totalVolume / vehicle.volumeCubicFeet) * 100,
      weightUtilization: (totalWeight / vehicle.maxWeightLbs) * 100,
    },
  };
}
...
export async function planLoadWithVehicle(params: {
  ...
  const loadPlan = await loadOptimizationService.optimizeLoad({
    ...
    vehicleDimensions: {
      length: vehicle.dimensions.usableLengthInches || vehicle.dimensions.lengthInches,
      width: vehicle.dimensions.usableWidthInches || vehicle.dimensions.widthInches,
      height: vehicle.dimensions.usableHeightInches || vehicle.dimensions.heightInches,
    },
  ...
  return {
    ...
    loadPlan: {
      ...
      utilization: {
        ...
        weight: (totals.weight / vehicle.maxWeightLbs) * 100,
      },
      totalItems: loadPlan.totalItems,
      totalOrders: orders.length,
    },
    recommendation: {
      isOptimal: loadPlan.utilizationPercent >= 75 && loadPlan.utilizationPercent <= 95,
      estimatedCost: vehicle.estimatedCostPerMile ? 
        vehicle.estimatedCostPerMile * loadPlan.estimatedDistance : undefined,
    },
  };
}
...
export const vehicleVoiceCommands = {
  ...
  'what vehicle fits *volume cubic feet': async (volume: string) => {
    const volumeNumber = parseFloat(volume);
    const vehicle = recommendVehicle({
      totalVolumeCubicFeet: volumeNumber,
      totalWeightLbs: volumeNumber * 10, // Estimate
      region: 'UK',
    });
  ...
  return {
    volume: orders.reduce((sum, order) => {
      return sum + order.items.reduce((itemSum: number, item: any) => {
        const volumeCuFt = (item.lengthInches * item.widthInches * item.heightInches) / 1728;
...
export function VehicleRecommendationButton({ orderIds }: { orderIds: string[] }) {
    ...
    <div>
      ...
      {recommendation && (
        <div className="vehicle-recommendation">
          ...
          <p>Volume utilization: {recommendation.loadPlan.utilization.volume}%</p>
          <p>Weight utilization: {recommendation.loadPlan.utilization.weight}%</p>
          ...
          {recommendation.recommendation.isOptimal ? (
            ...
            <span className="badge-warning">Consider larger/smaller vehicle</span>
          )}
        </div>
      )}
    </div>
