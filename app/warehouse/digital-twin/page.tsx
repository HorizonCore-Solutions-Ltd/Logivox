...
import { Box, Activity, Users, Truck, Zap, AlertTriangle, TrendingUp } from 'lucide-react';
...
export default function DigitalTwinDashboard() {
  ...
  const [liveData, setLiveData] = useState<{ sensors: Sensor[] }>({ sensors: [] });
  const [equipment, setEquipment] = useState<Equipment[]>([]);
  const [workers, setWorkers] = useState<Worker[]>([]);
  const [loading, setLoading] = useState(false);
  const [activeView, setActiveView] = useState<'3d' | 'sensors' | 'equipment' | 'workers'>('3d');
  ...
  const render3DWarehouse = () => {
    ...
    warehouseModel.aisles?.forEach((aisle: any) => {
      ...
      ctx.fillRect(x, z, (aisle.dimensions.width / 100) * 700, (aisle.dimensions.length / 100) * 400);
    });
  ...
  const getStatusColor = (status: string) => {
    if (status === 'ACTIVE' || status === 'NORMAL') return 'bg-green-100 text-green-800';
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Digital Twin 3D Visualization</h1>
            <p className="text-gray-600 mt-2">Real-time warehouse monitoring with IoT integration</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 mt-1">{liveData.sensors.length}</p>
            </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 mt-1">
                {workers.length > 0
                  ? Math.round(workers.reduce((sum, w) => sum + w.efficiency, 0) / workers.length)
                  : 0}
                %
              </p>
            </div>
      ...
      <div className="mb-6 flex gap-2">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 rounded-lg font-medium transition ${
            activeView === '3d' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 rounded-lg font-medium transition ${
            activeView === 'sensors' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 rounded-lg font-medium transition ${
            activeView === 'equipment' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 rounded-lg font-medium transition ${
            activeView === 'workers' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'
          }`}
        >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        {loading ? (
          ...
          <>
            ...
            {activeView === '3d' && (
              <div className="p-6">
                <h2 className="text-xl font-bold text-gray-900 mb-6">3D Warehouse Visualization</h2>
                <canvas ref={canvasRef} width={1000} height={500} className="border border-gray-200 rounded-lg" />
                <p className="text-sm text-gray-600 mt-4">
                  Real-time warehouse layout with equipment and worker positions. Updates every 5 seconds.
                </p>
              </div>
            )}
            ...
            {activeView === 'sensors' && (
              <div className="p-6">
                <h2 className="text-xl font-bold text-gray-900 mb-6">IoT Sensor Data</h2>
                  ...
                  {liveData.sensors.map((sensor) => (
                    <div key={sensor.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${getStatusColor(sensor.status)}`}>
                          {sensor.status}
                        </span>
                      </div>
                      <h3 className="font-semibold text-gray-900">{sensor.id}</h3>
                      <p className="text-sm text-gray-600">{sensor.type.replace(/_/g, ' ')}</p>
                      <div className="mt-3">
                      ...
                      <div className="mt-2 text-xs text-gray-500">
                        Location: ({sensor.location.x}, {sensor.location.y}, {sensor.location.z})
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {equipment.map((eq) => (
                    <div key={eq.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`px-3 py-1 rounded font-semibold ${getStatusColor(eq.status)}`}>
                          {eq.status}
                        </span>
                      </div>
                      <div className="grid grid-cols-3 gap-4 mt-4">
                        {eq.battery !== undefined && (
                          <div>
                            ...
                            <p className="text-lg font-semibold text-gray-900">{eq.battery}%</p>
                          </div>
                        )}
                        {eq.operator && (
                          <div>
                            <p className="text-sm text-gray-600">Operator</p>
                            <p className="text-lg font-semibold text-gray-900">{eq.operator}</p>
                          </div>
                        )}
                        <div>
            ...
            {activeView === 'workers' && (
              <div className="p-6">
                <h2 className="text-xl font-bold text-gray-900 mb-6">Worker Tracking</h2>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200">
                        ...
                        <th className="text-left py-3 px-4 text-sm font-semibold text-gray-700">Current Task</th>
                      ...
                      {workers.map((worker) => (
                        <tr key={worker.id} className="border-b border-gray-100 hover:bg-gray-50">
                          <td className="py-3 px-4">
                            ...
                            <div className="text-sm text-gray-600">{worker.id}</div>
                          </td>
                          <td className="py-3 px-4 text-sm text-gray-700">{worker.role}</td>
                          <td className="py-3 px-4 text-sm text-gray-700">{worker.currentTask}</td>
                          <td className="py-3 px-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <span className="text-sm font-medium text-gray-900">{worker.taskProgress}%</span>
                            </div>
                          </td>
                          <td className="py-3 px-4 text-right">
                            <span className="text-lg font-bold text-green-600">{worker.efficiency}%</span>
                          </td>
