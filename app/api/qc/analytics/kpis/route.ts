...
export async function GET(req: NextRequest) {
  try {
    ...
    // Calculate trends
    const ncrTrend = previousNCRs > 0 
      ? Math.round(((currentNCRs - previousNCRs) / previousNCRs) * 100)
      : 0;

    const capaTrend = previousOpenCAPAs > 0
      ? Math.round(((openCAPAs - previousOpenCAPAs) / previousOpenCAPAs) * 100)
      : 0;

    // Calculate total claims
    const totalClaims = allNCRs.reduce((sum: number, ncr: any) => sum + (ncr.claimAmount || 0), 0);

    // Calculate quality score (simplified)
    const totalMetrics = currentNCRs + openCAPAs + criticalRisks + overdueAudits;
