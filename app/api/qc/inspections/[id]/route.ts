...
export async function PATCH(
  ...
  try {
    ...
    if (action === 'start') {
      const inspection = await QCInspectionService.startInspection(params.id, userId);
      return NextResponse.json({ inspection });
    }
    ...
    if (action === 'addItem') {
      const item = await QCInspectionService.addInspectionItem(params.id, body.itemData);
      return NextResponse.json({ item });
    }

    if (action === 'recordDefect') {
      const defect = await QCInspectionService.recordDefect(params.id, body.defectData);
