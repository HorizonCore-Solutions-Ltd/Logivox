...
async function calculateKPIs(organizationId: string) {
  ...
  // Sales Metrics
  const [todayOrders, yesterdayOrders, thisMonthOrders, lastMonthOrders] = await Promise.all([
  ...
  const [todayRevenue, yesterdayRevenue, thisMonthRevenue, lastMonthRevenue] = await Promise.all([
    ...
    prisma.salesOrder.aggregate({
      where: { organizationId, orderDate: { gte: yesterday, lt: today }, status: { not: 'CANCELLED' } },
      _sum: { total: true },
    }),
    prisma.salesOrder.aggregate({
      where: { organizationId, orderDate: { gte: thisMonth }, status: { not: 'CANCELLED' } },
      _sum: { total: true },
    }),
    prisma.salesOrder.aggregate({
      where: { organizationId, orderDate: { gte: lastMonth, lt: thisMonth }, status: { not: 'CANCELLED' } },
      _sum: { total: true },
    }),
  ]);

  // Inventory Metrics
  const [totalInventoryValue, lowStockItems, outOfStockItems, totalItems] = await Promise.all([
  ...
  const metrics = [
    ...
    },
    {
      ...
      category: 'QUALITY',
      currentValue: qcInspectionsToday > 0 ? (qcPassed / qcInspectionsToday) * 100 : 0,
      previousValue: null,
      targetValue: 95,
      unit: 'percentage',
      periodType: 'DAILY',
      periodStart: today,
      periodEnd: now,
      status: (qcInspectionsToday > 0 && (qcPassed / qcInspectionsToday) * 100 < 95) ? 'WARNING' : 'NORMAL',
    },
    // Production Metrics
    {
  ...
  return metrics.map((metric) => {
    const changeAmount = metric.previousValue !== null
      ? metric.currentValue - metric.previousValue
      : null;
    
    const changePercent = metric.previousValue !== null && metric.previousValue !== 0
