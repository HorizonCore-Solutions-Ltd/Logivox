...
export async function POST(request: NextRequest) {
  try {
    ...
    if (container.maxWeight) {
      const currentWeight = container.containerItems.reduce(
    ...
    if (container.loadSheetId) {
      await updateLoadSheetFromContainer(
        container.loadSheetId,
        updatedContainer,
      );
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const newWeight = Math.max(
      0,
      (item.container.weight || 0) - (item.weight || 0),
    );
    const newVolume = Math.max(
      0,
      (item.container.volume || 0) - (item.volume || 0),
    );
...
async function updateLoadSheetFromContainer(
  loadSheetId: string,
  container: any,
) {
  ...
  const totalItems = containers.reduce(
    (sum, c) => sum + c.containerItems.length,
    0,
  );
  const totalOrders = new Set(
    containers.flatMap((c) =>
      c.containerItems.map((i) => i.salesOrderId).filter(Boolean),
    ),
  ).size;
