    ...
    switch (action) {
      ...
      case "carrier-performance":
        ...
        if (!carrierName || !startDate || !endDate) {
          return NextResponse.json(
            {
              error:
                "Missing required parameters (carrierName, startDate, endDate)",
            },
            { status: 400 },
          );
        }
      ...
      case "gate-activity":
        ...
        const activity = await YardManagementService.getGateActivity({
          ...
          endDate: new Date(
            new Date(gateDate).setDate(new Date(gateDate).getDate() + 1),
          ),
        });
...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (action) {
      case "create-appointment":
        ...
        const appointment = await YardManagementService.createAppointment({
          ...
          scheduledEnd: new Date(
            new Date(scheduledDate).getTime() + (duration || 60) * 60000,
          ),
          carrierName,
      ...
      case "check-in":
        const { appointmentId: checkInAppointmentId, checkedInBy: checkInBy } =
          body;
