...
export async function PATCH(
  ...
  try {
    ...
    // General update
    const allowedFields = ['status', 'priority', 'assigned_to', 'estimated_days', 'notes'];
    const updates: any = {};
    ...
    return NextResponse.json(
      { error: 'Failed to update work order', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
