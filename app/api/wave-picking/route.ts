...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (action) {
      case 'create-wave':
        const { organizationId, warehouseId, orderIds, createdById, strategy, name, waveType, priority } = body;
        
        if (!organizationId || !warehouseId || !createdById || !name || !waveType) {
          return NextResponse.json(
            { error: 'Missing required fields (organizationId, warehouseId, createdById, name, waveType)' },
            { status: 400 }
          );
        }
      ...
      case 'record-pick':
        ...
        if (!lineId || !pickedById || pickedQuantity === undefined) {
          return NextResponse.json(
            { error: 'Missing required pick fields (lineId, pickedById, pickedQuantity)' },
            { status: 400 }
          );
        }
