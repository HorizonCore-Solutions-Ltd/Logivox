...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (action) {
      case 'create-grn':
        const { organizationId, purchaseOrderId, warehouseId, receivedById, items } = body;
      ...
      case 'process-asn':
        const { organizationId: asnOrgId, purchaseOrderId: asnPurchaseOrderId, asnData } = body;
        
        if (!asnOrgId || !asnPurchaseOrderId || !asnData) {
          return NextResponse.json(
            { error: 'Missing ASN data (requires organizationId, purchaseOrderId, asnData)' },
            { status: 400 }
          );
        }
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid action. Use: create-grn, quality-control, complete-putaway, process-asn' },
          { status: 400 }
        );
    }
  } catch (error: any) {
