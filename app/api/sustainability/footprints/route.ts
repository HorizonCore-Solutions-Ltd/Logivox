...
export async function GET(request: NextRequest) {
  try {
    ...
    const averageCO2ePerOrder = footprints.length > 0 ? totalCO2e / footprints.length : 0;
    ...
    const footprintsByDate = footprints.reduce((acc, f) => {
    ...
    return NextResponse.json({
      ...
      data: {
        summary: {
          ...
          percentages: {
            transportation: totalCO2e > 0 ? Math.round((breakdown.transportation / totalCO2e) * 100) : 0,
            packaging: totalCO2e > 0 ? Math.round((breakdown.packaging / totalCO2e) * 100) : 0,
            warehousing: totalCO2e > 0 ? Math.round((breakdown.warehousing / totalCO2e) * 100) : 0,
            manufacturing: totalCO2e > 0 ? Math.round((breakdown.manufacturing / totalCO2e) * 100) : 0,
          },
        },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { orderId, transportation, packaging, warehousing, manufacturing } = body;
    ...
    const totalCO2e = (transportation || 0) + (packaging || 0) + (warehousing || 0) + (manufacturing || 0);
