    ...
    return NextResponse.json({
      ...
      data: {
        ...
        ranking: {
          ...
          inClass: allInClass,
          rankDescription: 
            percentile <= 10 ? 'Elite performer' :
            percentile <= 25 ? 'Top quartile' :
            percentile <= 50 ? 'Above average' :
            percentile <= 75 ? 'Below average' : 'Needs improvement'
        },
        recommendations,
        peers: peers.map(p => ({
        ...
        insights: {
          strengths: classification.velocityClass === 'A' 
            ? ['High revenue contributor', 'Fast turnover', 'Customer favorite']
          ...
          concerns: classification.velocityClass === 'D'
