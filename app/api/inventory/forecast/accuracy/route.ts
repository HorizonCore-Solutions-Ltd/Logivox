...
export async function GET(request: NextRequest) {
  try {
    ...
    const accuracyResults = await Promise.all(
      forecasts.map(async (forecast) => {
        ...
        const daysSinceForecast = Math.min(
          Math.ceil(
            (endDate.getTime() - forecastDate.getTime()) /
              (1000 * 60 * 60 * 24),
          ),
        ...
        if (actualTotal > 0) {
          ...
          accuracy = Math.max(
            0,
            Math.min(100, (1 - error / actualTotal) * 100),
          );
        }
    ...
    if (groupBy === "product") {
      ...
      Object.values(groupedResults).forEach((group: any) => {
        group.avgAccuracy = Math.round(
          group.forecasts.reduce((sum: number, f: any) => sum + f.accuracy, 0) /
    ...
    const avgAccuracy =
      totalForecasts > 0
        ? Math.round(
            accuracyResults.reduce((sum, r) => sum + r.accuracy, 0) /
              totalForecasts,
          )
        : 0;
