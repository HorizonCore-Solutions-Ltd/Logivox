...
export async function GET(
  ...
  try {
    ...
    if (historicalForecasts.length > 0) {
      ...
      const oldestForecast =
        historicalForecasts[historicalForecasts.length - 1];
      ...
      if (actualSales._sum.quantity && oldestForecast.avgDailyDemand) {
        const daysDiff = Math.ceil(
          (endDate.getTime() - forecastDate.getTime()) / (1000 * 60 * 60 * 24),
        );
        ...
        accuracy = {
          ...
          accuracy: Math.min(
            100,
            Math.round(
              (1 - Math.abs(predictedTotal - actualTotal) / actualTotal) * 100,
            ),
          ),
        ...
        statistics: {
          ...
          avgConfidence:
            historicalForecasts.length > 0
              ? historicalForecasts.reduce((sum, f) => sum + f.confidence, 0) /
