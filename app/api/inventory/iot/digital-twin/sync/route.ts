...
export async function POST(request: NextRequest) {
  try {
    ...
    const twinState = await iotMonitoringService.synchronizeDigitalTwin(productId);
    ...
    return NextResponse.json({
      ...
      data: {
        twinState,
        recommendation: twinState.syncConfidence >= 85
  ...
  try {
    ...
    const syncStatuses = await Promise.all(
      products.map(async (product) => {
        try {
          const twinState = await iotMonitoringService.synchronizeDigitalTwin(product.id);
          return {
    ...
    const highConfidence = syncStatuses.filter(s => s.syncConfidence >= 85).length;
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        statistics: {
          ...
          needsSync,
          highConfidence,
          avgConfidence: syncStatuses.length > 0
            ? Math.round(syncStatuses.reduce((sum, s) => sum + s.syncConfidence, 0) / syncStatuses.length)
            : 0
        }
      }
    });
