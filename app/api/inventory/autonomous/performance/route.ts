  ...
  try {
    ...
    const calculateMetrics = (decisions: any[]) => {
      ...
      const totalCost = decisions.reduce((sum, d) => 
        sum + parseFloat(d.estimatedCost?.toString() || '0'), 0
      );
      const totalSavings = decisions.reduce((sum, d) => 
        sum + parseFloat(d.estimatedSavings?.toString() || '0'), 0
      );
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        overall: {
          ...
          performance: overallMetrics.successRate >= 85 ? 'EXCELLENT' :
                       overallMetrics.successRate >= 75 ? 'GOOD' :
                       overallMetrics.successRate >= 65 ? 'ACCEPTABLE' : 'NEEDS_IMPROVEMENT'
        },
        byType: {
          reorders: {
            ...reorderMetrics,
    ...
    return NextResponse.json(
      { error: 'Failed to generate performance metrics', message: error.message },
      { status: 500 }
    );
  }
}
