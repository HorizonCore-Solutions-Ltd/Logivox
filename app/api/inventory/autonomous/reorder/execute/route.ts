...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const decisions = await autonomousOperationsService.executeAutonomousReorders(
      organizationId,
      productIds
    );
    ...
    const pendingApproval = decisions.filter(d => !d.executed && d.requiresApproval);
    ...
    const totalSavings = decisions.reduce((sum, d) => sum + (d.estimatedSavings || 0), 0);
...
export async function GET() {
  const config = await autonomousOperationsService.getAutonomousConfig('default-org');
