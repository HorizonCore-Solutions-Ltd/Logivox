...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const decisions = await autonomousOperationsService.executeAutonomousTransfers(
      organizationId
    );
    ...
    const avgConfidence = decisions.length > 0
      ...
      : 0;
    const stockoutsAvoided = executed.filter(d => 
      (d.fromIntelligence?.stockoutRisk || 0) > 60
    ).length;
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        impact: {
          description: 'Autonomous transfers balance inventory across warehouses',
          benefits: [
