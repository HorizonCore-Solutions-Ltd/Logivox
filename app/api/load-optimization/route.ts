...
export const LOAD_OPTIMIZATION_VOICE_COMMANDS = [
  ...
  // Auto-assign orders
  {
    ...
    examples: [
      ...
      'Automatically assign 25 orders',
    ],
  },
  
  // Check trailer capacity
  {
    pattern: /(?:check|show|what\'s|what is) (?:the )?capacity (?:of |for )?trailer (.+)/i,
    category: 'load_optimization',
    action: async (matches: string[]) => {
  ...
  // Optimize load plan
  {
    ...
    examples: [
      'Optimize load plan LP-12345',
      'Optimize load plan A100',
    ],
  },
  ...
  // Check utilization
  {
    pattern: /(?:what\'s|what is|show|check) (?:the )?(?:trailer )?utilization/i,
    category: 'load_optimization',
    action: async () => {
      return {
  ...
  // Next item to load
  {
    pattern: /(?:what\'s|what is|show) (?:the )?next (?:item|order) (?:to load)?/i,
    category: 'load_optimization',
    action: async () => {
      return {
