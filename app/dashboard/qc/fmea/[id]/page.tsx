...
export default function FMEADetail() {
  ...
  const calculateStats = () => {
    ...
    const high = fmea.failureModes.filter(
      (fm) => fm.rpn >= 125 && fm.rpn < 200,
    ).length;
    const open = fmea.failureModes.filter(
      (fm) => fm.status !== "CLOSED",
    ).length;
    const avgRPN =
      total > 0
        ? Math.round(
            fmea.failureModes.reduce((sum, fm) => sum + fm.rpn, 0) / total,
          )
        : 0;
        ...
        <TabsContent value="failure-modes" className="space-y-4">
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>
                  Failure Modes ({fmea.failureModes.length})
                </CardTitle>
                ...
                <div className="border p-4 rounded-lg bg-gray-50 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Input
                      ...
                      onChange={(e) =>
                        setNewMode({ ...newMode, processStep: e.target.value })
                      }
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        setNewMode({ ...newMode, failureMode: e.target.value })
                      }
                    />
                  </div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setNewMode({ ...newMode, effects: e.target.value })
                    }
                    rows={2}
                  />
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setNewMode({ ...newMode, causes: e.target.value })
                    }
                    rows={2}
                  />
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setNewMode({ ...newMode, controls: e.target.value })
                    }
                    rows={2}
                  />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Textarea
                      ...
                      onChange={(e) =>
                        setNewMode({ ...newMode, actions: e.target.value })
                      }
                      rows={2}
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        setNewMode({ ...newMode, responsible: e.target.value })
                      }
                    />
                  </div>
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-2">
                            <Badge className={getRPNColor(fm.rpn)}>
                              RPN: {fm.rpn}
                            </Badge>
                            <Badge className={getStatusBadge(fm.status)}>
                      ...
                      <div className="mt-3 flex items-center justify-between text-xs">
                        <div>
                          <span className="text-muted-foreground">
                            S: {fm.severity} × O: {fm.occurrence} × D:{" "}
                            {fm.detection}
                          </span>
                          {fm.residualSeverity && (
                            <span className="ml-4 text-green-600">
                              → S: {fm.residualSeverity} × O:{" "}
                              {fm.residualOccurrence} × D:{" "}
                              {fm.residualDetection}
                            </span>
                          )}
                        </div>
                        <div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">
                    {new Date(fmea.startDate).toLocaleDateString()}
                  </p>
                </div>
                {fmea.lastReviewDate && (
                  <div>
