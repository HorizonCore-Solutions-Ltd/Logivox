...
export default function VendorConcessionsPage() {
      ...
      const mockConcessions: Concession[] = [
        {
          ...
          concessionValue: 15000.00,
          originalClaimAmount: 22000.00,
        ...
        },
        {
          ...
          concessionValue: 8500.00,
          originalClaimAmount: 12000.00,
          relatedIssue: 'RTV',
          status: 'ACTIVE',
        ...
        },
        {
          ...
          concessionValue: 5400.00,
          originalClaimAmount: 5400.00,
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats({
        totalConcessions: 38,
        totalValue: 245680.00,
        activeValue: 189420.00,
        utilizedValue: 98540.00,
        remainingValue: 90880.00,
        expiredValue: 56260.00,
        utilizationRate: 52,
      });
    } catch (error) {
  ...
  const getStatusBadge = (status: string) => {
    const config: Record<string, { color: string; icon: any; text: string }> = {
      ...
      FULLY_UTILIZED: { color: 'bg-blue-500', icon: DollarSign, text: 'Fully Utilized' },
  ...
  const filteredConcessions = concessions.filter(con => {
    ...
    const matchesSearch = con.concessionNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-4 items-center">
        ...
        />
        <div className="flex gap-2">
          {['ALL', 'ACTIVE', 'FULLY_UTILIZED', 'EXPIRED', 'CANCELLED'].map((status) => (
            <Button
              key={status}
      ...
      <div className="space-y-4">
        {filteredConcessions.length === 0 ? (
          ...
          filteredConcessions.map((concession) => (
            <Card key={concession.id} className="hover:shadow-md transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-semibold text-lg">{concession.concessionNumber}</h3>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <p className="text-muted-foreground">Concession Value</p>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <p className="text-muted-foreground text-xs">Related Issue</p>
                      ...
                      <div>
                        ...
                        <p className="font-medium mt-1">
                          {concession.expiresAt 
                            ? new Date(concession.expiresAt).toLocaleDateString()
                            : 'No expiration'}
                        </p>
                      </div>
                    </div>
                    {concession.status === 'ACTIVE' && concession.remaining > 0 && (
                      <div className="mt-3 bg-green-50 border border-green-200 rounded p-2">
                        <p className="text-sm text-green-800">
                          <CheckCircle className="w-4 h-4 inline mr-1" />
                          <strong>${concession.remaining.toLocaleString()}</strong> available for use
                        </p>
                      </div>
                    )}
                  </div>
      ...
      <Card className="bg-yellow-50 border-yellow-200">
        <CardContent className="pt-6">
          <div className="flex gap-4">
            ...
            <div>
              <h3 className="font-semibold text-yellow-900 mb-2">About Vendor Concessions</h3>
              <p className="text-sm text-yellow-800">
                Concessions are negotiated alternatives to chargebacks, debit memos, or RTVs. They can include
                price discounts, credit allowances, free goods, extended payment terms, or promotional allowances.
                Concessions help maintain vendor relationships while still recovering value from quality issues or delays.
              </p>
