...
export default function RootCauseAnalysisPage() {
    ...
    try {
      ...
      const mockAnalyses: RootCauseAnalysis[] = [
        ...
        },
        {
          ...
          rootCause: 'Lack of defined packaging standards and quality control procedures',
          correctiveActions: 3,
        ...
        },
        {
          ...
          rootCause: 'Inadequate IT maintenance procedures and lack of automated monitoring',
          correctiveActions: 4,
        ...
        },
        {
          ...
          rootCause: 'Absence of formal preventive maintenance program and clear ownership',
          correctiveActions: 5,
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats({
        ...
        inProgress: 18,
        completed: 94,
        avgImpactCost: 11250.00,
        totalImpactCost: 1395000.00,
  ...
  const getStatusBadge = (status: string) => {
    const config: Record<string, { color: string; icon: any; text: string }> = {
      ...
      VERIFICATION: { color: 'bg-yellow-500', icon: Target, text: 'Verification' },
  ...
  const filteredAnalyses = analyses.filter(analysis => {
    ...
    const matchesSearch = analysis.analysisNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-4 items-center">
        ...
        />
        <div className="flex gap-2">
          {['ALL', 'IN_PROGRESS', 'COMPLETED', 'VERIFICATION', 'CLOSED'].map((status) => (
            <Button
              key={status}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-semibold text-lg">{analysis.analysisNumber}</h3>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{analysis.affectedItems.toLocaleString()} units</p>
                      </div>
                      <div>
                      ...
                      <div>
                        <p className="text-muted-foreground">Corrective Actions</p>
                        <p className="font-medium">{analysis.correctiveActions} actions</p>
                      </div>
                    </div>
                    ...
                    <div className="bg-pink-50 border border-pink-200 rounded p-3 mb-3">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1">
                          <p className="font-semibold text-sm text-pink-900 mb-1">5 Whys Chain:</p>
                          <div className="space-y-1 text-xs text-pink-800">
                            <div><strong>Why 1:</strong> {analysis.why1}</div>
                            {analysis.why2 && <div><strong>Why 2:</strong> {analysis.why2}</div>}
                            {analysis.why3 && <div><strong>Why 3:</strong> {analysis.why3}</div>}
                            {analysis.why4 && <div><strong>Why 4:</strong> {analysis.why4}</div>}
                            {analysis.why5 && <div><strong>Why 5:</strong> {analysis.why5}</div>}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {analysis.rootCause && (
                      <div className="bg-green-50 border border-green-200 rounded p-3">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            <p className="font-semibold text-sm text-green-900 mb-1">Root Cause:</p>
                            <p className="text-sm text-green-800">{analysis.rootCause}</p>
                    ...
                    <div className="mt-3 text-xs text-muted-foreground">
                      Created: {new Date(analysis.createdDate).toLocaleDateString()}
                      {analysis.completedDate && ` • Completed: ${new Date(analysis.completedDate).toLocaleDateString()}`}
                    </div>
                  </div>
                  <Link href={`/dashboard/qc/root-cause-analysis/${analysis.id}`}>
      ...
      <Card className="bg-pink-50 border-pink-200">
        <CardContent className="pt-6">
          <div className="flex gap-4">
            ...
            <div>
              <h3 className="font-semibold text-pink-900 mb-2">About 5 Whys Methodology</h3>
              <p className="text-sm text-pink-800 mb-3">
                The 5 Whys is an iterative interrogative technique used to explore cause-and-effect relationships.
                By repeatedly asking "Why?" (typically five times), you can peel away layers of symptoms to reveal
                the root cause of a problem. This method helps prevent recurrence by addressing fundamental issues
