...
export default function PerformanceReviewsPage() {
  ...
  const getStatusBadge = (status: string) => {
    const config: Record<string, { color: string; icon: any; text: string }> = {
      ...
      REQUIRES_ACTION: { color: 'bg-red-500', icon: AlertTriangle, text: 'Requires Action' },
    };
  ...
  const getRatingBadge = (rating: string, score: number) => {
    ...
    const { color, text } = config[rating] || { color: 'bg-gray-500', text: rating };
    return (
      <Badge className={`${color} text-white flex items-center gap-1`}>
  ...
  const filteredReviews = reviews.filter(review => {
    ...
    const matchesSearch = review.reviewNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-4 items-center">
        ...
        />
        <div className="flex gap-2">
          {['ALL', 'IN_PROGRESS', 'COMPLETED', 'REQUIRES_ACTION'].map((status) => (
            <Button
              key={status}
        ...
        {filteredReviews.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <h3 className="text-lg font-semibold">No performance reviews found</h3>
              <p className="text-muted-foreground">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-semibold text-lg">{review.reviewNumber}</h3>
                      {getStatusBadge(review.status)}
                      {getRatingBadge(review.rating, review.overallScore)}
                    </div>
                    <p className="text-muted-foreground mb-3">
                      <strong>Vendor:</strong> {review.vendor.name} • {review.reviewPeriod}
                    </p>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div 
                          className={`h-3 rounded-full ${
                            review.overallScore >= 90 ? 'bg-purple-600' :
                            review.overallScore >= 80 ? 'bg-green-600' :
                            review.overallScore >= 70 ? 'bg-blue-600' :
                            review.overallScore >= 60 ? 'bg-yellow-600' : 'bg-red-600'
                          }`}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
                      <div className="text-center p-2 bg-gray-50 rounded">
                        ...
                        <p className={`font-bold text-lg ${getScoreColor(review.qualityScore)}`}>
                          {review.qualityScore}
                        </p>
                      ...
                      <div className="text-center p-2 bg-gray-50 rounded">
                        ...
                        <p className={`font-bold text-lg ${getScoreColor(review.deliveryScore)}`}>
                      ...
                      <div className="text-center p-2 bg-gray-50 rounded">
                        ...
                        <p className={`font-bold text-lg ${getScoreColor(review.responsivenessScore)}`}>
                      ...
                      <div className="text-center p-2 bg-gray-50 rounded">
                        ...
                        <p className={`font-bold text-lg ${getScoreColor(review.pricingScore)}`}>
                          {review.pricingScore}
                        </p>
                      ...
                      <div className="text-center p-2 bg-gray-50 rounded">
                        ...
                        <p className={`font-bold text-lg ${getScoreColor(review.complianceScore)}`}>
                    ...
                    <div className="mt-3 text-xs text-muted-foreground">
                      Review Period: {new Date(review.periodStart).toLocaleDateString()} - {new Date(review.periodEnd).toLocaleDateString()}
                    </div>
                  </div>
      ...
      <Card className="bg-indigo-50 border-indigo-200">
        <CardContent className="pt-6">
          <div className="flex gap-4">
            ...
            <div>
              <h3 className="font-semibold text-indigo-900 mb-2">Performance Scoring Methodology</h3>
              <p className="text-sm text-indigo-800 mb-3">
                Vendor performance is evaluated quarterly using weighted criteria. Overall scores are automatically
                calculated based on individual category performance with predefined weights.
              </p>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs">
                <div className="bg-white/50 p-2 rounded">
                  <strong>Quality:</strong> 30%<br/>Defect rates, QC pass rates
                </div>
                <div className="bg-white/50 p-2 rounded">
                  <strong>Delivery:</strong> 25%<br/>On-time, fill rates
                </div>
                <div className="bg-white/50 p-2 rounded">
                  <strong>Responsiveness:</strong> 20%<br/>Communication, issue resolution
                </div>
                <div className="bg-white/50 p-2 rounded">
                  <strong>Pricing:</strong> 15%<br/>Competitiveness, stability
                </div>
                <div className="bg-white/50 p-2 rounded">
                  <strong>Compliance:</strong> 10%<br/>Documentation, standards
                </div>
              </div>
            </div>
          </div>
