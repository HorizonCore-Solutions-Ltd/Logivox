...
export default function QualityHoldsPage() {
  ...
  const filteredHolds = holds.filter(hold =>
    ...
    (hold.lotNumber && hold.lotNumber.toLowerCase().includes(searchTerm.toLowerCase()))
  );
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage quarantine and hold releases</p>
        </div>
      ...
      {stats && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">{stats.activeHolds}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.totalQuantityOnHold.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground">
                units in quarantine
              </p>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.releaseRate.toFixed(1)}%</div>
              <p className="text-xs text-muted-foreground">
                successful releases
              </p>
            </CardContent>
                ...
                {filteredHolds.length === 0 ? (
                  ...
                  filteredHolds.map((hold) => (
                    <TableRow key={hold.id}>
                      <TableCell className="font-medium">{hold.holdNumber}</TableCell>
                      ...
                      <TableCell>{hold.quantityOnHold.toLocaleString()}</TableCell>
                      <TableCell className="font-semibold">{hold.quantityRemaining.toLocaleString()}</TableCell>
                      <TableCell>{getStatusBadge(hold.status)}</TableCell>
                      <TableCell>{getReleaseStatusBadge(hold.releaseStatus)}</TableCell>
