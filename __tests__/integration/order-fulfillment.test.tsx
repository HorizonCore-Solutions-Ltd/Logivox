...
describe("Integration: Order Fulfillment Workflow", () => {
  ...
  it("should complete full order fulfillment workflow", async () => {
    ...
    const generateWaveButton = screen.getByRole("button", {
      name: /generate wave/i,
    });
    ...
    const confirmPickButton = screen.getByRole("button", {
      name: /confirm pick/i,
    });
    ...
    const confirmPackButton = screen.getByRole("button", {
      name: /confirm packing/i,
    });
    fireEvent.click(confirmPackButton);

    // Step 6: Generate shipping label
    await waitFor(() => {
      const shipButton = screen.getByRole("button", {
        name: /generate label/i,
      });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/order shipped successfully/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should handle partial picking", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /create backorder/i }),
    ).toBeInTheDocument();
  });
