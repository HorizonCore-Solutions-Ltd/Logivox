...
test.describe('Smoke Tests - Critical Paths', () => {
  ...
  test('Dashboard requires authentication', async ({ page }) => {
    ...
    const url = page.url();
    expect(url.includes('/login') || url.includes('/auth') || url.includes('/api/auth')).toBe(true);
  });
...
test.describe('Security Smoke Tests', () => {
  ...
  test('API rate limiting is configured', async ({ request }) => {
    ...
    const requests = Array(20).fill(null).map(() => request.get('/api/health'));
  ...
  test('Images are optimized', async ({ page }) => {
    ...
    for (const img of images.slice(0, 5)) {
      ...
      if (src && !src.startsWith('data:')) {
        ...
        const isOptimized = src.includes('_next/image') || src.includes('/_next/static');
        // Images should either be optimized or be SVGs/external
        expect(isOptimized || src.endsWith('.svg') || src.startsWith('http')).toBe(true);
      }
    }
  });
