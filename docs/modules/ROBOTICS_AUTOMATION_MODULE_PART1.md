...
interface RobotFleetManagement {
  ...
  // Control
  sendCommand: (robotId: string, command: RobotCommand) => Promise<CommandResponse>;
...
interface RobotRegistration {
  ...
  capabilities: {
    ...
    // Features
    features: ('NAVIGATION' | 'LIFTING' | 'PICKING' | 'SORTING' | 'PALLETIZING' | 'SCANNING')[];
...
interface RobotIssue {
  ...
  category: 'MECHANICAL' | 'ELECTRICAL' | 'SOFTWARE' | 'SENSOR' | 'BATTERY' | 'NAVIGATION' | 'OTHER';
...
interface AGVCoordination {
  ...
  // Traffic management
  requestPassage: (vehicleId: string, path: PathSegment[]) => Promise<PassageApproval>;
  reportPosition: (vehicleId: string, position: VehiclePosition) => Promise<void>;
...
```typescript
type CobotOperation = 'PICKING' | 'PACKING' | 'PALLETIZING' | 'SORTING' | 'INSPECTION' | 'ASSEMBLY';
...
interface CobotIntegration {
  ...
  // Collaboration
  requestCollaboration: (request: CollaborationRequest) => Promise<CollaborationSession>;
  
  // Performance
  getCobotMetrics: (cobotId: string, period: DateRange) => Promise<CobotMetrics>;
}
...
interface CobotTaskStatus {
  ...
  status: 'QUEUED' | 'EXECUTING' | 'PAUSED' | 'COMPLETED' | 'FAILED' | 'CANCELLED';
...
```typescript
type ASRSType = 'UNIT_LOAD' | 'MINI_LOAD' | 'VERTICAL_LIFT' | 'CAROUSEL' | 'SHUTTLE';
...
```typescript
type ConveyorType = 'ROLLER' | 'BELT' | 'SORTATION' | 'ACCUMULATION' | 'VERTICAL';
...
interface ConveyorIntegration {
  ...
  // Monitoring
  getConveyorMetrics: (conveyorId: string, period: DateRange) => Promise<ConveyorMetrics>;
}
...
interface PackageStatus {
  ...
  status: 'QUEUED' | 'IN_TRANSIT' | 'SORTING' | 'ARRIVED' | 'DIVERTED' | 'STUCK';
  ...
  events: {
    timestamp: Date;
    event: 'INDUCTED' | 'TRANSFERRED' | 'SORTED' | 'ARRIVED' | 'DIVERTED' | 'ERROR';
    location: string;
  }[];
}
...
interface ConveyorMetrics {
  ...
  // Downtime reasons
  downtimeReasons: {
    reason: 'SCHEDULED_MAINTENANCE' | 'UNSCHEDULED_MAINTENANCE' | 'JAM' | 'ERROR' | 'NO_DEMAND';
