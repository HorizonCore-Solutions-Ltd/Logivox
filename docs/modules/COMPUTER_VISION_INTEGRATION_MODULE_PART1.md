...
```typescript
type CameraType = 'FIXED' | 'PTZ' | 'MOBILE_DEVICE' | 'DRONE' | 'FORKLIFT_MOUNTED' | '3D_DEPTH';
...
interface DamageDetectionResult {
  ...
  damages: {
    id: string;
    type: 'DENT' | 'TEAR' | 'STAIN' | 'BROKEN' | 'CRUSHED' | 'WATER_DAMAGE' | 'OTHER';
...
```typescript
type LabelType = 'BARCODE' | 'QR_CODE' | 'SKU_LABEL' | 'SHIPPING_LABEL' | 'PLACARD' | 'LICENSE_PLATE' | 'LOT_NUMBER';

interface OCREngine {
  readText: (imageRef: string, options?: OCROptions) => Promise<OCRResult>;
  readLabel: (imageRef: string, labelType: LabelType) => Promise<LabelReadResult>;
...
interface DimensionMeasurement {
  ...
  // Calibration
  calibrateCamera: (cameraId: string, referenceObject: ReferenceObject) => Promise<CalibrationResult>;
}
...
```typescript
type InspectionType = 'RECEIVING' | 'PRODUCTION' | 'PRE_SHIPMENT' | 'RETURNS' | 'CYCLE_COUNT';
...
interface VisualQualityInspection {
  ...
  performInspection: (inspectionId: string, images: string[]) => Promise<InspectionResult>;
  
  reviewInspection: (inspectionId: string, review: InspectionReview) => Promise<void>;
}
