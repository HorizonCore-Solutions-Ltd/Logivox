...
type AutonomyMode =
  | "SUGGEST_ONLY"
  | "AUTO_PLAN"
  | "AUTO_PLAN_WITH_GUARDRAILS"
  | "FULL_AUTONOMY";

interface PredictiveCrossDockPlanner {
  generateCandidates: (
    input: CandidateGenerationInput,
  ) => Promise<CrossDockCandidate[]>;
...
interface AutomationTask {
  ...
  from: {
    locationType: "DOOR" | "LANE" | "STATION" | "ZONE";
    locationId: string;
  };
  to: {
    locationType: "DOOR" | "LANE" | "STATION" | "ZONE";
...
interface CrossDockAutomationOrchestrator {
  createTask: (
    task: Omit<AutomationTask, "id" | "status" | "createdAt" | "updatedAt">,
  ) => Promise<string>;
  ...
  dispatchToVendor: (
    taskId: string,
    vendor: "Locus" | "OTTO" | "GreyOrange" | "Custom",
  ) => Promise<void>;
  updateTaskStatus: (
    taskId: string,
    status: AutomationTaskStatus,
