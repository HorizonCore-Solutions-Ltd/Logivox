...
interface TrailerYardStatus {
  ...
  // Current Status
  status: 'CHECK_IN' | 'WAITING' | 'AT_DOOR' | 'LOADING' | 'UNLOADING' | 'CHECK_OUT' | 'DROP';
...
interface YardMove {
  id: string;
  type: 'SPOT_TO_DOCK' | 'DOCK_TO_SPOT' | 'SPOT_TO_SPOT' | 'GATE_TO_SPOT' | 'SPOT_TO_GATE';
...
interface DwellTimeTracking {
  ...
  // Reasons
  delayReason?: 'WAITING_DOCK' | 'LOADING_DELAY' | 'DOCUMENT_ISSUE' | 'EQUIPMENT_FAILURE';
...
interface AutonomousYardTruck {
  ...
  // Status
  status: 'IDLE' | 'MOVING' | 'COUPLING' | 'UNCOUPLING' | 'CHARGING' | 'MAINTENANCE' | 'ERROR';
...
interface BlockchainYardManagement {
  ...
  recordLoadVerification: (verification: LoadVerification) => Promise<BlockchainTx>;
