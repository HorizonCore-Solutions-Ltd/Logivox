...
interface Kit {
  ...
  // Classification
  type: 'PROMOTIONAL' | 'SUBSCRIPTION' | 'BUNDLE' | 'GIFT_SET' | 'CUSTOM' | 'ASSEMBLY';
  category: string;
...
interface QualityCheckpoint {
  stepNumber: number;
  checkpointType: 'VISUAL' | 'MEASUREMENT' | 'FUNCTION_TEST' | 'CV_SCAN' | 'WEIGHT' | 'DIMENSION';
  description: string;
...
interface WorkOrder {
  ...
  // Status
  status: 'PENDING' | 'STARTED' | 'IN_PROGRESS' | 'PAUSED' | 'COMPLETED' | 'REJECTED';
  currentStep?: number;
  totalSteps: number;
...
interface AssemblyIssue {
  id: string;
  type: 'COMPONENT_SHORTAGE' | 'TOOL_MALFUNCTION' | 'QUALITY_DEFECT' | 'UNCLEAR_INSTRUCTIONS' | 'SAFETY_CONCERN' | 'OTHER';
...
interface KitQualityControl {
  ...
  // Disposition
  disposition: 'ACCEPT' | 'REJECT' | 'REWORK' | 'SCRAP' | 'USE_AS_IS' | 'RETURN';
...
interface QCCheckItem {
  ...
  checkNumber: number;
  description: string;
  checkType: 'VISUAL' | 'MEASUREMENT' | 'FUNCTIONAL' | 'WEIGHT' | 'COUNT' | 'SCAN';
...
interface AIComponentOptimizer {
  ...
  recommendSubstitutions: (component: string, shortage: number) => Promise<Substitution[]>;
  ...
  optimizeWorkstationAssignment: (workOrders: WorkOrder[]) => Promise<Assignment[]>;
...
interface VoiceGuidedAssembly {
  // Voice Prompts
  provideStepInstructions: (step: AssemblyStep, language: string) => Promise<AudioBuffer>;
  provideSafetyWarning: (warning: string) => Promise<AudioBuffer>;
  provideQualityGuidance: (checkpoint: QualityCheckpoint) => Promise<AudioBuffer>;
...
interface ComputerVisionQC {
  ...
  verifyComponentType: (image: Image, expected: string) => Promise<TypeVerification>;
  ...
  checkCompleteness: (image: Image, bom: BillOfMaterials) => Promise<CompletenessCheck>;
...
interface CVQualityCheck {
  ...
  // Analysis
  analysisType: 'COMPONENT_COUNT' | 'COMPONENT_TYPE' | 'DEFECT_DETECTION' | 'MEASUREMENT' | 'COMPLETENESS';
  analysisResult: any;
...
interface GenealogyTracking {
  ...
  recordComponentUsage: (component: string, lot: string, serial: string) => Promise<void>;
  ...
  // Compliance
  generateTraceabilityReport: (kitSerial: string) => Promise<TraceabilityReport>;
