...
class VoiceControlEngine {
   ...
   */
  startWorkflow(
    workflowType: VoiceWorkflowContext["workflowType"],
    ...
    this.speak(
      `Starting ${workflowType} workflow. ${this.getNextStepInstruction()}`,
    );
    this.updateState({
      error: null,
  ...
  completeWorkflow(): void {
    ...
    window.dispatchEvent(
      new CustomEvent("voice-workflow-complete", {
  ...
  private handleConfirmation(transcript: string): boolean {
    ...
    if (
      this.pendingConfirmation.expectedResponses.some((r) =>
    ...
    if (
      this.pendingConfirmation.cancelResponses.some((r) =>
        normalized.includes(r),
      )
    ) {
  ...
  speak(text: string): void {
    ...
    if (this.feedbackConfig.speech.voice) {
      ...
      const voice = voices.find(
        (v) => v.name === this.feedbackConfig.speech.voice,
      );
  ...
  private getNextStepInstruction(): string {
    ...
    return (
      instructions[workflowType]?.[currentStep] || "Continue with next step"
    );
  }
