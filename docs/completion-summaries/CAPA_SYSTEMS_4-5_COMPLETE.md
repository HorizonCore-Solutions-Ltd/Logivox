...
const response = await fetch('/api/capa/effectiveness?status=PENDING&daysBack=90');
...
const createResponse = await fetch('/api/capa/voice', {
  ...
  body: JSON.stringify({
    ...
    context: { location: 'Assembly Line 3', equipment: 'Packaging Machine PM-12' }
  })
});
...
await fetch('/api/capa/voice', {
  ...
  body: JSON.stringify({
    command: 'ADD_ACTION',
    capaId: capa.id,
    transcript: 'Immediately stop production and inspect all packages from last 2 hours'
  })
});
