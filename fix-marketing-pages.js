...
const data = {
  "floor-management": {
    ...
    f1Desc:
      "Group orders automatically based on carrier cut-offs and SLA urgency without manual batching.",
    ...
    f3Desc:
      "Visualize forklift and picker movement to eliminate congestion chokepoints.",
  },
  "load-planning": {
    ...
    f1Desc:
      "Interactive, real-time loadsheets accessible right on mobile scanners and driver tablets.",
    f2Title: "Drop-Sequence Loading",
    f2Desc:
      "Items sorted automatically by delivery route stops (LIFO topology) to prevent dock shuffling.",
    f3Title: "Weight Compliance",
    f3Desc:
      "Prevent DOT fines with automated weight distribution calculations and axle math.",
  },
  "cold-chain": {
    ...
    f1Desc:
      "Read instant state data directly from wireless warehouse temperature and humidity probes.",
    f2Title: "Automated Quarantine",
    f2Desc:
      "Self-executing holds on inventory if environmental bounds are breached mid-transit.",
    f3Title: "Audit-Ready Logs",
    f3Desc:
      "Generate instant compliance logs validating integrity for every pallet in the building.",
  },
};
...
for (const [key, val] of Object.entries(data)) {
  ...
  text = text.replace(
    /Unlock the power of your supply chain data with LogiVox.*?predictive analytics.*?<\/p>/s,
    val.heroDesc + "</p>",
  );
  ...
  text = text.replace(
    /Anticipate order volumes and seasonal trends.*?\"/g,
    `${val.f1Desc}\"`,
  );

  text = text.replace(/"Custom Dashboards"/g, `"${val.f2Title}"`);
  text = text.replace(
    /Build targeted views for different roles.*?\"/g,
    `${val.f2Desc}\"`,
  );

  text = text.replace(/"Performance Tracking"/g, `"${val.f3Title}"`);
  text = text.replace(
    /Monitor KPIs across facilities in real-time.*?\"/g,
    `${val.f3Desc}\"`,
  );
