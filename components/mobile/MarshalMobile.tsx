...
export default function MarshalMobile() {
  ...
  const [selectedLoadSheet, setSelectedLoadSheet] = useState<LoadSheet | null>(
    null,
  );
  ...
  const [loadedContainers, setLoadedContainers] = useState<Set<string>>(
    new Set(),
  );
  ...
  const fetchActiveLoadSheets = async () => {
    try {
      ...
      if (data.loadSheets) {
        ...
        const activeSheets = data.loadSheets.filter(
          (ls: LoadSheet) => ls.bayDoor,
        );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (selectedLoadSheet) {
          const sortedContainers = [...selectedLoadSheet.containers].sort(
            (a, b) => {
              const seqA = a.loadSequence || 999;
              const seqB = b.loadSequence || 999;
              return seqA - seqB;
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className="font-bold text-lg">
                    {selectedLoadSheet.loadSheetNumber}
                  </h3>
                  <p className="text-sm text-gray-400">
                    {selectedLoadSheet.customer.name}
                  </p>
                </div>
                <button
                  onClick={() => {
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-semibold text-green-400">
                    {loadedContainers.size} /{" "}
                    {selectedLoadSheet.totalContainers}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400">Bay Door:</span>
            ...
            {currentContainer ? (
                ...
                <div className="space-y-3 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <p className="font-bold text-lg">
                          {currentContainer.weight}kg
                        </p>
                      </div>
                      <div>
                        <p className="text-blue-200">Volume</p>
                        <p className="font-bold text-lg">
                          {currentContainer.volume.toFixed(2)}m³
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {currentContainer.containerItems && (
                    <div className="flex gap-2">
                      {currentContainer.containerItems.some(
                        (item) => item.isFragile,
                      ) && (
              ...
              <div className="bg-green-600 rounded-lg p-8 text-center">
                ...
                <p className="text-green-100 mb-6">
                  {selectedLoadSheet.totalContainers} containers loaded
                  successfully
                </p>
            ...
            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              ...
              <div className="space-y-2">
                {selectedLoadSheet.containers
                  .sort(
                    (a, b) => (a.loadSequence || 999) - (b.loadSequence || 999),
                  )
                  .map((container) => {
                      ...
                      >
                        <div>
                          ...
                          <div className="text-xs text-gray-400">
                            {container.weight}kg • {container.volume.toFixed(1)}
                            m³
                          </div>
                        </div>
