...
export default function QualityBadge({
  ...
  return (
    <Badge className={`${config.color} ${config.textColor} ${sizeClasses[size]} flex items-center gap-1`}>
...
export function QualityScoreCircle({ score, size = 80 }: { score: number; size?: number }) {
  const getScoreConfig = (score: number) => {
...
export function QualityScoreBar({ score, label }: { score: number; label: string }) {
  const getScoreConfig = (score: number) => {
    if (score >= 90) return { color: 'bg-green-500', textColor: 'text-green-600' };
    if (score >= 75) return { color: 'bg-blue-500', textColor: 'text-blue-600' };
    if (score >= 60) return { color: 'bg-yellow-500', textColor: 'text-yellow-600' };
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className={`font-bold ${config.textColor}`}>{score.toFixed(0)}</span>
      </div>
