...
async function main() {
  ...
  let warehouse = await prisma.warehouse.findFirst({ where: { organizationId: org.id }});
  if (!warehouse) {
  ...
  if (tasksCreated > 0) {
      console.log(`   ✅ SUCCESS: ${tasksCreated} Replenishment Task(s) Generated.`);
  } else {
  ...
  console.log(`   ✅ Short Pick Task Created: ${shortPickTask.id} (Priority: ${shortPickTask.priority})`);
  ...
  if (driverTasks.length >= 2) {
      console.log(`   ✅ Driver Queue Verified: ${driverTasks.length} Pending Tasks found.`);
  } else {
