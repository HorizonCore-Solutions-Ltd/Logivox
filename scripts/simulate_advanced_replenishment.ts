...
async function main() {
    ...
    for (const item of items) {
        ...
        if (daysToEmpty < 7) {
            console.log(`   ⚠️ Prediction: ${item.sku} will stock out in ${daysToEmpty.toFixed(1)} days.`);
    ...
    const loc = await prisma.location.findFirst({ where: { organizationId: org.id, type: "PICKING" }});
    if (loc && items[0]) {
        ...
        console.log(`   ✅ Real-time Alert: Sensor at ${loc.locationCode} reported EMPTY_BIN for ${items[0].sku}`);
    }

    console.log("🎯 Advanced Simulation Complete. Dashboard should now show AI & IoT data.");
}
