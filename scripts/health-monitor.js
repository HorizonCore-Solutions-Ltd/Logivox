...
async function startMonitoring() {
  ...
  console.log(`  Alert Webhook: ${config.alertWebhook ? 'Configured' : 'Not configured'}`);
...
async function sendAlerts(results) {
  ...
  alertHistory = alertHistory.filter(h => Date.now() - h.timestamp < ALERT_COOLDOWN);
...
function displaySummary(results) {
  ...
  if (results.checks.system) {
    ...
    console.log(`  Memory: ${results.checks.system.metrics.memory.percentage}%`);
  ...
  if (results.checks.api) {
    console.log(`\nAPI:`);
    console.log(`  Avg Response Time: ${results.checks.api.metrics.avgResponseTime}ms`);
    console.log(`  Error Rate: ${results.checks.api.metrics.errorRate}%`);
  }

  if (results.checks.database) {
    console.log(`\nDatabase:`);
    console.log(`  Connection Time: ${results.checks.database.metrics.connectionTime}ms`);
    console.log(`  Size: ${results.checks.database.metrics.databaseSize}`);
  }

  if (results.checks.application) {
    console.log(`\nApplication:`);
    console.log(`  Pending Orders: ${results.checks.application.metrics.pendingOrders}`);
    console.log(`  Low Stock Items: ${results.checks.application.metrics.lowStockItems}`);
    console.log(`  Negative Stock: ${results.checks.application.metrics.negativeStock}`);
  }
