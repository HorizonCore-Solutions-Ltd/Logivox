...
function flushModel() {
  ...
  idsToIndx.forEach(idField => {
    let hasIndex = currentModelLines.some(line => line.includes(`@@index([${idField}])`));
    let isId = currentModelLines.some(line => line.includes(`@id`) && line.includes(idField));
    let partOfCompound = currentModelLines.some(line => line.match(new RegExp(`@@(unique|id|index)\\(\\[.*${idField}.*\\]\\)`)));
    
    if (!hasIndex && !isId && !partOfCompound) {
      // We need to add it before the closing brace
      let closeBraceIdx = currentModelLines.findIndex(line => line.trim() === '}');
      if (closeBraceIdx !== -1) {
...
if (modified) {
  ...
  console.log("✅ Zero-Tolerance Schema Patched: Injected missing foreign key indexes.");
} else {
