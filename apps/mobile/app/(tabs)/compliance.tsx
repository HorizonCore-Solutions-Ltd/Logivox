...
export default function ComplianceScreen() {
  ...
  const completeMutation = useMutation({
    mutationFn: (id: string) => completeComplianceTask(id, { notes: "Completed via mobile app" }),
    onSuccess: () => {
  ...
  const score = (scoreData as any)?.overall ?? (scoreData as any)?.score ?? null;
  const tasks: ComplianceTask[] =
    (tasksData as any)?.tasks ?? (Array.isArray(tasksData) ? tasksData : []);
  const logs = (auditData as any)?.items ?? (Array.isArray(auditData) ? auditData : []);

  return (
    <View style={styles.container}>
      {/* Score Header */}
      <View style={styles.scoreHeader}>
        {score !== null ? <ScoreRing score={Math.round(score)} /> : <View style={{ width: 80 }} />}
      ...
      <View style={styles.tabRow}>
        ...
        >
          <Text style={[styles.tabBtnText, tab === "audit" && styles.tabBtnTextActive]}>
            Audit Log
          </Text>
        </TouchableOpacity>
      </View>
        ...
        tasksLoading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <View style={[styles.card, overdue ? styles.cardOverdue : undefined]}>
                  ...
                  {item.dueDate ? (
                    <Text style={[styles.detail, overdue ? { color: "#ef4444" } : undefined]}>
        ...
        <FlatList
          ...
          renderItem={({ item }: { item: any }) => (
            <View style={styles.logRow}>
              <Ionicons name="time-outline" size={14} color="#6b7280" style={{ marginTop: 2 }} />
              <View style={{ flex: 1, marginLeft: 8 }}>
                <Text style={styles.logAction}>{item.action ?? item.event}</Text>
...
const styles = StyleSheet.create({
  ...
  scoreMeta: { flex: 1 },
  scorePeriod: { color: "#f1f5f9", fontSize: 14, fontWeight: "700", marginBottom: 4 },
  ...
  taskTitle: { fontSize: 13, fontWeight: "700", color: "#111827", flex: 1, marginRight: 4 },
  detail: { fontSize: 12, color: "#6b7280", marginBottom: 2 },
  completeBtn: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#10b981", paddingHorizontal: 12, paddingVertical: 7, borderRadius: 6, marginTop: 8, alignSelf: "flex-start" },
  completeBtnText: { color: "#fff", fontSize: 12, fontWeight: "600" },
  logRow: { flexDirection: "row", backgroundColor: "#fff", borderRadius: 8, padding: 12, marginBottom: 8 },
