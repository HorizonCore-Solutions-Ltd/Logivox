    ...
    <View style={styles.card}>
      <View style={styles.cardRow}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          <Ionicons
            name={
              (TYPE_ICONS[item.appointmentType] ??
        ...
        <View
          style={[
            ...
            {
              backgroundColor:
                APPOINTMENT_STATUS_COLORS[item.status] ?? "#6b7280",
            },
          ]}
        >
      ...
      {item.expectedPallets ? (
        <Text style={styles.detail}>
          Expected pallets: {item.expectedPallets}
        </Text>
      ) : null}
...
function StagingZoneCard({
  ...
  const pct =
    zone.utilizationPct ??
    Math.round((zone.currentItems / zone.capacity) * 100);
...
export default function DockScreen() {
  ...
  const { data: appointmentsData, isLoading: apptLoading } = useQuery({
    ...
    queryFn: () =>
      getDockAppointments({ appointmentType: typeFilter ?? undefined }),
  ...
  const markReadyMutation = useMutation({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["stagingZones"] }),
      ...
      {dockStatus ? (
        <View style={styles.statusStrip}>
          {[
            {
              label: "Doors Active",
              value: `${dockStatus.activeDoors}/${dockStatus.totalDoors}`,
              color: "#10b981",
            },
            {
              label: "Today",
              value: dockStatus.appointmentsToday,
              color: "#3b82f6",
            },
            {
              label: "Pending",
              value: dockStatus.pendingCheckIn,
              color: "#f59e0b",
            },
          ].map((s) => (
      ...
      {tab === "appointments" ? (
        <>
          <ScrollView
            ...
            contentContainerStyle={{ padding: 8, gap: 6, flexDirection: "row" }}
          >
