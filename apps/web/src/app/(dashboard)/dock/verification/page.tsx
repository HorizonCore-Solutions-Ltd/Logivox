...
export default function LoadVerification() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [verificationsRes, discrepanciesRes, metricsRes] =
        await Promise.all([
        ...
        <Card className="border-red-300 bg-red-50">
          <CardHeader>
            <CardTitle className="text-red-800 flex items-center gap-2">
              🚨 {criticalDiscrepancies.length} Critical Discrepanc
              {criticalDiscrepancies.length !== 1 ? "ies" : "y"} Require
              Immediate Attention
            </CardTitle>
          </CardHeader>
          <CardContent>
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <span
                              className={`text-xs px-2 py-1 rounded ${getStatusColor(verification.status)}`}
                            >
                              {verification.status}
                            </span>
                          ...
                          <div className="grid grid-cols-4 gap-4 mt-3 text-sm">
                            ...
                            <div>
                              ...
                              <span
                                className={`font-medium ${verification.discrepancies > 0 ? "text-red-600" : "text-green-600"}`}
                              >
                            ...
                            <div className="text-center">
                              <div
                                className={`text-lg font-bold ${
                                  verification.verificationScore >= 95
                                    ? "text-green-600"
                                    : verification.verificationScore >= 85
                                      ? "text-yellow-600"
                                      : "text-red-600"
                                }`}
                              >
        ...
        <TabsContent value="discrepancies" className="space-y-4">
            ...
            <Card className="border-yellow-300">
              <CardContent className="pt-4 text-center">
                <div className="text-2xl font-bold text-yellow-600">
                  {
                    discrepancies.filter(
                      (d) => d.severity === "MEDIUM" && d.status === "OPEN",
                    ).length
                  }
                </div>
            ...
            <Card className="border-blue-300">
              <CardContent className="pt-4 text-center">
                <div className="text-2xl font-bold text-blue-600">
                  {
                    discrepancies.filter(
                      (d) => d.severity === "LOW" && d.status === "OPEN",
                    ).length
                  }
                </div>
          ...
          <div className="space-y-3">
            {discrepancies.map((disc) => (
              <Card
                key={disc.id}
                className={disc.severity === "CRITICAL" ? "border-red-300" : ""}
              >
                <CardContent className="pt-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span
                          className={`text-xs px-2 py-1 rounded border ${getSeverityColor(disc.severity)}`}
                        >
                          {disc.severity}
                        </span>
                        <span
                          className={`text-xs px-2 py-1 rounded ${getStatusColor(disc.status)}`}
                        >
                          {disc.status}
                        </span>
                      </div>
                      ...
                      {disc.status === "RESOLVED" && disc.resolution && (
                        <div className="mt-3 p-2 bg-green-50 rounded text-sm">
                          ...
                          <div className="text-gray-500 text-xs mt-1">
                            {disc.resolvedAt &&
                              new Date(disc.resolvedAt).toLocaleString()}
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-2 text-sm">
                        <div>
                          ...
                          <div className="font-medium">
                            {verifier.verificationsCompleted}
                          </div>
                        </div>
                        <div>
                          ...
                          <div
                            className={`font-medium ${
                              verifier.avgAccuracy >= 95
                                ? "text-green-600"
                                : verifier.avgAccuracy >= 85
                                  ? "text-yellow-600"
                                  : "text-red-600"
                            }`}
                          >
              ...
              <div className="p-4 bg-gray-50 rounded">
                ...
                <ul className="space-y-2 text-sm">
                  <li className="flex items-start gap-2">
                    ...
                    <span>
                      <strong>95% verification accuracy</strong> - Barcode
                      scanning eliminates manual errors
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600">✓</span>
                    <span>
                      <strong>65% reduction in returns</strong> - Catch errors
                      before shipment
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600">✓</span>
                    <span>
                      <strong>50% faster verification</strong> - Automated
                      scanning workflow
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600">✓</span>
                    <span>
                      <strong>100% traceability</strong> - Complete verification
                      audit trail
                    </span>
                  </li>
                </ul>
