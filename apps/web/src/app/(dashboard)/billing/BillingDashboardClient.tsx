...
export default function BillingDashboardClient({
  ...
  const handleRunBilling = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Billing Cycle Complete! ${result.count} Invoices Generated.`,
        );
        ...
        <div className="flex items-center space-x-4 bg-white p-3 rounded-lg border shadow-sm">
          <div className="flex items-center space-x-2">
            <Switch
              id="adminless-mode"
              checked={isAdminless}
              onCheckedChange={setIsAdminless}
            />
            <Label
              htmlFor="adminless-mode"
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-gray-900">
              $
              {stats.invoicedThisMonth.toLocaleString("en-US", {
                minimumFractionDigits: 2,
              })}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-gray-900">
              $
              {stats.vasRevenue.toLocaleString("en-US", {
                minimumFractionDigits: 2,
              })}
            </div>
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <Button onClick={handleRunBilling} disabled={isProcessing}>
              {isProcessing ? (
                ...
                <>
                  <Play className="mr-2 h-4 w-4" /> Run Billing Cycle
                </>
              )}
            </Button>
          </div>
              ...
              <Table>
                ...
                <TableBody>
                  {vasRequests.map((vas: any) => (
                    <TableRow key={vas.id}>
                      <TableCell>
                        {new Date(vas.createdAt).toLocaleDateString()}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant="outline"
                      ...
                      <TableCell
                        ...
                        title={vas.instructions}
                      >
                        {vas.instructions}
                      </TableCell>
                      <TableCell className="text-right font-medium">
                        ${Number(vas.fee).toFixed(2)}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant={
                            vas.status === "COMPLETED" ? "default" : "secondary"
                          }
                        >
                          {vas.status}
                        </Badge>
              ...
              <Table>
                ...
                <TableBody>
                  {chargebacks.map((cb: any) => (
                    <TableRow key={cb.id}>
                      <TableCell>
                        {new Date(cb.date).toLocaleDateString()}
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge variant="outline">{cb.status}</Badge>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
