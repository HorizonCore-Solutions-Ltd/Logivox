...
export default function SlottingOptimizationDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "recommendations" | "analysis" | "heatmap"
  >("recommendations");
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/slotting-optimization?action=${action}`,
      );
  ...
  const runOptimization = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(
          `Optimization complete! ${data.result.itemsToRelocate} items to relocate. Avg improvement: ${data.result.avgImprovement.toFixed(1)}%`,
        );
      }
    } catch (error) {
  ...
  const applyRecommendations = async () => {
    if (!confirm(`Apply ${recommendations.length} slotting recommendations?`))
      return;
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              ML-Based Slotting Optimization
            </h1>
                ...
                {recommendations.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500">
                      No recommendations. Current slotting is optimal!
                    </p>
                  </div>
                ) : (
                      ...
                      <tbody>
                        {recommendations.map((rec, idx) => (
                          ...
                          >
                            ...
                            <td className="py-3 px-4">
                              <span
                                className={`px-2 py-1 rounded text-xs font-semibold ${getPriorityColor(rec.priority)}`}
                              >
                                {rec.priority}
                              </span>
                            </td>
              ...
              <div className="p-6">
                ...
                <div className="border-t pt-6">
                  ...
                  <div className="space-y-4">
                    {Object.entries(analysis.zoneDistribution).map(
                      ([zone, data]) => (
                        <div
                          key={zone}
