...
export default function PlanningBoard() {
  ...
  const { data, isLoading } = useQuery<{ waves: Wave[], recommendations: Recommendation[] }>({
  ...
  const createWaveMutation = useMutation({
    mutationFn: async () => {
        const res = await fetch('/api/operations/planning', {
            method: 'POST',
            body: JSON.stringify({ name: `Wave ${new Date().toLocaleTimeString()}`, type: 'BATCH' })
        });
    ...
    <div className="flex flex-col space-y-6 pb-20">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Orchestrate waves, manage labor, and optimize flow.</p>
        </div>
        <div className="flex gap-2">
            ...
            <Button onClick={() => createWaveMutation.mutate()} disabled={createWaveMutation.isPending}>
      ...
      {simulationMode && (
          <div className="bg-indigo-50 border border-indigo-200 p-4 rounded-lg flex items-center gap-4 animate-in fade-in">
              ...
              <div>
                  <h3 className="font-semibold text-indigo-900">Simulation Enabled</h3>
                  <p className="text-sm text-indigo-700">Changes made here will not affect live operations until applied.</p>
              </div>
          </div>
      )}
      ...
      <div className="grid gap-6 md:grid-cols-3">
            ...
            <Card>
                ...
                <CardContent className="space-y-4">
                    {waves.length === 0 ? (
                        ...
                        waves.map((wave) => (
                            <div key={wave.id} className="flex items-center justify-between p-4 border rounded-lg bg-card hover:shadow-sm transition-all group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-muted-foreground flex gap-2">
                                            ...
                                            <span>• {new Date(wave.createdAt).toLocaleTimeString()}</span>
                                ...
                                <div className="flex items-center gap-8 flex-1 justify-end">
                                    <div className="w-32 space-y-1">
                                        ...
                                        <Progress value={Number(wave.progress)} className="h-2" />
                                    </div>
                                    
                                    <Badge variant={wave.status === 'COMPLETED' ? 'default' : wave.status === 'IN_PROGRESS' ? 'secondary' : 'outline'}>
                                        {wave.status}
                                    </Badge>
        ...
        <div className="space-y-6">
            <Card className="bg-gradient-to-br from-indigo-50 to-white border-indigo-100">
                ...
                <CardContent className="space-y-4">
                    {reco.map((rec) => (
                        <div key={rec.id} className="bg-white p-3 rounded shadow-sm border text-sm">
                            <div className="font-semibold flex justify-between">
                                {rec.zone}
                                <Badge variant="outline" className={rec.action === 'ADD_LABOR' ? 'text-green-600 bg-green-50' : 'text-amber-600 bg-amber-50'}>
                                    {rec.action.replace('_', ' ')}
                                </Badge>
                            </div>
                            <p className="mt-2 text-muted-foreground">{rec.reason}</p>
                            <div className="mt-3 flex gap-2">
                                <Button size="sm" variant="secondary" className="w-full text-xs">Apply</Button>
                                <Button size="sm" variant="ghost" className="w-full text-xs">Ignore</Button>
                            </div>
                        </div>
                    ))}
                    
                    <div className="pt-4 border-t border-indigo-100">
                         <h4 className="font-semibold text-sm mb-2 text-indigo-900">Predicted Congestion</h4>
            ...
            <Card>
                <CardHeader><CardTitle>Labor Snapshot</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                ...
                <CardFooter>
                    <Button variant="ghost" className="w-full text-xs">View detailed labor map &rarr;</Button>
                </CardFooter>
            </Card>
        </div>
      </div>
