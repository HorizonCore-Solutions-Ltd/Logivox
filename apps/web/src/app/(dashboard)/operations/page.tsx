  ...
  if (isError) {
    return (
      <div className="p-8 text-center text-red-500">
        Failed to load operations data. Please check connection.
      </div>
    );
  }
    ...
    <div className="flex flex-col space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Real-time control center for floor operations, planning, and
            marshalling.
          </p>
        </div>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="pallet-term" className="text-right">
                    Pallet
                  </Label>
                  <Input
                    id="pallet-term"
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="tote-term" className="text-right">
                    Container/Tote
                  </Label>
                  <Input
                    id="tote-term"
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="tipping-term" className="text-right">
                    Unloading
                  </Label>
                  <Input
                    id="tipping-term"
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {data?.summary.activePickers || 0} Pickers
            </div>
            <p className="text-xs text-muted-foreground">
              + {data?.summary.activeReplen || 0} Replen /{" "}
              {data?.summary.activePutaway || 0} Putaway
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {data?.summary.wavesReleased || 0} Active
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {data?.summary.loadSheetsReady || 0} Loads
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Math.round(
                (((data?.dockStatus.inboundOccupied ?? 0) +
                  (data?.dockStatus.outboundOccupied ?? 0)) /
                  Math.max(
                    1,
                    (data?.dockStatus.inboundTotal ?? 0) +
                      (data?.dockStatus.outboundTotal ?? 0),
                  )) *
                  100,
              )}
              %
            ...
            <p className="text-xs text-muted-foreground">
              {data?.dockStatus.outboundOccupied} Outbound /{" "}
              {data?.dockStatus.inboundOccupied} Inbound Active
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-7">
        ...
        <Card className="col-span-4 md:col-span-5 h-full">
          <CardHeader>
            ...
            <CardDescription>
              Visual representation of Bay Doors{" "}
              {data?.doors?.length ? `(${data.doors.length} Active)` : ""},
              Zones, and Staff.
            </CardDescription>
          </CardHeader>
          <CardContent className="min-h-[400px] bg-slate-50 border-2 border-dashed rounded-md relative m-4 p-6">
            <Tabs defaultValue="overview">
              ...
              <TabsContent value="overview" className="h-[400px]">
                ...
                <div className="grid grid-cols-12 gap-4 h-full">
                  ...
                  <div className="col-span-2 flex flex-col gap-2 border-r pr-4">
                    ...
                    {!data?.doors?.filter((d) => d.doorType === "INBOUND")
                      .length && (
                      <div className="text-xs text-slate-400 italic">
                        No Inbound Doors Configured
                      </div>
                    )}
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-8 mt-10">
                      <div className="h-24 bg-blue-50 border border-blue-200 rounded p-2 flex flex-col justify-between group cursor-pointer hover:border-blue-400">
                        ...
                        <div className="text-right text-xs text-blue-500">
                          {data?.laborMap?.filter(
                            (u) => u.currentZone === "ZONE-A",
                          ).length || 0}{" "}
                          Staff Active
                        </div>
                      </div>
                        ...
                        <div className="flex justify-between items-end">
                          <div className="flex gap-2">
                            {Array.from({
                              length: Math.min(
                                5,
                                data?.summary.loadSheetsReady || 0,
                              ),
                            }).map((_, i) => (
                              <Truck
                                key={i}
                            ...
                            {(data?.summary.loadSheetsReady || 0) > 5 && (
                              <span className="text-xs text-green-600 self-center">
                                +{(data?.summary.loadSheetsReady || 0) - 5} more
                              </span>
                            )}
                          </div>
                          <div className="text-xs text-green-600">
                            {data?.laborMap?.filter(
                              (u) => u.currentZone === "MARSHALLING",
                            ).length || 0}{" "}
                            Staff
                          </div>
                        </div>
                  ...
                  <div className="col-span-2 flex flex-col gap-2 border-l pl-4">
                    <div className="text-xs font-bold text-slate-400 mb-2 text-right">
                      OUTBOUND DOCKS
                    </div>
                    {data?.doors
                      ?.filter((d) => d.doorType === "OUTBOUND")
                      .map((door) => (
                        <div
                          key={door.id}
                          className={`p-3 rounded border text-center ${
                    ...
                    {!data?.doors?.filter((d) => d.doorType === "OUTBOUND")
                      .length && (
                      <div className="text-xs text-slate-400 italic text-right">
                        No Outbound Doors Configured
                      </div>
                    )}
                  </div>
                </div>
              ...
              <TabsContent value="labor" className="h-[400px] overflow-y-auto">
                <div className="grid grid-cols-3 gap-6">
                  {[
                    ...
                    <div
                      key={zone}
                      className="border rounded-lg bg-white p-3 shadow-sm min-h-[150px]"
                    >
                      ...
                      <div className="space-y-2">
                        {data?.laborMap
                          ?.filter((u) => u.currentZone === zone)
                          .map((u) => (
                            <div
                              key={u.id}
                              className="flex items-center gap-2 p-1 hover:bg-slate-50 rounded cursor-move"
                            >
                        ...
                        {data?.laborMap?.filter((u) => u.currentZone === zone)
                          .length === 0 && (
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        className="w-full mt-2 text-xs border border-dashed text-slate-400 hover:text-slate-600"
                      >
                        + Assign Staff
                      </Button>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
                  <div className="space-y-4 pr-2 overflow-y-auto">
                    <h3 className="font-semibold flex items-center gap-2">
                      <Award className="h-5 w-5 text-amber-500" /> Top
                      Performers (Shift)
                    </h3>
                  ...
                  <div className="bg-slate-50 p-4 rounded-lg flex flex-col justify-between">
                    <div>
                      <h3 className="font-semibold mb-4 text-sm flex items-center gap-2">
                        <Zap className="h-4 w-4 text-purple-500" /> Current
                        Shift Stats
                      </h3>
              ...
              <TabsContent
                value="quality"
                className="h-[400px] gap-4 grid grid-cols-1 overflow-y-auto"
              >
                <div className="grid grid-cols-3 gap-4 h-24">
                  <div className="bg-red-50 border border-red-200 rounded p-4 flex flex-col justify-between">
                    <div className="text-xs font-bold text-red-700 flex items-center gap-2">
                      <AlertTriangle className="h-4 w-4" /> SAFETY
                    </div>
                    <div className="text-2xl font-bold text-red-800">
                      {qualityData?.incidents?.filter(
                        (i) => i.data?.incidentType === "Safety",
                      ).length || 0}
                    </div>
                  </div>
                  <div className="bg-blue-50 border border-blue-200 rounded p-4 flex flex-col justify-between">
                    <div className="text-xs font-bold text-blue-700 flex items-center gap-2">
                      <Construction className="h-4 w-4" /> DAMAGED
                    </div>
                    <div className="text-2xl font-bold text-blue-800">
                      {qualityData?.incidents?.filter(
                        (i) => i.data?.incidentType === "Damaged",
                      ).length || 0}
                    </div>
                  </div>
                  <div className="bg-amber-50 border border-amber-200 rounded p-4 flex flex-col justify-between">
                    <div className="text-xs font-bold text-amber-700 flex items-center gap-2">
                      <AlertOctagon className="h-4 w-4" /> LABELING
                    </div>
                    <div className="text-2xl font-bold text-amber-800">
                      {qualityData?.incidents?.filter(
                        (i) => i.data?.incidentType === "Labeling",
                      ).length || 0}
                    </div>
                  </div>
                </div>
                ...
                <div className="border rounded-lg overflow-hidden">
                  ...
                  <div className="bg-white divide-y max-h-[250px] overflow-y-auto">
                    {qualityData?.incidents?.length === 0 && (
                      <div className="p-4 text-center text-sm text-muted-foreground">
                        No incidents reported recently.
                      </div>
                    )}
                    {qualityData?.incidents?.map(
                      (incident: QualityIncident) => (
                        <div
                          key={incident.id}
                          className="p-3 flex justify-between hover:bg-slate-50"
                        >
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              >
                                {incident.data?.incidentType || "General"}
                              </Badge>
                            ...
                            <div className="text-xs text-muted-foreground mt-1 flex gap-4">
                              <span>
                                Reported by:{" "}
                                {incident.data?.reportedBy ? "Staff" : "System"}
                              </span>
                              <span>
                                Location: {incident.data?.location || "N/A"}
                              </span>
                            </div>
                          </div>
              ...
              <TabsContent value="returns" className="h-[400px]">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
                  ...
                  <div className="space-y-4">
                    <h3 className="font-semibold flex items-center gap-2">
                      <Undo2 className="h-5 w-5 text-blue-500" /> Inbound{" "}
                      {terms.returns} Flow
                    </h3>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 bg-white border rounded shadow-sm">
                        ...
                        <div className="text-3xl font-bold mt-2">
                          {data?.returns?.pendingTrailers || 0}
                        </div>
                          ...
                          {[1, 2].map((i) => (
                            ...
                            >
                              <div>
                                ...
                                <div className="text-xs text-muted-foreground">
                                  Dock {8 + i} • {terms.tipping} in progress
                                </div>
                              </div>
                              <Badge
                                variant="outline"
                  ...
                  <div className="bg-slate-50 p-4 rounded border">
                    <h3 className="font-semibold flex items-center gap-2 mb-4">
                      <Recycle className="h-5 w-5 text-green-600" /> Reusable
                      Assets (Empties)
                    </h3>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-white p-3 rounded shadow-sm border-l-4 border-l-amber-500">
                        <div className="flex justify-between mb-2">
                          ...
                          <Badge variant="secondary">
                            {data?.returns.reusables.pallets.onHand} On Floor
                          </Badge>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-center text-xs">
                          <div className="bg-slate-50 p-2 rounded">
                            <div className="font-bold text-slate-700">
                              {data?.returns.reusables.pallets.dispatched}
                            </div>
                            <div className="text-slate-400">Dispatched</div>
                          </div>
                          <div className="bg-slate-50 p-2 rounded">
                            <div className="font-bold text-slate-700">
                              {data?.returns.reusables.pallets.damaged}
                            </div>
                      ...
                      <div className="bg-white p-3 rounded shadow-sm border-l-4 border-l-blue-500">
                        <div className="flex justify-between mb-2">
                          <span className="font-bold">
                            {terms.container}s / Totes
                          </span>
                          <Badge variant="secondary">
                            {data?.returns.reusables.totes.onHand} On Floor
                          </Badge>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-center text-xs">
                          <div className="bg-slate-50 p-2 rounded">
                            <div className="font-bold text-slate-700">
                              {data?.returns.reusables.totes.dispatched}
                            </div>
                            <div className="text-slate-400">Dispatched</div>
                          </div>
                          <div className="bg-slate-50 p-2 rounded">
                            <div className="font-bold text-slate-700">
                              {data?.returns.reusables.totes.damaged}
                            </div>
                            <div className="text-red-400">Damaged</div>
                          </div>
                          <div className="bg-slate-50 p-2 rounded">
              ...
              <TabsContent
                value="measure"
                className="h-[400px] flex items-center justify-center bg-slate-100 rounded"
              >
                <div className="text-center space-y-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Select two points on the map to measure linear distance.
                  </p>
                </div>
              </TabsContent>
            </Tabs>
        ...
        <Card className="col-span-3 md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Real-time blocker feed from the floor
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 max-h-[600px] overflow-y-auto">
            ...
            {data?.alerts.map((alert, i) => (
              <div
                key={alert.id || i}
                className={`p-3 rounded-lg border-l-4 text-sm shadow-sm animate-in fade-in slide-in-from-right-4 duration-300 ${
                  alert.severity === "HIGH"
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground flex justify-between items-center">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    className="h-6 text-xs hover:bg-white"
                  >
                    Resolve
                  </Button>
                </div>
