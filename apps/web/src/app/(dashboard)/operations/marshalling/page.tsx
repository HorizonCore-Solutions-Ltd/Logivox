...
export default function MarshallingBoard() {
  ...
  const { data, isLoading } = useQuery<{ loadSheets: LoadSheet[], history: LoadSheet[], stagingLanes: StagingLane[] }>({
  ...
  const handlePrintLoadSheet = async (sheetObj: LoadSheet) => {
    try {
        ...
        // Details
        let yPos = 55;
        doc.text(`Scheduled Departure: ${fullSheet.scheduledDeparture ? new Date(fullSheet.scheduledDeparture).toLocaleString() : 'TBD'}`, 14, yPos);
        yPos += 6;
        doc.text(`Vehicle Type: ${fullSheet.vehicleType?.name || 'Standard'}`, 14, yPos);
        yPos += 6;
        doc.text(`Assigned Bay Door: ${fullSheet.bayDoor?.name || 'Unassigned'}`, 14, yPos);
        ...
        // Table of Containers
        const tableBody = fullSheet.containers?.map((c: any) => [
  ...
  const createLoadSheetMutation = useMutation({
    mutationFn: async () => {
        const res = await fetch('/api/operations/marshalling', {
            method: 'POST',
            body: JSON.stringify({ carrierName: 'FedEx Express', stagingLane: 'S3' })
        });
  ...
  const onTimeRate = history.length > 0 ? Math.round((onTimeDepartures / history.length) * 100) : 100;

  const avgLoadTime = history.reduce((acc, curr) => {
      if (!curr.startedLoadingAt || !curr.finishedLoadingAt) return acc;
      return acc + (new Date(curr.finishedLoadingAt).getTime() - new Date(curr.startedLoadingAt).getTime());
  }, 0) / (history.length || 1) / (1000 * 60); // in minutes

  // Find a suitable status display - check variance against schedule
  const getStatusDisplay = (sheet: LoadSheet) => {
      const scheduled = sheet.scheduledDeparture ? new Date(sheet.scheduledDeparture) : null;
      const isLate = scheduled && new Date() > scheduled;
      
      if (sheet.status === 'LOADING') {
          return <Badge className={isLate ? "bg-amber-600" : "bg-green-600"}>{isLate ? "Late Loading" : "Loading"}</Badge>;
      }
  ...
  return (
    <div className="flex flex-col space-y-6 pb-20">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Marshalling & Load Planning</h1>
          <p className="text-muted-foreground">Manage staging lanes, load sheets, and outbound dispatch.</p>
        </div>
        <div className="flex gap-2">
            <Button onClick={() => createLoadSheetMutation.mutate()} disabled={createLoadSheetMutation.isPending}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${onTimeRate < 95 ? "text-amber-600" : "text-green-600"}`}>{onTimeRate}%</div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">From scan start to seal</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
             <div className="text-2xl font-bold">{loadSheets.filter(l => l.status === 'LOADING').length}</div>
             <p className="text-xs text-muted-foreground">{loadSheets.length} pending staging</p>
          </CardContent>
        </Card>
         <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Late Departures</CardTitle>
            <AlertTriangle className="h-4 w-4 text-red-500" />
          </CardHeader>
          <CardContent>
             <div className="text-2xl font-bold text-red-600">{history.length - onTimeDepartures}</div>
             <p className="text-xs text-muted-foreground">Require root cause analysis</p>
          </CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="active" className="space-y-4">
            {loadSheets.length === 0 ? (
                  ...
                  {loadSheets.map((sheet) => (
                      <Card key={sheet.id} className={`hover:shadow-md transition-shadow border-l-4 ${sheet.status === 'LOADING' ? 'border-l-green-500' : 'border-l-slate-200'}`}>
                          ...
                          <CardContent className="text-sm space-y-4">
                              <div className="grid grid-cols-2 gap-2 mt-2">
                                  <div className="flex items-center gap-2 text-muted-foreground">
                                      ...
                                      {sheet.scheduledDeparture ? new Date(sheet.scheduledDeparture).toLocaleString() : "TBD"}
                                  </div>
                                  <div className="flex items-center gap-2 text-muted-foreground">
                              ...
                              <div className="flex items-center gap-2 p-2 bg-slate-50 rounded border border-slate-100">
                                   ...
                                   <span className="font-mono text-xs">{sheet.vehicleType?.name || "Standard 53ft"}</span>
                              </div>
                              ...
                              <div className="space-y-2 pt-2 border-t">
                                  ...
                                  <Progress value={sheet.status === 'LOADING' ? 45 : 0} className="h-1.5" />
                              </div>
                          </CardContent>
                          <CardFooter className="bg-muted/30 pt-4 flex gap-2">
                              {/* PDF Download */}
                              <Button variant="ghost" size="icon" className="h-9 w-9 text-blue-600 hover:text-blue-800 hover:bg-blue-50"
                              ...
                              <Button variant="ghost" size="icon" className="h-9 w-9 text-slate-500 hover:text-slate-700 hover:bg-slate-100"
                                  ...
                                  onClick={(e) => { 
                                      e.stopPropagation();
                                      window.open(`/api/operations/handover?loadSheetId=${sheet.id}`, '_blank');
                                  }}
                              >
                                  <Share2 className="h-4 w-4" />
                              </Button>
                              <Button variant="ghost" size="icon" className="h-9 w-9 text-amber-600 hover:text-amber-700 hover:bg-amber-50"
                              ...
                              {sheet.status === 'LOADING' ? (
                                   <Button size="sm" className="w-full bg-slate-900 hover:bg-slate-800 text-white"
                                      onClick={() => { setSelectedSheet(sheet); setDepartDialogOpen(true); }}
                                   >
                                      Depart <ArrowRight className="ml-1 h-3 w-3" />
                                   </Button>
                              ) : (
                                   <Button size="sm" variant="outline" className="w-full"
                                      onClick={() => updateStatusMutation.mutate({ id: sheet.id, status: 'LOADING' })}
                                   >
                                      Start Loading
                                   </Button>
                              )}
        ...
        <TabsContent value="history">
             <Card>
                <CardHeader>
                    ...
                    <CardDescription>Recent history and delay analysis</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="space-y-4">
                        {history.length === 0 ? <p className="text-muted-foreground">No historical data available.</p> : history.map(h => (
                            <div key={h.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 border rounded-lg gap-4">
                                <div>
                                    ...
                                    <div className="text-sm text-muted-foreground mt-1">
                                        Scheduled: {h.scheduledDeparture ? new Date(h.scheduledDeparture).toLocaleString() : 'N/A'}
                                    </div>
                                </div>
                                <div className="text-right flex flex-col items-end">
                                    {h.delayReason ? (
                                        <div className="flex items-center text-red-600 text-sm font-medium bg-red-50 px-2 py-1 rounded">
                                            <AlertTriangle className="h-3 w-3 mr-1" /> Late: {h.delayReason}
                                        </div>
                                    ) : (
                                    ...
                                    <div className="text-xs text-muted-foreground mt-1">
                                        Departed: {h.actualDeparture ? new Date(h.actualDeparture).toLocaleTimeString() : 'Unknown'}
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <CardContent>
                    <div className="grid grid-cols-4 gap-4">
                        {['S1', 'S2', 'S3', 'S4'].map((laneId) => {
                            const lane = lanes.find(l => l.id === laneId) || { id: laneId, status: "AVAILABLE" };
                            const occupiedSheet = loadSheets.find(ls => ls.id === lane.loadSheetId);

                            return (
                                <div key={laneId} className={`
                                ...
                                `}>
                                    <div className="flex justify-between font-bold text-lg text-slate-700">
                                        {laneId}
                                        <Badge variant={lane.status === 'OCCUPIED' ? 'secondary' : 'outline'}>
                                            {lane.status}
                                        </Badge>
                                    </div>
                                    ...
                                    {occupiedSheet ? (
                                        <div className="text-sm">
                                            <div className="font-semibold text-blue-800">{occupiedSheet.loadSheetNumber}</div>
                                            <div className="text-xs text-blue-600 truncate">{occupiedSheet.carrierName}</div>
                                        </div>
                                    ) : (
                                        <div className="text-center text-slate-400 text-sm">Empty</div>
                                    )}
                                </div>
                            );
                        })}
      ...
      <Dialog open={departDialogOpen} onOpenChange={setDepartDialogOpen}>
          <DialogContent>
              <DialogHeader>
                  ...
                  <DialogDescription>
                      Review safety check and confirm departure for {selectedSheet?.loadSheetNumber}.
                  </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                  ...
                  <div className="grid gap-2">
                       ...
                       <div className="p-3 bg-red-50 border border-red-200 rounded text-sm flex gap-2 items-start opacity-70">
                            ...
                            <div>
                                <div className="font-semibold text-red-900">Pending Safety Verification</div>
                                <p className="text-red-700 text-xs">Verify load securement, seal integrity, and driver clearance.</p>
                            </div>
                       </div>
                  </div>
                  ...
                  <div className="grid gap-2">
                      ...
                      {selectedSheet?.scheduledDeparture && new Date() > new Date(selectedSheet.scheduledDeparture) ? (
                            <div className="p-2 bg-red-50 text-red-800 text-sm rounded border border-red-200 flex items-center">
                                <AlertTriangle className="h-4 w-4 mr-2" />
                                Departure is late by {Math.round((new Date().getTime() - new Date(selectedSheet.scheduledDeparture).getTime()) / 60000)} minutes.
                            </div>
                      ) : (
                  ...
                  <div className="flex items-start space-x-2 pt-2 border-t">
                      ...
                      <div className="grid gap-1.5 leading-none">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Digital Signature: Releasing this load certifies all safety checks are passed.
                        </p>
                      </div>
                  </div>
              </div>
              <DialogFooter>
                  <Button variant="outline" onClick={() => setDepartDialogOpen(false)}>Cancel</Button>
                  <Button 
                  ...
                  >
                      {updateStatusMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <CheckCircle2 className="mr-2 h-4 w-4"/>}
      ...
      <Dialog open={incidentDialogOpen} onOpenChange={setIncidentDialogOpen}>
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    Log an issue for Load {selectedSheet?.loadSheetNumber}. This will be flagged for review.
                </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                    ...
                    <Select onValueChange={setIncidentType} value={incidentType}>
                        ...
                        <SelectContent>
                            <SelectGroup>
                                ...
                                <SelectItem value="Safety">Safety / Unsafe Loading</SelectItem>
                                <SelectItem value="Labeling">Labeling Error</SelectItem>
                                <SelectItem value="Documentation">Missing Paperwork</SelectItem>
                            </SelectGroup>
                            <SelectGroup>
                                <SelectLabel>Execution</SelectLabel>
                                <SelectItem value="Damaged">Damaged Goods</SelectItem>
                                <SelectItem value="Driver">Driver Missed Window</SelectItem>
                                <SelectItem value="Inventory">Inventory Shortage</SelectItem>
                            </SelectGroup>
                        </SelectContent>
                    </Select>
                </div>
                    ...
                    <div className="flex gap-2">
                        {['LOW', 'MEDIUM', 'HIGH'].map((s) => (
                            <Button 
                                ...
                                className={incidentSeverity === s ? 
                                    (s === 'HIGH' ? 'bg-red-600 hover:bg-red-700' : s === 'MEDIUM' ? 'bg-amber-600 hover:bg-amber-700' : 'bg-blue-600 hover:bg-blue-700') 
                                    : ''}
                            >
                                {s}
                            </Button>
            ...
            <DialogFooter>
                ...
                <Button onClick={() => reportIncidentMutation.mutate()} disabled={!incidentType || !incidentDesc || reportIncidentMutation.isPending}>
                    {reportIncidentMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
                    Submit Report
                </Button>
            </DialogFooter>
        </DialogContent>
