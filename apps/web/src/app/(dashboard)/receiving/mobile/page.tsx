...
export default function ReceivingMobile() {
    ...
    try {
      ...
      if (data.success && data.result) {
        ...
        if (task) {
          ...
          alert(
            `Shipment ${data.result.shipmentNumber} found but not in your active list. Refreshing...`,
          );
          fetchMobileData();
        }
        setScanInput("");
  ...
  const handleSubmitReceipt = async () => {
    ...
    // Build GRN Payload
    const itemsPayload = selectedTask.items
      .map((item) => {
          ...
          <Card className="bg-green-50 border-green-200 shadow-lg animate-in fade-in zoom-in duration-300">
            <CardHeader>
              <CardTitle className="text-green-800 flex items-center">
                <Check className="h-6 w-6 mr-2 bg-green-200 rounded-full p-1" />{" "}
                Receipt Complete
              </CardTitle>
              <CardDescription className="text-green-700">
                GRN: {completedResult.grnNumber}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <h4 className="font-semibold text-sm uppercase tracking-wide text-green-800 border-b border-green-200 pb-1">
                  Put-away Instructions
                </h4>
                {completedResult.items.map((item: any) => (
                  <div
                    key={item.id}
                    className="text-sm pb-2 border-b border-green-100 last:border-0"
                  >
      ...
      <Tabs defaultValue="tasks" className="w-full">
              ...
              >
                <CardContent className="p-4">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span className="text-[10px] bg-slate-100 text-slate-600 px-2 py-1 rounded-full border border-slate-200 font-medium">
                      {task.status}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between text-xs text-slate-400 border-t pt-2 mt-2">
                    ...
                    <span>
                      {task.appointmentTime
                        ? new Date(task.appointmentTime).toLocaleDateString()
                        : "No Appt"}
                    </span>
                  </div>
                </CardContent>
        ...
        <TabsContent value="recent">
          <div className="text-center py-8 text-gray-400 text-sm">
            History shown in full dashboard
          </div>
        </TabsContent>
      </Tabs>
    </div>
