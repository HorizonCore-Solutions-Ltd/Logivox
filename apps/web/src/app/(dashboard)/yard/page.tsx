...
export default function YardDashboardPage() {
  ...
  // --- Queries ---
  const { data: appointments, isLoading: isLoadingApts } = useQuery<DockAppointment[]>({
    queryKey: ['yard-appointments'],
    queryFn: async () => (await fetch('/api/yard/appointments')).json()
  });

  const { data: locations, isLoading: isLoadingLocs } = useQuery<YardLocation[]>({
  ...
  const checkInMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Check-In Successful", description: "Vehicle logged at gate." });
  ...
  // --- Metrics ---
  const activeCount = appointments?.filter(a => ['CHECKED_IN', 'IN_PROGRESS'].includes(a.status)).length || 0;
  ...
  return (
    <div className="flex flex-col space-y-6 p-6">
      <div className="flex flex-col space-y-2">
        ...
        <p className="text-muted-foreground">Manage your yard, appointments, and dock schedules from a central hub.</p>
      </div>

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Active In Yard</CardTitle>
            <Truck className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{activeCount}</div>
            <p className="text-xs text-muted-foreground">Checked-in Appointments</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Trailers waiting for shunter</p>
          </CardContent>
        </Card>
         <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Tabs defaultValue="appointments" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="gate"><LogIn className="mr-2 h-4 w-4"/> Gate Console</TabsTrigger>
          <TabsTrigger value="map"><MapIcon className="mr-2 h-4 w-4"/> Yard Map</TabsTrigger>
          <TabsTrigger value="moves"><Move className="mr-2 h-4 w-4"/> Task Queue</TabsTrigger>
        </TabsList>
              ...
              <Table>
                ...
                <TableBody>
                  {appointments?.map(apt => (
                    <TableRow key={apt.id}>
                      <TableCell>{new Date(apt.scheduledStart).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</TableCell>
                      <TableCell>{apt.carrierName}</TableCell>
                      <TableCell>{apt.trailerNumber}</TableCell>
                      <TableCell>{apt.yardLocation?.locationCode || "Unassigned"}</TableCell>
                      <TableCell>
                         <Badge variant={apt.status === 'CHECKED_IN' ? 'secondary' : 'outline'}>{apt.status}</Badge>
                      </TableCell>
                       <TableCell>
                        {apt.gateEntries?.[0] ? new Date(apt.gateEntries[0].entryTime).toLocaleTimeString() : "-"}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
                ...
                <CardTitle>Recent Gate Activity</CardTitle>
                 <Dialog open={isCheckInOpen} onOpenChange={setIsCheckInOpen}>
                  <DialogTrigger asChild>
                    <Button size="sm"><Plus className="mr-2 h-4 w-4" /> Check-In</Button>
                  </DialogTrigger>
                  <DialogContent className="sm:max-w-[425px]">
                    ...
                    <form onSubmit={handleSubmit} className="grid gap-4 py-4">
                      ...
                      {/* Appointment Link */}
                      <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="appointment" className="text-right">Link Appt</Label>
                        <Select 
                          value={formData.appointmentId} 
                          onValueChange={(val) => setFormData({...formData, appointmentId: val})}
                        >
                          <SelectTrigger className="w-[280px]">
                          ...
                          <SelectContent>
                            ...
                            {appointments?.filter(a => a.status === 'SCHEDULED' || a.status === 'CHECKED_IN').map(a => (
                               <SelectItem key={a.id} value={a.id}>
                                 {a.carrierName} ({new Date(a.scheduledStart).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})})
                               </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      ...
                      <div className="grid grid-cols-4 items-center gap-4">
                       ...
                       {/* Trailer Number */}
                       <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="trailer" className="text-right">Trailer #</Label>
                        <Input
                          id="trailer"
                       ...
                       {/* Gate Number */}
                       <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="gate" className="text-right">Gate</Label>
                        <Select 
                          value={formData.gateNumber} 
                          onValueChange={(val) => setFormData({...formData, gateNumber: val})}
                        >
                          <SelectTrigger className="w-[280px]">
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="GATE-3">Gate 3 (Express)</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                  ...
                  <TableBody>
                    {gateEntries?.map(entry => (
                       <TableRow key={entry.id}>
                          ...
                          <TableCell><Badge>{entry.direction}</Badge></TableCell>
                          <TableCell>{entry.vehicleNumber || entry.trailerNumber}</TableCell>
        ...
        <TabsContent value="map" className="space-y-6">
           ...
           <div className="space-y-2">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
               {locations?.filter(l => l.locationType.includes('DOCK')).map(loc => (
                 <Card key={loc.id} className={loc.isOccupied ? "bg-blue-50 border-blue-200" : "bg-card"}>
                   <CardHeader className="p-4 flex flex-row items-center justify-between space-y-0 pb-2">
                       <span className="font-bold text-lg">{loc.locationCode}</span>
                       <Badge variant={loc.locationType.includes('LOADING') ? "default" : "secondary"} className="text-[10px]">
               ...
               {(!locations || locations.filter(l => l.locationType.includes('DOCK')).length === 0) && (
                  <div className="col-span-full text-sm text-muted-foreground italic">No dock doors found.</div>
               )}
             </div>
           </div>
           ...
           <div className="space-y-2 pt-4 border-t">
             ...
             <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-2">
               {locations?.filter(l => !l.locationType.includes('DOCK')).map(loc => (
                 <Card key={loc.id} className={loc.isOccupied ? "bg-orange-50 border-orange-200" : "bg-muted/30"}>
                   <CardContent className="p-3">
                     <div className="flex justify-between items-start mb-2">
                       <span className="font-bold text-sm">{loc.locationCode}</span>
                       <Badge variant="outline" className="text-[9px] h-4 px-1">{loc.locationType.split('_')[0]}</Badge>
                     </div>
                     {loc.isOccupied ? (
                        <div className="text-xs text-orange-800 font-medium flex items-center gap-1">
                           <Truck className="h-3 w-3"/>
                           <span className="truncate">{loc.appointments?.[0]?.carrierName || "Trailer"}</span>
                        </div>
                     ) : (
                        <div className="text-xs text-muted-foreground flex items-center gap-1">
                           <div className="h-2 w-2 rounded-full bg-green-400" /> Empty
                        </div>
                     )}
                   </CardContent>
                 </Card>
               ...
               {(!locations || locations.filter(l => !l.locationType.includes('DOCK')).length === 0) && (
                  <div className="col-span-full text-sm text-muted-foreground italic">No yard spaces defined.</div>
               )}
             </div>
           </div>
        </TabsContent>
        ...
        <TabsContent value="moves">
          <Card>
             <CardHeader><CardTitle>Yard Move Tasks</CardTitle></CardHeader>
             <CardContent>
                {isLoadingMoves ? <div>Loading...</div> : (
                   <Table>
                     <TableHeader>
                       <TableRow>
                     ...
                     <TableBody>
                        {moves?.map(move => (
                           <TableRow key={move.id}>
                              ...
                              <TableCell><Badge>{move.status}</Badge></TableCell>
                              <TableCell>
