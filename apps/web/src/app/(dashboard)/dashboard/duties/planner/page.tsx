...
export default function DutyPlannerPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/duties-types?limit=200")
        .then((r) => r.json())
        .catch(() => ({ dutyTypes: [] })),
      fetch("/api/shifts?limit=100")
        .then((r) => r.json())
        .catch(() => ({ shifts: [] })),
    ]).then(([dt, sh]) => {
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Rule-based auto-assignment: skills, certs, zone preference, fatigue
            avoidance.
          </p>
        </div>
            ...
            <div className="space-y-3">
              {lines.map((line, idx) => {
                ...
                return (
                  ...
                  >
                    <span className="text-xs text-gray-400 mt-2.5 w-4">
                      {idx + 1}
                    </span>
                    {/* Duty type */}
                    <div className="flex-1">
                      <select
                        ...
                        onChange={(e) =>
                          updateLine(line.id, "dutyTypeId", e.target.value)
                        }
                      ...
                      >
                        ...
                        {dutyTypes.length === 0 && (
                          <option disabled>No duty types</option>
                        )}
                        {dutyTypes.map((d) => (
                          <option key={d.id} value={d.id}>
                            {d.name} ({d.category.replace(/_/g, " ")}) ·{" "}
                            {d.defaultDuration}min
                          </option>
                        ))}
                      </select>
                      {dt &&
                        (dt.requiredSkills.length > 0 ||
                          dt.requiredCerts.length > 0) && (
                          <p className="text-xs text-blue-600 mt-1">
                            Needs:{" "}
                            {[...dt.requiredSkills, ...dt.requiredCerts].join(
                              ", ",
                            )}
                          </p>
                        )}
                    ...
                    <div className="w-28">
                      ...
                      <input
                        ...
                        value={line.zoneId}
                        onChange={(e) =>
                          updateLine(line.id, "zoneId", e.target.value)
                        }
                        placeholder="Zone A"
                        className="w-full text-sm border border-gray-200 rounded-lg px-2 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    ...
                    <div className="w-36">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateLine(line.id, "scheduledStart", e.target.value)
                        }
          ...
          {errors.length > 0 && (
            <div className="mb-4 p-3 bg-red-50 rounded-lg border border-red-200">
              {errors.map((e, i) => (
                <p
                  key={i}
                  className="text-sm text-red-700 flex items-center gap-1.5"
                >
        ...
        {summary && results && (
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            {/* Summary bar */}
            <div
              className={`flex items-center gap-3 p-4 rounded-lg mb-5 ${summary.unassigned === 0 ? "bg-green-50 border border-green-200" : "bg-yellow-50 border border-yellow-200"}`}
            >
              ...
              <div>
                <p
                  className={`text-sm font-semibold ${summary.unassigned === 0 ? "text-green-800" : "text-yellow-800"}`}
                >
                  {summary.assignmentRate}% assignment rate — {summary.assigned}
                  /{summary.total} duties filled
                </p>
                {summary.unassigned > 0 && (
                  <p className="text-xs text-yellow-700 mt-0.5">
                    {summary.unassigned} duties unassigned — insufficient
                    eligible staff for those skills/certs.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              {results.map((r, idx) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <Link
                      href={`/dashboard/duties/${r.dutyId}`}
                  ...
                  {r.employeeId ? (
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500">
                        {r.rule} · {Math.round(r.confidence * 100)}% match
                      </p>
                    </div>
                  ) : (
