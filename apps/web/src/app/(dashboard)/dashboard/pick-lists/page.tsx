    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Track warehouse picking tasks and progress</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Total", value: stats.total, icon: <ClipboardList className="h-4 w-4 text-blue-500" />, color: "text-blue-600" },
          { label: "Pending", value: stats.pending, icon: <Clock className="h-4 w-4 text-amber-500" />, color: "text-amber-600" },
          { label: "In Progress", value: stats.inProgress, icon: <Play className="h-4 w-4 text-purple-500" />, color: "text-purple-600" },
          { label: "Completed", value: stats.completed, icon: <CheckCircle className="h-4 w-4 text-green-500" />, color: "text-green-600" },
        ].map((s) => (
          <Card key={s.label}>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 text-muted-foreground">
              ...
              <p>No pick lists found. Approve a sales order to generate one.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {filtered.map((pl) => {
                  const cfg = STATUS_CFG[pl.status] || { label: pl.status, variant: "outline" as const };
                  const pct = progressPct(pl);
                  return (
                    <TableRow key={pl.id}>
                      <TableCell className="font-mono font-medium">{pl.pickListNumber}</TableCell>
                      <TableCell className="font-mono text-sm">{pl.salesOrder?.soNumber}</TableCell>
                      <TableCell>{pl.salesOrder?.customer?.name}</TableCell>
                      <TableCell>
                        <span className="text-sm">{pl.warehouse?.name}</span>
                        <span className="text-xs text-muted-foreground block">{pl.warehouse?.code}</span>
                      </TableCell>
                      <TableCell>{pl.assignedTo?.name || <span className="text-muted-foreground text-sm">Unassigned</span>}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-right">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => router.push(`/dashboard/pick-lists/${pl.id}`)}
                        >
