...
export default function NewRMAPage() {
  ...
  const handleSearchOrder = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setOrderItems([
          { id: "item-1", productName: "Wireless Headset", sku: "WH-001", quantity: 2, unitPrice: 150.00 },
          { id: "item-2", productName: "Ergonomic Mouse", sku: "EM-002", quantity: 1, unitPrice: 45.00 },
        ]);
        setStep(2);
        setLoading(false);
      }, 800);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "RMA Created",
        description: "Return Merchandise Authorization has been created successfully.",
      });
      ...
      <div className="grid gap-6">
        ...
        <Card className={step === 1 ? "border-primary" : ""}>
          <CardHeader>
            ...
            <CardDescription>Enter the original Sales Order number.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex gap-4">
              ...
              {step === 1 && (
                <Button onClick={handleSearchOrder} disabled={loading || !orderNumber}>
                  {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  Find Order
                </Button>
              )}
               {step > 1 && (
                <Button variant="outline" onClick={() => { setStep(1); setOrderItems([]); setSelectedItems({}); }}>
                  Change
                </Button>
              )}
            </div>
          ...
          <form onSubmit={handleSubmit}>
            <Card className="mb-6">
              <CardHeader>
                ...
                <CardDescription>Specify quantities for each item being returned.</CardDescription>
              </CardHeader>
                    ...
                    {orderItems.map(item => (
                        <div key={item.id} className="p-4 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">SKU: {item.sku} • Sold: ${item.unitPrice}</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="text-sm text-muted-foreground">Qty Purchased: {item.quantity}</span>
                                <Input 
                                    type="number" 
                                    min="0" 
                                    max={item.quantity}
