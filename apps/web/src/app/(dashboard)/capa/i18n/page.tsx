...
export default function MultiLanguagePage() {
  ...
  const translateCapaReport = async () => {
    ...
    try {
      ...
      const searchResponse = await fetch(`/api/capa?capaNumber=${capaNumberToTranslate}`)
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">Translate CAPAs for global operations - 14 languages supported</p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="text-blue-600 text-sm font-medium">Supported Languages</div>
            <div className="text-3xl font-bold text-blue-700 mt-1">{Object.keys(languages).length}</div>
          </div>

          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="text-green-600 text-sm font-medium">Total Translations</div>
            <div className="text-3xl font-bold text-green-700 mt-1">{stats.totalTranslations}</div>
          </div>

          <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
            <div className="text-purple-600 text-sm font-medium">Languages in Use</div>
            <div className="text-3xl font-bold text-purple-700 mt-1">{stats.languageCount}</div>
          </div>

          <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
            <div className="text-orange-600 text-sm font-medium">Compliance Regions</div>
            <div className="text-3xl font-bold text-orange-700 mt-1">{regionalCompliance ? Object.keys(regionalCompliance).length : 0}</div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border rounded-lg p-6">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-2">CAPA Number</label>
              <input
                type="text"
          ...
          {translatedReport && (
            <div className="mt-4 bg-gradient-to-br from-blue-50 to-purple-50 border-2 border-blue-300 rounded-lg p-6">
              ...
              <div className="space-y-3">
                <div>
                  <span className="text-sm font-medium text-gray-600">CAPA Number:</span>
                ...
                <div>
                  ...
                  <div className="text-gray-700">{translatedReport.description}</div>
                </div>

                {translatedReport.rootCause && (
                  <div>
                    <span className="text-sm font-medium text-gray-600">Root Cause:</span>
                    <div className="text-gray-700">{translatedReport.rootCause}</div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                  <div>
                    ...
                    <div className="font-bold text-blue-600">{translatedReport.status}</div>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-gray-600">Priority:</span>
                    <div className="font-bold text-orange-600">{translatedReport.priority}</div>
                  </div>
                </div>
              </div>
      ...
      {regionalCompliance && (
        <div className="bg-white border rounded-lg p-6">
          <h2 className="font-bold text-lg mb-4">🏛️ Regional Compliance Requirements</h2>
          <div className="space-y-4">
            {Object.entries(regionalCompliance).map(([key, compliance]: [string, any]) => (
              <div key={key} className="border rounded-lg p-4 hover:bg-gray-50">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-gray-700">{compliance.regulation}</span>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 mt-1">
                      {compliance.languages.map((lang: string) => (
                        <span key={lang} className="px-2 py-1 bg-green-100 text-green-700 rounded text-xs">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded p-3 mt-2">
                    <div className="font-medium text-blue-900">📝 Documentation Requirements:</div>
                    <div className="text-blue-800 text-xs mt-1">{compliance.documentationRequirements}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            <div className="font-bold text-green-900 mb-2">🌍 Language Support:</div>
            <ul className="space-y-1 text-green-800">
              <li>• 14 languages (English, Spanish, French, German, Chinese, Japanese, etc.)</li>
          ...
          <div>
            <div className="font-bold text-green-900 mb-2">📄 Translation Capabilities:</div>
            <ul className="space-y-1 text-green-800">
          ...
          <div>
            <div className="font-bold text-green-900 mb-2">🏛️ Regulatory Compliance:</div>
            <ul className="space-y-1 text-green-800">
        ...
        <div className="mt-6 pt-6 border-t border-green-300">
          <div className="text-sm text-green-800">
            <strong>Pro Tip:</strong> Click any language card to set it as your preferred language. 
            All future CAPAs will default to your chosen language!
          </div>
        </div>
      </div>
    </div>
