...
export default function ClosureVerificationPage() {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'Actions': return '⚡'
      case 'Effectiveness': return '✅'
      case 'Training': return '📚'
      case 'Documentation': return '📄'
      case 'Approval': return '👔'
      case 'Customer Impact': return '👥'
      default: return '📋'
    }
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">Automated validation of closure criteria before closing CAPAs</p>
      </div>

      {/* Verification Input */}
      <div className="bg-white border rounded-lg p-6">
        <h2 className="font-bold text-lg mb-4">Verify CAPA Closure Readiness</h2>
      ...
      {verificationResult && (
        <div className="space-y-6">
          ...
          <div className={`border-2 rounded-lg p-6 ${
            verificationResult.overallStatus === 'APPROVED' ? 'bg-green-50 border-green-500' :
            verificationResult.overallStatus === 'REJECTED' ? 'bg-red-50 border-red-500' :
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className={`text-2xl font-bold mb-2 ${
                  verificationResult.overallStatus === 'APPROVED' ? 'text-green-800' :
                  verificationResult.overallStatus === 'REJECTED' ? 'text-red-800' :
                  'text-yellow-800'
                }`}>
                  {verificationResult.overallStatus === 'APPROVED' ? 'READY TO CLOSE' :
                   verificationResult.overallStatus === 'REJECTED' ? 'CANNOT CLOSE' :
                   'NEEDS REVIEW'}
                </div>
                <div className={`text-lg ${
                  verificationResult.overallStatus === 'APPROVED' ? 'text-green-700' :
                  verificationResult.overallStatus === 'REJECTED' ? 'text-red-700' :
                  'text-yellow-700'
                }`}>
            ...
            <div className="mt-6 bg-white rounded-lg p-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-2xl font-bold text-blue-600">{verificationResult.completionScore}%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-4">
                <div
                  className={`h-4 rounded-full ${
                    verificationResult.completionScore >= 90 ? 'bg-green-500' :
                    verificationResult.completionScore >= 70 ? 'bg-yellow-500' :
                    'bg-red-500'
                  }`}
          ...
          {verificationResult.checkResults && verificationResult.checkResults.length > 0 && (
            <div className="bg-white border rounded-lg p-6">
                ...
                {verificationResult.checkResults.map((check: CheckResult) => (
                  <div
                    ...
                    className={`border-l-4 p-4 rounded ${
                      check.critical ? 'bg-red-50 border-red-500' : 'bg-yellow-50 border-yellow-500'
                    }`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="font-bold text-lg">{check.requirement}</div>
                    ...
                    <div className="mt-3 space-y-2">
                      ...
                      <div>
                        ...
                        <span className="text-gray-600 text-sm">{check.description}</span>
                      </div>
                      {check.recommendation && (
                        <div className="bg-blue-50 border border-blue-200 rounded p-3 mt-2">
                          <div className="font-medium text-blue-900">💡 Recommendation:</div>
                          <div className="text-blue-800 text-sm mt-1">{check.recommendation}</div>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="bg-white border rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {(verificationResult.verification.checkResults as CheckResult[]).map((check) => (
                <div
                  key={check.id}
                  className={`flex items-center gap-3 p-3 rounded border ${
                    check.passed ? 'bg-green-50 border-green-200' :
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-600">{check.category}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {verificationResult.canClose && (
            <div className="bg-green-50 border border-green-300 rounded-lg p-6 text-center">
              <div className="text-green-800 font-bold text-lg mb-2">
                ✅ This CAPA has passed all critical checks and is ready to be closed!
              </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h3 className="font-bold text-lg mb-4">🤖 How Automated Verification Works</h3>
        <div className="space-y-3 text-gray-700">
          <div className="flex gap-3">
            <span className="font-bold text-blue-600">1.</span>
            <div>
              <div className="font-medium">Enter CAPA Number</div>
              <div className="text-sm">System retrieves complete CAPA data including actions, training, effectiveness, etc.</div>
            </div>
          </div>
          <div className="flex gap-3">
            <span className="font-bold text-blue-600">2.</span>
            <div>
              <div className="font-medium">AI Validation Engine Runs 14 Checks</div>
              <div className="text-sm">Automatically validates root cause, actions, effectiveness, training, documentation, approvals, and customer impacts</div>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            <div>
              ...
              <div className="text-sm">8 CRITICAL checks must pass. 6 optional checks improve score but don't block closure.</div>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            <div>
              ...
              <div className="text-sm">✅ APPROVED (ready), ❌ REJECTED (critical failures), ⚠️ NEEDS REVIEW (optional failures)</div>
            </div>
          </div>
        </div>
