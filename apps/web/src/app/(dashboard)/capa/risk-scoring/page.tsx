    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Risk Priority Number = Severity × Occurrence × Detection</p>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="text-red-600 text-sm font-medium">CRITICAL RISK</div>
            <div className="text-3xl font-bold text-red-700 mt-2">{stats.criticalRisk}</div>
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
            ...
            <div className="text-3xl font-bold text-orange-700 mt-2">{stats.highRisk}</div>
            <div className="text-xs text-orange-600 mt-1">RPN 100-199</div>
          </div>

          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="text-yellow-700 text-sm font-medium">MEDIUM RISK</div>
            <div className="text-3xl font-bold text-yellow-800 mt-2">{stats.mediumRisk}</div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="text-3xl font-bold text-blue-700 mt-2">{stats.lowRisk}</div>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            <div className="text-2xl font-bold text-green-800 mt-1">{stats.riskReduced}</div>
            <div className="text-xs text-green-600 mt-1">Post-action RPN lower than initial</div>
          </div>

          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="text-green-700 text-sm">Average Risk Reduction</div>
            <div className="text-2xl font-bold text-green-800 mt-1">{stats.averageReduction}%</div>
            <div className="text-xs text-green-600 mt-1">Average improvement after CAPA</div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Calculate Risk Priority Number (RPN)</h2>
              <button onClick={() => setShowCalculator(false)} className="text-gray-500 hover:text-gray-700 text-2xl">&times;</button>
            </div>

            <div className="space-y-6">
              {/* CAPA Selection */}
              <div>
                <label className="block text-sm font-medium mb-2">CAPA Number/ID</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium mb-2">
                  Severity (Impact): <span className="text-blue-600 font-bold">{severity}</span>
                </label>
                <input
                  type="range"
                  min="1"
                ...
                <div className="text-xs text-gray-600 mt-1">
                  {severity >= 9 ? '9-10: Catastrophic (death, serious injury, FDA recall)' :
                   severity >= 7 ? '7-8: Critical (injury, product recall, regulatory action)' :
                   severity >= 5 ? '5-6: Moderate (customer dissatisfaction, performance impact)' :
                   severity >= 3 ? '3-4: Low (minor impact)' :
              ...
              <div>
                <label className="block text-sm font-medium mb-2">
                  Occurrence (Frequency): <span className="text-blue-600 font-bold">{occurrence}</span>
                </label>
                <input
                  type="range"
                  min="1"
                ...
                />
                <div className="text-xs text-gray-600 mt-1">
                  {occurrence >= 9 ? '9-10: Almost certain (≥33% of time)' :
                   occurrence >= 7 ? '7-8: Frequent (5-12% of time)' :
                   occurrence >= 5 ? '5-6: Moderate (0.25-1.25%)' :
                   occurrence >= 3 ? '3-4: Low (0.007-0.05%)' :
                   '1-2: Rare (≤0.0007%)'}
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium mb-2">
                  Detection (Ability to Catch): <span className="text-blue-600 font-bold">{detection}</span>
                </label>
                <input
                  type="range"
                  min="1"
                ...
                />
                <div className="text-xs text-gray-600 mt-1">
                  {detection >= 9 ? '9-10: No detection method (defects reach customer)' :
                   detection >= 7 ? '7-8: Very low detection chance' :
                   detection >= 5 ? '5-6: Moderate detection' :
                   detection >= 3 ? '3-4: High detection chance' :
              ...
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border-2 border-blue-300 rounded-lg p-6">
                <div className="text-center">
                  <div className="text-sm text-gray-600 mb-2">Risk Priority Number (RPN)</div>
                  <div className="text-5xl font-bold text-blue-700 mb-2">{calculatedRPN}</div>
                  <div className="text-sm text-gray-600">
                    {severity} (Severity) × {occurrence} (Occurrence) × {detection} (Detection)
                  </div>
                  <div className={`inline-block mt-3 px-4 py-2 rounded text-sm font-bold ${
                    calculatedRPN >= 200 ? 'bg-red-100 text-red-800' :
                    calculatedRPN >= 100 ? 'bg-orange-100 text-orange-800' :
                    calculatedRPN >= 50 ? 'bg-yellow-100 text-yellow-800' :
                    'bg-blue-100 text-blue-800'
                  }`}>
                    {calculatedRPN >= 200 ? 'CRITICAL RISK' :
                     calculatedRPN >= 100 ? 'HIGH RISK' :
                     calculatedRPN >= 50 ? 'MEDIUM RISK' :
                     'LOW RISK'}
                  </div>
                </div>
              </div>
      ...
      <div className="bg-white border rounded-lg overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase">CAPA</th>
              ...
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase">Initial RPN</th>
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase">Post-Action RPN</th>
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase">Reduction</th>
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase">Risk Level</th>
          ...
          <tbody className="divide-y">
            {loading ? (
              ...
              <tr>
                <td colSpan={10} className="px-4 py-8 text-center text-gray-500">No risk assessments found</td>
              </tr>
            ) : (
              filteredAssessments.map((assessment) => (
                <tr key={assessment.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <div className="font-medium text-blue-600">{assessment.capa.capaNumber}</div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-sm max-w-xs truncate">{assessment.capa.title}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className={`font-bold text-lg ${
                      assessment.initialRPN >= 200 ? 'text-red-700' :
                      assessment.initialRPN >= 100 ? 'text-orange-600' :
                      assessment.initialRPN >= 50 ? 'text-yellow-600' :
                      'text-blue-600'
                    }`}>
                  ...
                  <td className="px-4 py-3 text-center">
                    {assessment.postActionRPN ? (
                      <div className="font-bold text-green-700">{assessment.postActionRPN}</div>
                    ) : (
                  ...
                  <td className="px-4 py-3 text-center">
                    {assessment.reductionPercent ? (
                      <div className="text-green-700 font-bold">↓ {assessment.reductionPercent}%</div>
                    ) : (
                      <span className="text-gray-400">-</span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`inline-block px-2 py-1 rounded text-xs ${
                      assessment.capa.status === 'CLOSED' ? 'bg-green-100 text-green-800' :
                      assessment.capa.status === 'IN_PROGRESS' ? 'bg-blue-100 text-blue-800' :
