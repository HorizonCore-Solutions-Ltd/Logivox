...
export default function MobileCAPAPage() {
  ...
  const [filter, setFilter] = useState<"all" | "assigned" | "open" | "overdue">(
    "assigned",
  );
  ...
  const [newCapaPriority, setNewCapaPriority] = useState<
    "HIGH" | "MEDIUM" | "LOW"
  >("MEDIUM");
  ...
  const createMobileCapa = async () => {
    ...
    try {
      const capaData = {
        ...
        deviceInfo: {
          deviceType: /iPhone|iPad|iPod/.test(navigator.userAgent)
            ? "iOS"
      ...
      if (!isOnline) {
        ...
        const pending = JSON.parse(
          localStorage.getItem("pendingCAPAs") || "[]",
        );
        pending.push({
          ...capaData,
  ...
  const filteredCapas =
    filter === "overdue"
      ? capas.filter(
          (c) =>
            new Date(c.targetCompletionDate) < new Date() &&
          ...
          filteredCapas.map((capa) => (
            ...
            >
              ...
              <div className="flex gap-2 mb-3 text-xs">
                ...
                {capa.mobileMetadata?.photoUrls &&
                  capa.mobileMetadata.photoUrls.length > 0 && (
            ...
            {!isOnline && (
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                ...
                <div className="text-orange-700 text-sm mt-1">
                  CAPA will be saved locally and synced when connection is
                  restored.
                </div>
              </div>
            )}
