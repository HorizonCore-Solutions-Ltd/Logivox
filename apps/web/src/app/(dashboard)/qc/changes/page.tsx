...
const MOCK: ChangeRequest[] = [
  {
    id: "1",
    changeNumber: "CCR-2026-001",
    title: "Update CNC tooling parameters for Shaft OD operation",
    description:
      "Adjust cutting speed and feed rate to improve tool life and reduce cycle time.",
    changeType: "PROCESS",
    priority: "MEDIUM",
    status: "APPROVED",
    ...
    implementedAt: null,
    impactAssessment:
      "Reduces tool cost by ~18%. No dimensional impact expected.",
  },
  {
    id: "2",
    changeNumber: "CCR-2026-002",
    title: "Qualify alternate adhesive supplier",
    description:
      "Current supplier delivery time exceeds SLA. Adding backup supplier.",
    changeType: "SUPPLIER",
    priority: "HIGH",
    status: "UNDER_REVIEW",
    submittedBy: "Supply Chain Mgr",
    approvedBy: null,
  ...
  {
    id: "3",
    changeNumber: "CCR-2025-015",
    title: "Revise inspection criteria for coating thickness",
    description: "Updated per customer specification rev change.",
    changeType: "DOCUMENT",
    priority: "HIGH",
    status: "CLOSED",
    submittedBy: "QA Engineer",
    approvedBy: "QA Manager",
    submittedAt: "2025-11-10T08:00:00Z",
    targetDate: "2025-12-01",
    implementedAt: "2025-11-28T00:00:00Z",
    impactAssessment: "Affects 3 SOPs. Training required for 15 operators.",
  },
];
...
export default function ChangesPage() {
  ...
  useEffect(() => {
    fetch("/api/qc/changes")
      .then((r) => r.json())
      .then((d) =>
        setChanges(Array.isArray(d.changes ?? d) ? (d.changes ?? d) : MOCK),
      )
  ...
  const stats = {
    ...
    open: changes.filter((c) => !["CLOSED", "REJECTED"].includes(c.status))
      .length,
    approved: changes.filter((c) => c.status === "APPROVED").length,
    emergency: changes.filter((c) => c.priority === "EMERGENCY").length,
  };
  const filtered =
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-gray-600">
              Submit, review, and approve changes to processes, products,
              documents, equipment, and suppliers.
            </p>
          </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex gap-2 items-center mb-1 flex-wrap">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-semibold ${TYPE_COLOR[c.changeType]}`}
                        >
                          {c.changeType}
                        </span>
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-semibold ${PRI_COLOR[c.priority]}`}
                        >
                          {c.priority}
                        </span>
                        <span
