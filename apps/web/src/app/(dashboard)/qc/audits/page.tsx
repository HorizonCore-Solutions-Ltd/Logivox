...
const DEMO_AUDITS: Audit[] = [
  {
    ...
    auditNumber: "AUD-001",
    type: "INTERNAL",
    scope: "Warehouse Operations — Zone A",
    standard: "ISO 9001:2015",
    auditDate: new Date(Date.now() - 7 * 86400000).toISOString(),
    location: "Site Alpha",
    auditorName: "Sarah Mitchell",
    auditorOrg: "Internal QA",
    auditeeName: "Ops Team",
    status: "REPORT_ISSUED",
    summary: "Minor process deviations found in putaway procedures.",
    findings: [
      {
        id: "F1",
        findingNumber: "AUD-001-F001",
        severity: "MINOR",
        status: "CAPA_ASSIGNED",
        clause: "8.5.1",
        category: "Process Control",
        description: "Putaway confirmation steps not consistently followed.",
      },
      {
        id: "F2",
        findingNumber: "AUD-001-F002",
        severity: "OBSERVATION",
        status: "OPEN",
        clause: "7.5.3",
        category: "Documentation",
        description: "Label records not archived within SLA window.",
      },
    ],
  },
  {
    ...
    auditNumber: "AUD-002",
    type: "SUPPLIER",
    scope: "Supplier Quality — Acme Packaging",
    standard: "ISO 9001:2015",
    auditDate: new Date(Date.now() - 14 * 86400000).toISOString(),
    location: "Supplier Site",
    auditorName: "James Okafor",
    auditorOrg: "Procurement QA",
    status: "COMPLETED",
    findings: [
      {
        id: "F3",
        findingNumber: "AUD-002-F001",
        severity: "MAJOR",
        status: "OPEN",
        clause: "4.4.1",
        category: "Process Compliance",
        description:
          "Critical process step omitted in packaging line—product integrity risk.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    auditNumber: "AUD-005",
    type: "CERTIFICATION",
    scope: "ISO 45001 Recertification",
    standard: "ISO 45001:2018",
    auditDate: new Date(Date.now() - 28 * 86400000).toISOString(),
    location: "HQ",
    auditorName: "BSI Certifying Body",
    status: "CLOSED",
    findings: [
      {
        id: "F4",
        findingNumber: "AUD-005-F001",
        severity: "MINOR",
        status: "CLOSED",
        clause: "6.1.2.3",
        category: "Risk Assessment",
        description:
          "Hazard register not reviewed within required annual cycle.",
      },
    ],
  },
];
...
export default function AuditReportPage() {
  ...
  const [activeTab, setActiveTab] = useState<"list" | "schedule" | "findings">(
    "list",
  );
  ...
  const dSchedule = schedule ?? {
    ...
    recentlyClosed: DEMO_AUDITS.filter(
      (a) => a.status === "CLOSED" || a.status === "REPORT_ISSUED",
    ),
    summary: { overdueCount: 1, upcomingCount: 1, recentlyClosedCount: 2 },
  };

  const allFindings = dAudits.flatMap((a) =>
    a.findings.map((f) => ({
      ...f,
  ...
  const severityPieData = [
    {
      name: "Major",
      value: dMetrics.findingsBySeverity.MAJOR,
      fill: "#ef4444",
    },
    {
      name: "Minor",
      value: dMetrics.findingsBySeverity.MINOR,
      fill: "#f59e0b",
    },
    {
      name: "Observation",
      value: dMetrics.findingsBySeverity.OBSERVATION,
      fill: "#3b82f6",
    },
  ];
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Internal · Supplier · Regulatory · Certification audits — findings,
            schedule &amp; closure tracking
          </p>
        </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-300 rounded-lg">
          ...
          <div>
            <p className="font-semibold text-red-800">
              {majorOpen.length} Major Finding{majorOpen.length > 1 ? "s" : ""}{" "}
              Require Immediate Action
            </p>
            <p className="text-sm text-red-700 mt-0.5">
              {majorOpen
                .map((f) => `${f.auditNumber} — ${f.findingNumber}`)
                .join(" · ")}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-5">
            ...
            <p className="text-xs text-gray-400 mt-1">
              {dMetrics.completionRate}% completed
            </p>
          </CardContent>
        </Card>
        <Card
          className={
            dMetrics.openMajorFindings > 0
        ...
        >
          <CardContent className="pt-5">
            ...
            <p
              className={`text-3xl font-bold ${dMetrics.openMajorFindings > 0 ? "text-red-700" : "text-green-700"}`}
            >
              {dMetrics.openMajorFindings}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              {dMetrics.findingsBySeverity.MINOR} minor ·{" "}
              {dMetrics.findingsBySeverity.OBSERVATION} obs
            </p>
          </CardContent>
        </Card>
        <Card
          className={
            dMetrics.closureRate >= 80
              ? "border-green-200 bg-green-50"
        ...
        >
          <CardContent className="pt-5">
            ...
            <p
              className={`text-3xl font-bold ${dMetrics.closureRate >= 80 ? "text-green-700" : "text-yellow-700"}`}
            >
        ...
        <Card
          className={
            dSchedule.summary.overdueCount > 0
        ...
        >
          <CardContent className="pt-5">
            ...
            <p
              className={`text-3xl font-bold ${dSchedule.summary.overdueCount > 0 ? "text-orange-700" : "text-gray-700"}`}
            >
              {dSchedule.summary.overdueCount}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              {dSchedule.summary.upcomingCount} upcoming (30d)
            </p>
          </CardContent>
        </Card>
      </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  {dAudits.map((audit) => {
                    ...
                    const majorCount = audit.findings.filter(
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            ...
                            />
                            <Badge
                              className={`text-xs border ${cfg.badge} px-1.5`}
                            >
                              {cfg.label}
                            </Badge>
                          </div>
              ...
              <div className="border-t bg-muted/20 p-5 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold">
                    {selectedAudit.auditNumber} — {selectedAudit.scope}
                  </h3>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedAudit.findings.map((f) => (
                        <div
                          key={f.id}
                          className={`rounded-lg border p-3 text-sm ${f.severity === "MAJOR" ? "bg-red-50 border-red-200" : f.severity === "MINOR" ? "bg-yellow-50 border-yellow-200" : "bg-blue-50 border-blue-200"}`}
                        >
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <Badge
                              className={`text-xs border ${FINDING_SEVERITY_CFG[f.severity].badge}`}
                            >
                              {f.severity}
                            </Badge>
                            {f.clause && (
        ...
        <div className="space-y-4">
            ...
            <Card className="border-orange-300">
              ...
              <CardContent className="space-y-2">
                {dSchedule.overdue.map((a) => (
                  <div
                    key={a.id}
                    className="flex items-center justify-between p-3 bg-orange-50 border border-orange-200 rounded-lg text-sm"
                  >
                    <div>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-blue-500" /> Upcoming (30
                  days) — {dSchedule.upcoming.length}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
          ...
          {dSchedule.recentlyClosed.length > 0 && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-green-500" /> Recently
                  Closed
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
      ...
      {activeTab === "findings" && (
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              All findings requiring action — sorted by severity
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-2">
                {[...openFindings]
                  ...
                  .map((f) => (
                    <div
                      key={f.id}
                      className={`rounded-lg border p-4 ${f.severity === "MAJOR" ? "bg-red-50 border-red-300" : f.severity === "MINOR" ? "bg-yellow-50 border-yellow-200" : "bg-blue-50 border-blue-200"}`}
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            <Badge variant="outline" className="text-xs">
                              {AUDIT_TYPE_LABEL[(f as any).auditType] ?? ""}
                            </Badge>
                            {(f as any).clause && (
                              <span className="text-xs text-muted-foreground">
                                Clause {(f as any).clause}
                              </span>
                            )}
                          </div>
                          <p className="text-sm">{f.description}</p>
