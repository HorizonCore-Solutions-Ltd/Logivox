...
export default function ExecuteInspectionPage() {
  ...
  const handleComplete = async (result: "PASS" | "FAIL") => {
    ...
    const updates = Object.keys(results).map((cpId) => ({
      ...
      data: {
        status:
          results[cpId].status === "PASS" || results[cpId].status === "FAIL"
            ? results[cpId].status
            : "COMPLETED",
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {inspection.category} • Qty: {inspection.quantity} •{" "}
            ...
            <CardContent className="space-y-4">
              {inspection.checkpoints?.length === 0 && (
                <div className="text-muted-foreground italic">
                  No checkpoints defined for this inspection.
                </div>
              )}
            ...
            <CardContent>
              {inspection.status === "COMPLETED" ? (
                <div className="space-y-4">
                  <div
                    className={`p-4 rounded-lg text-center font-bold text-xl ${inspection.result === "PASS" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                  >
                    Inspection {inspection.result}ED
                  </div>
                  {inspection.result === "FAIL" && (
                    <Link
                      href={`/quality/capa/new?sourceType=INSPECTION&sourceId=${inspection.id}`}
                      className="block"
                    >
            ...
            <CardContent className="space-y-4 text-sm">
              <div>
                ...
                <div className="text-muted-foreground truncate">
                  {inspection.inventoryId}
                </div>
              </div>
              <div>
                <span className="font-medium">Reference:</span>
                <div className="text-muted-foreground">
                  {inspection.referenceType} {inspection.referenceId || "N/A"}
                </div>
              </div>
              <div>
