...
export default function CAPADetailsPage() {
  ...
  const { data: capa, isLoading, isError } = useQuery({
    queryKey: ['capa', id],
    queryFn: async () => {
  ...
  if (isError || !capa) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        <p className="text-destructive font-medium">Error loading CAPA details</p>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold tracking-tight">{capa.capaNumber}</h1>
              <Badge variant={statusColors[capa.status] || "outline"}>{capa.status}</Badge>
            </div>
        ...
        <div className="flex gap-2">
          ...
          {capa.status === 'IN_PROGRESS' && (
            <Button onClick={() => handleStatusChange('RESOLVED')} variant="default">
              Mark Resolved
            </Button>
          )}
        ...
        <div className="md:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-muted-foreground whitespace-pre-wrap">{capa.description}</p>
            </CardContent>
            <CardFooter className="bg-muted/50 p-4 text-sm text-muted-foreground flex justify-between">
              <div className="flex items-center gap-2">
                <AlertTriangle className="h-4 w-4" />
                Category: <span className="font-medium text-foreground">{capa.category}</span>
              </div>
              <div className="flex items-center gap-2">
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                   ...
                   <div className="text-sm text-muted-foreground">
                     {capa.dueDate ? new Date(capa.dueDate).toLocaleDateString() : "No due date"}
                   </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
             ...
             <CardContent className="text-sm space-y-2">
                <div>
                  <span className="text-muted-foreground">Source Type:</span> Use {capa.sourceType}
                </div>
                {capa.sourceId && (
                  <div>
                    <span className="text-muted-foreground">ID:</span> <span className="font-mono">{capa.sourceId}</span>
                  </div>
                )}
             </CardContent>
          </Card>
