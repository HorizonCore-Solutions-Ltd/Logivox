...
export default function CarrierPortalPage() {
    ...
    // Booking Modal State
    const [selectedSlot, setSelectedSlot] = useState<{ dockId: string, time: string, dockName: string } | null>(null);
    ...
    useEffect(() => {
        ...
        fetch(`/api/portal/carrier/appointments?warehouseId=${selectedWarehouse}&date=${selectedDate}`)
            .then(res => res.json())
            .then(data => {
    ...
    const handleBook = async () => {
        ...
        try {
            ...
            setLoadRef("");
            // Refresh schedule
            const newAppts = await fetch(`/api/portal/carrier/appointments?warehouseId=${selectedWarehouse}&date=${selectedDate}`).then(r => r.json());
    ...
    const isSlotBooked = (dockId: string, hour: number) => {
        const slotStart = new Date(`${selectedDate}T${hour.toString().padStart(2, '0')}:00:00`).getTime();
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-gray-900">Carrier Portal</h1>
                    <p className="text-gray-500">Book dock appointments and manage deliveries.</p>
                </div>
            </div>
                 ...
                 <Card>
                    ...
                    <CardContent>
                        {loading ? (
                            ...
                            <div className="text-center py-8 text-gray-500">No docks found for this warehouse.</div>
                        ) : (
                            <div className="relative overflow-x-auto">
                                <div className="min-w-[800px]">
                                    ...
                                    <div className="flex border-b">
                                        ...
                                        {hours.map(h => (
                                            <div key={h} className="flex-1 p-2 text-center font-medium text-sm">
                                                {h}:00
                                            </div>
                                        ))}
                                    </div>
                                                ...
                                                return (
                                                    <div key={h} className="flex-1 p-1">
                                                        <button 
                                                            ...
                                                            onClick={() => setSelectedSlot({ dockId: dock.id, dockName: dock.name, time: timeString })}
                                                        >
                                                            Available
                                                        </button>
                                                    </div>
            ...
            <Dialog open={!!selectedSlot} onOpenChange={(open) => !open && setSelectedSlot(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {selectedSlot && `Dock: ${selectedSlot.dockName} | Date: ${selectedDate} | Time: ${selectedSlot.time}`}
                        </DialogDescription>
                    </DialogHeader>
