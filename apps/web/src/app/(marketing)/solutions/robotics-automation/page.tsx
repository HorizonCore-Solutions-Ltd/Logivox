...
const DEVICE_TYPES = [
  {
    ...
    desc: "Goods-to-person, replenishment, putaway",
  },
  {
    type: "AGV",
    icon: "🚗",
    label: "Automated Guided Vehicle",
    desc: "Heavy pallet transport, long-aisle runs",
  },
  {
    type: "COBOT",
    icon: "🦾",
    label: "Collaborative Robot",
    desc: "Packing station assistance, pick support",
  },
  {
    type: "ROBOT_ARM",
    icon: "🦿",
    label: "Robot Arm / Articulated",
    desc: "Palletising, depalletising, kitting",
  },
  {
    type: "CONVEYOR",
    icon: "⚙️",
    label: "Conveyor System",
    desc: "Inbound induction, outbound dispatch lines",
  },
  {
    type: "SORTER",
    icon: "🔀",
    label: "Cross-Belt / Shoe Sorter",
    desc: "Order sorting, returns processing",
  },
  {
    type: "AS_RS",
    icon: "🏗️",
    label: "AS/RS Crane",
    desc: "High-density automated storage and retrieval",
  },
];
        ...
        <div className="container-enterprise">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold mb-4">
              Every Automation Device. One Dashboard.
            </h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">
              LogiVox natively supports 7 automation device types with
              purpose-built dispatch logic for each. No generic "integration" —
              genuine operational intelligence per device type.
            </p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {DEVICE_TYPES.map((device) => (
              <Card
                key={device.type}
                className="border-violet-100 hover:border-violet-300 hover:shadow-md transition-all"
              >
                <CardContent className="pt-5">
                  <div className="text-3xl mb-3">{device.icon}</div>
                  <div className="font-semibold text-sm text-violet-700">
                    {device.type}
                  </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <p className="text-muted-foreground mb-8 leading-relaxed">
                Legacy WMS systems treat robots as dumb conveyor belts — they
                queue tasks with no awareness of who should execute them.
                LogiVox analyses battery level, device type, current floor
                location, and task urgency to match every queued task to the
                optimal available device automatically.
              </p>
            ...
            <div className="bg-gradient-to-br from-teal-50 to-violet-50 dark:from-teal-950/20 dark:to-violet-950/20 rounded-2xl p-8 border border-teal-100">
              <div className="space-y-3">
                <div className="font-bold text-lg mb-4">
                  Fleet Command Centre — Live View
                </div>
                {[
                  {
                    name: "AMR-04",
                  ...
                  <div
                    key={device.name}
                    className={`rounded-lg border p-3 ${device.status === "ACTIVE" ? "bg-green-50 border-green-200" : device.status === "CHARGING" ? "bg-yellow-50 border-yellow-200" : "bg-white border-gray-200"}`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span
                        className={`text-xs font-semibold px-2 py-0.5 rounded-full ${device.status === "ACTIVE" ? "bg-green-200 text-green-800" : device.status === "CHARGING" ? "bg-yellow-200 text-yellow-800" : "bg-gray-200 text-gray-700"}`}
                      >
                        {device.status}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <div className="flex-1 h-1.5 bg-gray-200 rounded-full">
                        <div
                          className={`h-1.5 rounded-full ${device.battery < 30 ? "bg-red-400" : device.battery < 60 ? "bg-yellow-400" : "bg-green-400"}`}
        ...
        <div className="container-enterprise">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="bg-gradient-to-br from-teal-50 to-teal-100/50 dark:from-teal-950/20 dark:to-teal-950/10 rounded-2xl p-8 border border-teal-100 order-2 lg:order-1">
              <div className="font-bold text-lg mb-4">
                Sortation Throughput — Last 8 Hours
              </div>
                ...
                {[
                  ...
                  <div key={row.hour} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-5 bg-gray-100 rounded relative">
                      <div
                        className="h-5 rounded bg-teal-500 flex items-center justify-end pr-1.5"
                        style={{ width: `${row.pct}%` }}
                      >
              ...
              <div className="grid grid-cols-3 gap-2 mt-4">
                {[
                  ...
                  <div
                    key={row.device}
                    className={`bg-${row.color}-50 border border-${row.color}-200 rounded p-2 text-center`}
                  >
                    <div className="text-xs font-medium truncate">
                      {row.device}
                    </div>
                    <div className={`text-sm font-bold text-${row.color}-700`}>
                      {row.rate}
                    </div>
                    <div className={`text-[10px] text-${row.color}-600`}>
                      {row.status}
                    </div>
                  </div>
                ))}
            ...
            <div className="order-1 lg:order-2">
              <Badge className="mb-4 bg-violet-100 text-violet-800 border-violet-300">
              ...
              <p className="text-muted-foreground mb-8 leading-relaxed">
                Cross-belt sorters, conveyors, and AS/RS cranes are the arteries
                of a high-volume warehouse. When they underperform, every
                downstream process suffers. LogiVox gives supervisors instant
                visibility into throughput, per-device success rates, and cycle
      ...
      <section className="py-20">
        <div className="container-enterprise">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold mb-4">
              Everything You Need to Run a Lights-Out Warehouse
            </h2>
          </div>
              ...
              {[
                {
                  ...
                  detail:
                    "Removing human bottlenecks from robot task assignment delivers immediate throughput gains — without adding headcount.",
                  icon: TrendingUp,
                  color: "violet",
                },
                {
                  ...
                  detail:
                    "Per-device success rate monitoring catches underperforming sorters before they generate exceptions, chargebacks, or returns.",
                  icon: Zap,
                  color: "teal",
                },
                {
                  ...
                  detail:
                    "REST API and webhook integrations mean most robot vendors are live in a single sprint — no 18-month implementation project.",
                  icon: Clock,
                  color: "green",
                },
              ].map((item) => (
                <Card
                  key={item.headline}
                  className={`border-${item.color}-200 bg-${item.color}-50/50 dark:bg-${item.color}-950/10`}
                >
                  <CardHeader>
                    <item.icon
                      className={`h-8 w-8 text-${item.color}-600 mb-2`}
                    />
                    <CardTitle className={`text-2xl text-${item.color}-700`}>
                      {item.headline}
                    </CardTitle>
      ...
      <section className="py-20 bg-gradient-to-r from-violet-600 to-teal-600 text-white">
        <div className="container-enterprise text-center space-y-6">
          ...
          <p className="text-violet-100 max-w-2xl mx-auto text-lg">
            Book a live demo and see AI fleet dispatch, sortation monitoring,
            and multi-device control in action — with your device types and your
            operational scenarios.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button
              size="lg"
              className="bg-white text-violet-700 hover:bg-violet-50 px-8 font-semibold"
              asChild
            >
              <Link href="/contact">
            ...
            <Button
              ...
              asChild
            >
              <Link href="/comparison">Compare vs SAP / Manhattan</Link>
            </Button>
          </div>
        </div>
      </section>
