...
export async function PUT(
  ...
  try {
    ...
    if (effectsOfFailure !== undefined)
      updateData.effectsOfFailure = effectsOfFailure;
    if (potentialCauses !== undefined)
      updateData.potentialCauses = potentialCauses;
    if (currentControls !== undefined)
      updateData.currentControls = currentControls;
    ...
    if (linkedCAPAIds !== undefined)
      updateData.linkedCAPAIds = JSON.stringify(linkedCAPAIds);
    ...
    if (
      ...
      if (current) {
        ...
        const newOccurrence =
          occurrence !== undefined ? occurrence : current.occurrence;
        const newDetection =
          detection !== undefined ? detection : current.detection;
    ...
    if (
      ...
      residualOccurrence !== undefined ||
      residualDetection !== undefined
    ) {
      const current = await prisma.fMEAFailureMode.findUnique({
        where: { id: params.fmId },
      });
      ...
      if (current) {
        const newResSeverity =
          residualSeverity !== undefined
            ? residualSeverity
            : current.residualSeverity || current.severity;
        const newResOccurrence =
          residualOccurrence !== undefined
            ? residualOccurrence
            : current.residualOccurrence || current.occurrence;
        const newResDetection =
          residualDetection !== undefined
            ? residualDetection
            : current.residualDetection || current.detection;
        ...
        updateData.residualRPN =
          newResSeverity * newResOccurrence * newResDetection;
      }
    }
