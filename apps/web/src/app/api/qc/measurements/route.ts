...
export async function GET(request: Request) {
  try {
    ...
    const measurements = await prisma.qualityMeasurement.findMany({ where, orderBy: { measuredAt: "desc" }, take: 100 });
...
export async function POST(request: Request) {
  try {
    ...
    const measurement = await prisma.qualityMeasurement.create({ data: { ...body, organizationId: orgId } });
