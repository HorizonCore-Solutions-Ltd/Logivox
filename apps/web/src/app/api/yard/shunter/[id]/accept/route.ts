...
// Accept a shunter task — marks the related DockAppointment as IN_PROGRESS
...
export async function POST(
  ...
  try {
    ...
    const updated = await prisma.dockAppointment.update({
      ...
      data: {
        status: "IN_PROGRESS",
        actualStart: appointment.actualStart ?? new Date(),
      },
    });
    ...
    console.error("Error accepting shunter task:", error);
    return NextResponse.json(
