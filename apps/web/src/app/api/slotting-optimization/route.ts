  ...
  try {
    ...
    if (action === "recommendations") {
      ...
      const recommendations =
        await generateSlottingRecommendations(warehouseId);
  ...
  try {
    ...
    for (const item of items) {
      ...
      const currentScore = currentLocation
        ...
        : 0;
      const optimalScore = calculateLocationScore(
        optimalLocation,
        item,
        velocity,
      );
      ...
      if (improvement > 5) {
        ...
        recommendations.push({
          ...
          priority:
            improvement > 30 ? "HIGH" : improvement > 15 ? "MEDIUM" : "LOW",
        });
      }
    }
 ...
 */
async function calculateItemVelocity(
  warehouseId: string,
...
function calculateLocationScore(
  ...
  const sizeMatch = item.dimensions?.volume <= location.maxVolume ? 15 : 0;
...
async function analyzeSlottingEfficiency(warehouseId: string) {
  try {
    ...
    let totalScore = 0;
    let itemCount = 0;
    const zoneDistribution: Record<
      string,
      { count: number; avgVelocity: number }
    > = {};
    ...
    Object.keys(zoneDistribution).forEach((zone) => {
      zoneDistribution[zone].avgVelocity = Math.round(
        zoneDistribution[zone].avgVelocity / zoneDistribution[zone].count,
      );
    });
