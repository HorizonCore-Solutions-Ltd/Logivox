...
const effectivenessSchema = z.object({
  ...
  performanceRating: z.enum(['EXCELLENT', 'SATISFACTORY', 'NEEDS_IMPROVEMENT', 'UNSATISFACTORY']),
  ...
  try {
    ...
    if (requiresVerification === 'true') {
      filteredEnrollments = enrollments.filter(e => 
        e.status === 'COMPLETED' && !e.effectivenessCheck
      )
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'CREATE_REQUIREMENT') {
      ...
      } else if (data.targetAudience === 'DEPARTMENT' && data.targetDepartment) {
    ...
    if (action === 'RECORD_COMPLETION') {
      ...
      if (data.score !== undefined && enrollment.requirement.competencyRequired) {
        passingStatus = data.score >= enrollment.requirement.minimumScore ? 'PASSED' : 'FAILED'
      }
      ...
      const newStatus = passingStatus === 'PASSED' ? 'COMPLETED' : 
                       passingStatus === 'FAILED' ? 'ENROLLED' : 'ENROLLED'

      await prisma.capaTrainingEnrollment.update({
        where: { id: data.enrollmentId },
        data: { 
          status: newStatus,
          completedAt: passingStatus === 'PASSED' ? new Date(data.completionDate) : null,
        }
      })
    ...
    if (action === 'VERIFY_EFFECTIVENESS') {
      ...
      const effectivenessCheck = await prisma.capaTrainingEffectivenessCheck.create({
        data: {
          enrollmentId: data.enrollmentId,
      ...
      if (allVerified) {
        ...
        const allRequirementsComplete = allRequirements.every(r => r.status === 'COMPLETED')
      ...
      return NextResponse.json({
        success: true,
        effectivenessCheck,
        message: 'Effectiveness verified. Waiting for other enrollees to complete.'
      })
    }
    ...
    if (action === 'CHECK_CAPA_CLOSURE_ELIGIBILITY') {
      ...
      const requirementStatus = requirements.map(req => {
        const allEnrollmentsVerified = req.enrollments.every(e => e.status === 'VERIFIED')
        return {
          requirementId: req.id,
          topic: req.trainingTopic,
          status: req.status,
          totalEnrollments: req.enrollments.length,
          verified: req.enrollments.filter(e => e.status === 'VERIFIED').length,
