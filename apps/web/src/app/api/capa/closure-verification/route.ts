...
const CLOSURE_CRITERIA: ClosureCheck[] = [
  // Investigation Checks
  {
    ...
    critical: true,
    description: 'Root cause analysis must be completed using 5-Whys, Fishbone, or similar method'
  },
  {
  ...
  },
  {
    ...
    critical: true,
    description: 'All corrective and preventive actions must be marked as implemented'
  },
  {
    ...
    critical: false,
    description: 'Photos, documents, or records proving actions were implemented'
  },
  ...
  // Effectiveness Checks
  {
    ...
    critical: true,
    description: 'Effectiveness check must be completed and show positive results'
  },
  {
    ...
    critical: true,
    description: 'At least 30 days since action implementation (or justified shorter period)'
  },
  {
    id: 'NO_RECURRENCE',
    category: 'Effectiveness',
  ...
  // Training Checks
  {
    ...
    critical: true,
    description: 'All users enrolled in CAPA training must have completed and passed'
  },
  {
  ...
  // Customer Impact Checks
  {
    ...
    critical: true,
    description: 'If customers were affected, all notifications sent and responses received'
  },
]
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      needsReview: verifications.filter(v => v.overallStatus === 'NEEDS_REVIEW').length,
      averageScore: verifications.length > 0
        ? Math.round(verifications.reduce((sum, v) => sum + (v.completionScore || 0), 0) / verifications.length)
        : 0,
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'VERIFY_CLOSURE_READINESS') {
      ...
      const completionScore = Math.round((totalPassed / CLOSURE_CRITERIA.length) * 100)
      ...
      if (criticalFailures > 0) {
        ...
        statusReason = 'Some optional criteria not met. Manager review recommended.'
      }
    ...
    if (action === 'OVERRIDE_CHECK') {
      ...
      // Recalculate scores
      const totalPassed = checkResults.filter(c => c.passed || c.overridden).length
      const completionScore = Math.round((totalPassed / checkResults.length) * 100)
      const criticalFailures = checkResults.filter(c => c.critical && !c.passed && !c.overridden).length
...
async function performClosureCheck(capa: any, criterion: ClosureCheck) {
  ...
  switch (criterion.id) {
    case 'ROOT_CAUSE_IDENTIFIED':
      passed = !!(capa.rootCauseAnalysis && capa.rootCauseAnalysis.trim().length > 50)
      evidence = passed ? 'Root cause analysis documented' : 'No root cause analysis found'
      recommendation = !passed ? 'Complete root cause analysis using 5-Whys, Fishbone, or similar method' : ''
      break

    case 'ROOT_CAUSE_VERIFIED':
      passed = !!(capa.rootCauseVerified)
      evidence = passed ? 'Root cause verified by supervisor' : 'Root cause not verified'
      recommendation = !passed ? 'Have quality manager verify root cause analysis' : ''
      break

    case 'CORRECTIVE_ACTIONS_DEFINED':
      const hasCorrectiveActions = capa.correctiveActions && 
        Array.isArray(JSON.parse(capa.correctiveActions)) &&
        JSON.parse(capa.correctiveActions).length > 0
      passed = hasCorrectiveActions
      evidence = passed ? `${JSON.parse(capa.correctiveActions || '[]').length} corrective actions defined` : 'No corrective actions defined'
      recommendation = !passed ? 'Define at least one corrective action to address root cause' : ''
      break

    case 'ACTIONS_IMPLEMENTED':
      const actions = JSON.parse(capa.correctiveActions || '[]')
      const allImplemented = actions.length > 0 && actions.every((a: any) => a.status === 'COMPLETED')
      passed = allImplemented
      evidence = passed ? 'All actions marked as implemented' : `${actions.filter((a: any) => a.status !== 'COMPLETED').length} actions pending`
      recommendation = !passed ? 'Complete implementation of all corrective and preventive actions' : ''
      break

    case 'IMPLEMENTATION_EVIDENCE':
      passed = !!(capa.documents && Array.isArray(JSON.parse(capa.documents)) && JSON.parse(capa.documents).length >= 2)
      evidence = passed ? `${JSON.parse(capa.documents || '[]').length} documents attached` : 'Insufficient documentation'
      recommendation = !passed ? 'Attach photos, reports, or forms showing action implementation' : ''
      break

    case 'EFFECTIVENESS_VERIFIED':
      passed = !!(capa.effectivenessVerified && capa.effectivenessScore && capa.effectivenessScore >= 80)
      evidence = passed ? `Effectiveness verified at ${capa.effectivenessScore}%` : 'Effectiveness not verified'
      recommendation = !passed ? 'Complete effectiveness check after sufficient observation period' : ''
      break

    case 'SUFFICIENT_OBSERVATION_PERIOD':
      if (capa.effectivenessCheckDate) {
        const daysSinceImplementation = Math.floor(
          (new Date(capa.effectivenessCheckDate).getTime() - new Date(capa.targetCompletionDate).getTime()) / 
          (1000 * 60 * 60 * 24)
        )
        passed = daysSinceImplementation >= 30
        evidence = `${daysSinceImplementation} days observation period`
        recommendation = !passed ? 'Wait for 30-day observation period before closing' : ''
      } else {
        passed = false
    ...
    case 'NO_RECURRENCE':
      ...
      if (capa.ncr) {
        ...
        passed = recentNCRs === 0
        evidence = passed ? 'No recurrence detected' : `${recentNCRs} similar NCRs in last 30 days`
        recommendation = !passed ? 'Investigate why defect is recurring. Root cause may be incorrect.' : ''
      } else {
        passed = true
    ...
    case 'TRAINING_COMPLETED':
      ...
      if (trainingReqs.length === 0) {
        ...
        passed = allCompleted
        const totalEnrollments = trainingReqs.reduce((sum: number, req: any) => sum + req.enrollments.length, 0)
        const completedEnrollments = trainingReqs.reduce((sum: number, req: any) => 
          sum + req.enrollments.filter((e: any) => e.status === 'VERIFIED').length, 0
        )
        evidence = `${completedEnrollments}/${totalEnrollments} training enrollments verified`
        recommendation = !passed ? 'Ensure all required training is completed and effectiveness verified' : ''
      }
      break

    case 'TRAINING_EFFECTIVENESS':
      const hasEffectiveness = capa.capaTrainingRequirements?.some((req: any) =>
        req.enrollments.some((enr: any) => enr.effectivenessCheck?.performanceRating === 'EXCELLENT')
      )
      passed = hasEffectiveness || capa.capaTrainingRequirements?.length === 0
      evidence = passed ? 'Training effectiveness verified' : 'No effectiveness checks completed'
      recommendation = !passed ? 'Verify training effectiveness through on-job observation' : ''
      break
    ...
    case 'DOCUMENTS_ATTACHED':
      ...
      recommendation = !passed ? 'Attach reports, photos, updated procedures, or test results' : ''
      break

    case 'PROCEDURES_UPDATED':
      passed = !!(capa.updatedProcedures && JSON.parse(capa.updatedProcedures).length > 0)
      evidence = passed ? 'Procedures updated and documented' : 'No procedure updates documented'
      recommendation = !passed ? 'Update SOPs/WIs if process changes were made' : ''
      break

    case 'MANAGEMENT_APPROVAL':
      passed = !!(capa.closureApprovedBy)
      evidence = passed ? 'Closure approved by management' : 'No management approval'
      recommendation = !passed ? 'Request closure approval from quality manager' : ''
      break
    ...
    case 'CUSTOMER_IMPACT_RESOLVED':
      ...
      if (impacts.length === 0) {
        ...
        const allNotified = impacts.every((imp: any) => imp.notificationStatus === 'SENT')
        passed = allNotified
        evidence = passed ? `All ${impacts.length} customers notified` : 'Customer notifications pending'
        recommendation = !passed ? 'Send notifications to all affected customers' : ''
      }
      break
