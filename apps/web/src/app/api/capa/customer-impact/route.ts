...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      notificationsPending: impacts.filter(
        (i) => i.notificationStatus === "PENDING",
      ).length,
      notificationsSent: impacts.filter((i) => i.notificationStatus === "SENT")
        .length,
      responsesReceived: impacts.filter((i) => i.customerResponse !== null)
        .length,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "ANALYZE_IMPACT") {
      ...
      const impacts = await Promise.all(
        affectedShipments.map(async (shipment) => {
          ...
          const totalQuantity = affectedItems.reduce(
            (sum, item) => sum + parseFloat(item.quantity.toString()),
            0,
          );

          const totalValue = affectedItems.reduce(
            (sum, item) =>
              sum +
          ...
          return prisma.customerImpactAnalysis.create({
            data: {
              ...
              affectedSerialNumbers: data.serialNumber
                ? [data.serialNumber]
                : [],
      ...
      return NextResponse.json({
        ...
        summary: {
          ...
          totalValue: impacts.reduce(
            (sum, i) => sum + parseFloat(i.estimatedValue.toString()),
            0,
          ),
        },
      });
    ...
    if (action === "RECORD_CUSTOMER_RESPONSE") {
      const { impactId, responseText, responseReceived, actionRequested } =
        body;
      ...
      const impact = await prisma.customerImpactAnalysis.update({
        ...
        data: {
          ...
          customerResponseAt: responseReceived
            ? new Date(responseReceived)
            : new Date(),
          actionRequested,
        },
      });
