  ...
  try {
    ...
    const capas = await prisma.correctivePreventiveAction.findMany({
      ...
      include: !lightweight
        ...
        : undefined,
      orderBy: [{ priority: "desc" }, { targetCompletionDate: "asc" }],
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "ADD_PHOTO") {
      ...
      if (!capaId || !photoUrl) {
        return NextResponse.json(
          { error: "Missing capaId or photoUrl" },
          { status: 400 },
        );
      }
    ...
    if (action === "ADD_VOICE_NOTE") {
      ...
      if (!capaId || !voiceNoteUrl) {
        return NextResponse.json(
          { error: "Missing capaId or voiceNoteUrl" },
          { status: 400 },
        );
      }
