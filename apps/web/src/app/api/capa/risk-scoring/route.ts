...
const SEVERITY_GUIDE = {
  10: { level: 'Catastrophic', description: 'Death or serious injury, FDA recall, litigation' },
  9: { level: 'Critical', description: 'Serious injury, product recall, major regulatory action' },
  8: { level: 'Very High', description: 'Injury, customer complaint, regulatory warning' },
  7: { level: 'High', description: 'Major impact on product function, customer dissatisfaction' },
  6: { level: 'Moderate-High', description: 'Significant impact on product performance' },
  5: { level: 'Moderate', description: 'Noticeable impact on performance, minor customer complaint' },
  4: { level: 'Low-Moderate', description: 'Minor impact on performance' },
  3: { level: 'Low', description: 'Very minor impact, customer may not notice' },
  2: { level: 'Very Low', description: 'Negligible impact' },
  1: { level: 'None', description: 'No impact' },
}

const OCCURRENCE_GUIDE = {
  10: { level: 'Very High', description: 'Almost certain (≥1 in 2)', frequency: '≥50%' },
  9: { level: 'High', description: 'Very frequent (1 in 3)', frequency: '33%' },
  8: { level: 'High', description: 'Frequent (1 in 8)', frequency: '12.5%' },
  7: { level: 'Moderate-High', description: 'Moderately high (1 in 20)', frequency: '5%' },
  6: { level: 'Moderate', description: 'Moderate (1 in 80)', frequency: '1.25%' },
  5: { level: 'Moderate', description: 'Moderate (1 in 400)', frequency: '0.25%' },
  4: { level: 'Low-Moderate', description: 'Low (1 in 2,000)', frequency: '0.05%' },
  3: { level: 'Low', description: 'Low (1 in 15,000)', frequency: '0.007%' },
  2: { level: 'Very Low', description: 'Very rare (1 in 150,000)', frequency: '0.0007%' },
  1: { level: 'Remote', description: 'Nearly impossible (≤1 in 1,500,000)', frequency: '≤0.00007%' },
}

const DETECTION_GUIDE = {
  10: { level: 'Absolute Uncertainty', description: 'No detection method exists' },
  ...
  4: { level: 'Moderately High', description: 'Moderately high chance of detection' },
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      averageRPN: assessments.length > 0
        ? Math.round(assessments.reduce((sum, a) => sum + a.initialRPN, 0) / assessments.length)
        : 0,
      riskReduced: assessments.filter(a => a.postActionRPN && a.postActionRPN < a.initialRPN).length,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'CALCULATE_RPN') {
      ...
      } else if (riskLevel === 'HIGH' && (capa.priority === 'MEDIUM' || capa.priority === 'LOW')) {
      ...
      return NextResponse.json({
        ...
        recommendations: generateRPNRecommendations(data.severity, data.occurrence, data.detection, rpn),
      })
    }
    ...
    if (action === 'UPDATE_POST_ACTION_RPN') {
      ...
      const reductionPercent = Math.round((riskReduction / assessment.initialRPN) * 100)
      ...
      return NextResponse.json({
        ...
        riskReduction,
        reductionPercent,
        effectiveness: reductionPercent >= 50 ? 'HIGHLY_EFFECTIVE' :
                      reductionPercent >= 25 ? 'EFFECTIVE' :
                      reductionPercent > 0 ? 'SOMEWHAT_EFFECTIVE' :
                      'NOT_EFFECTIVE',
    ...
    if (action === 'AUTO_PRIORITIZE_CAPAS') {
      ...
      const assessments = await Promise.all(
        capasWithoutRPN.map(async (capa) => {
          ...
          let detection = 5
          if (capa.ncr?.detectionStage === 'RECEIVING') detection = 3 // Caught early
          else if (capa.ncr?.detectionStage === 'IN_PROCESS') detection = 5
          else if (capa.ncr?.detectionStage === 'FINAL_INSPECTION') detection = 7
          ...
          return prisma.riskPriorityAssessment.create({
            data: {
              ...
              initialRPN: rpn,
              riskLevel,
              notes: 'Auto-calculated based on CAPA severity and detection stage',
              assessedBy: session.user.id,
              assessedAt: new Date(),
            }
          })
...
function calculateAverageReduction(assessments: any[]) {
  ...
  const total = withReduction.reduce((sum, a) => sum + (a.reductionPercent || 0), 0)
  return Math.round(total / withReduction.length)
}

function generateRPNRecommendations(severity: number, occurrence: number, detection: number, rpn: number) {
  ...
  if (occurrence >= 7) {
    recommendations.push({
      factor: 'OCCURRENCE',
      priority: 'HIGH',
      action: 'Implement preventive controls, improve process capability, add error-proofing (poka-yoke)',
  ...
  if (detection >= 7) {
    recommendations.push({
      factor: 'DETECTION',
      priority: 'HIGH',
      action: 'Improve inspection methods, add automated checks, implement 100% inspection if needed',
  ...
  if (rpn >= 200) {
    recommendations.push({
      factor: 'RPN',
      priority: 'CRITICAL',
      action: 'STOP production if possible. Implement immediate containment. Assign dedicated team to resolve.',
    ...
    recommendations.push({
      factor: 'RPN',
      priority: 'HIGH',
      action: 'Prioritize resources, set aggressive timeline, daily status reviews',
