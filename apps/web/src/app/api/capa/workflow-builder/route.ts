...
const WORKFLOW_TEMPLATES = {
  STANDARD_CAPA: {
    ...
    description: 'Basic CAPA process: Create → Investigate → Implement → Verify → Close',
    icon: '📋',
    category: 'Basic',
    nodes: [
      {
  ...
  HIGH_PRIORITY_ESCALATION: {
    ...
    edges: [
      ...
      { id: 'e2', source: 'check_priority', target: 'escalate', condition: 'TRUE' },
      { id: 'e3', source: 'check_priority', target: 'standard_flow', condition: 'FALSE' },
  ...
  SUPPLIER_CAPA: {
    ...
    edges: [
      ...
      { id: 'e5', source: 'check_response', target: 'escalate_supplier', condition: 'FALSE' },
  ...
  TRAINING_INTEGRATION: {
    ...
    edges: [
      ...
      { id: 'e2', source: 'check_training_needed', target: 'create_training', condition: 'TRUE' },
      { id: 'e3', source: 'check_training_needed', target: 'end', condition: 'FALSE' },
  ...
  MULTI_APPROVAL_CHAIN: {
    ...
    edges: [
      ...
      { id: 'e3', source: 'check_cost', target: 'director_approval', condition: 'TRUE' },
      { id: 'e4', source: 'check_cost', target: 'approved', condition: 'FALSE' },
...
// @ts-ignore - Model exists but TS needs restart
    ...
    if (action === 'ADVANCE_WORKFLOW') {
      ...
      if// @ts-ignore - Model exists but TS needs restart
         (!nextEdge) {
      ...
      // Move to next node
      const nextNode = (instance.workflow.nodes as any[]).find(n => n.id === nextEdge.target)
...
async function executeWorkflowNode(instanceId: string, node: any, capaId: string) {
  try {
