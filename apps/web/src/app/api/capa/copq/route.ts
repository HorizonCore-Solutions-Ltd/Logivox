...
const copqEntrySchema = z.object({
  ...
  relatedEntity: z.enum(["CAPA", "NCR", "INSPECTION", "RTV", "TRAINING", "AUDIT", "OTHER"]).optional(),
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (breakdown.total > 0) {
      breakdown.prevention.percentage = (breakdown.prevention.cost / breakdown.total) * 100;
      breakdown.appraisal.percentage = (breakdown.appraisal.cost / breakdown.total) * 100;
      breakdown.internalFailure.percentage = (breakdown.internalFailure.cost / breakdown.total) * 100;
      breakdown.externalFailure.percentage = (breakdown.externalFailure.cost / breakdown.total) * 100;
    }

    // Calculate ROI if requested
    let roiAnalysis = null;
    if (calculateROI) {
      roiAnalysis = await calculateQualityROI(organizationId, startDate, endDate);
    }
    ...
    if (startDate && endDate) {
      ...
      const previousTotal = previousEntries.reduce((sum, entry) => sum + Number(entry.cost), 0);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === "CREATE_ENTRY") {
      ...
      costs.totalCost = costs.prevention + costs.appraisal + costs.internalFailure + costs.externalFailure;
      ...
      // Calculate ROI
      costs.roi = costs.estimatedSavings > 0 
...
async function calculateQualityROI(
  ...
  const failureCost = copqEntries
    .filter(e => e.category === "INTERNAL_FAILURE" || e.category === "EXTERNAL_FAILURE")
    .reduce((sum, e) => sum + Number(e.cost), 0);

  // Estimate savings (based on CAPA effectiveness)
  const completedCapas = capas.filter(c => c.status === "CLOSED" || c.status === "VERIFIED");
  const avgEffectiveness = completedCapas.length > 0
    ? completedCapas.reduce((sum, c) => sum + (c.effectivenessScore || 70), 0) / completedCapas.length
    : 70;

  // Assume each CAPA prevents 8 future incidents (conservative)
  const avgIncidentCost = failureCost / Math.max(capas.length, 1);
  const estimatedSavings = capas.length * 8 * avgIncidentCost * (avgEffectiveness / 100);

  const roi = preventionCost > 0 
