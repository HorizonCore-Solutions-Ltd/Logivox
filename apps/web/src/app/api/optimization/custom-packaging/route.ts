...
const FRAGILITY_LEVELS = {
  VERY_FRAGILE: { name: 'Very Fragile', minProtection: 'HIGH', paddingCost: 2.50 },
  ...
  VERY_DURABLE: { name: 'Very Durable', minProtection: 'LOW', paddingCost: 0.10 }
} as const;
...
const ExecuteActionSchema = z.object({
  ...
  items: z.array(z.object({
    ...
    width: z.number(),
    height: z.number(),
    fragility: z.enum(['VERY_FRAGILE', 'FRAGILE', 'MODERATE', 'DURABLE', 'VERY_DURABLE'])
  })).optional(),
...
function determineOptimalPackage(
  ...
  const fragilityOrder = ['VERY_DURABLE', 'DURABLE', 'MODERATE', 'FRAGILE', 'VERY_FRAGILE'];
  const maxFragilityIndex = Math.max(
    ...items.map(item => fragilityOrder.indexOf(item.fragility))
  );
  const maxFragility = fragilityOrder[maxFragilityIndex] as keyof typeof FRAGILITY_LEVELS;
  ...
  for (const [packageKey, packageSpec] of Object.entries(PACKAGE_TYPES)) {
    ...
    if (minProtection === 'HIGH' && packageSpec.protectionLevel !== 'HIGH') continue;
    if (minProtection === 'MEDIUM' && packageSpec.protectionLevel === 'LOW') continue;

    // Calculate package volume
    const packageVolume = packageSpec.maxLength * packageSpec.maxWidth * packageSpec.maxHeight;
  ...
  if (!bestPackage) {
    bestPackage = 'CUSTOM';
    const customVolume = PACKAGE_TYPES.CUSTOM.maxLength * 
...
function calculateShippingCostSavings(
  ...
  const originalCost = (4.50 + (originalBillableWeight * 0.65)) * zoneMultiplier;
  const optimizedCost = (4.50 + (optimizedBillableWeight * 0.65)) * zoneMultiplier;
...
function analyzeConsolidation(
  ...
  if (consolidated.packageType === 'CUSTOM' || consolidated.packageType === 'PALLET') {
    return {
      canConsolidate: false,
      estimatedSavings: 0,
      recommendedPackage: null,
  ...
  if (savings > 1.00) { // At least $1 savings to justify consolidation
    return {
      canConsolidate: true,
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'stats') {
      ...
      const [
        ...
        prisma.activityLog.findMany({
          where: {
            organizationId,
            action: {
              in: ['PACKAGING_OPTIMIZED', 'ORDERS_CONSOLIDATED', 'PACKAGE_RECOMMENDED']
            },
            createdAt: {
      ...
      const averageEfficiency = packagingLogs.length > 0
      ...
      return NextResponse.json({
        ...
        stats: {
          ...
          totalSavings: Math.round((totalMaterialSavings + totalShippingSavings) * 100) / 100,
  ...
  try {
    ...
    switch (validatedData.action) {
      ...
      case 'optimize_package': {
        if (!validatedData.orderId || !validatedData.items || !validatedData.currentPackageType) {
          return NextResponse.json(
        ...
        const currentType = validatedData.currentPackageType as keyof typeof PACKAGE_TYPES;
        const optimal = determineOptimalPackage(validatedData.items);
        const totalWeight = validatedData.items.reduce((sum, item) => sum + item.weight, 0);
      ...
      case 'calculate_savings': {
        ...
        return NextResponse.json({
          ...
          savings: {
            ...
            shippingSavings: Math.round(totalShippingSavings * 100) / 100,
            totalSavings: Math.round((totalMaterialSavings + totalShippingSavings) * 100) / 100,
            projectedAnnual: Math.round((totalMaterialSavings + totalShippingSavings) * 12 * 100) / 100
          }
        });
      }
