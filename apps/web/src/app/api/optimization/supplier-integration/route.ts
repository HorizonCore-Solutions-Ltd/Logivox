...
export async function GET(req: NextRequest) {
  try {
    ...
    const orgId = session.user.orgId || (await prisma.user.findUnique({ where: { id: session.user.id } })).organizationMemberships[0]?.organizationId; // Fallback
    ...
    if (action === "stats") {
      ...
      const activeCtx = await prisma.purchaseOrder.count({
        where: { organizationId: orgId, status: { notIn: ["CANCELLED", "CLOSED", "RECEIVED"] } }
      });
      // Pending
      ...
      const totalOrdersCtx = await prisma.purchaseOrder.count({ where: { organizationId: orgId } });
      ...
      return NextResponse.json({
        stats: {
          ...
          avgOrderValue: 3200, 
          activeSuppliers: await prisma.supplier.count({ where: { organizationId: orgId, status: "ACTIVE" } })
        }
      });
        ...
        for (const item of lowStockItems) {
            if (item.availableQty <= item.minStockLevel) {
                ...
                let qtyToOrder = (item.reorderQuantity && item.reorderQuantity > 0) 
                    ? item.reorderQuantity 
                    : (item.maxStockLevel ? item.maxStockLevel - item.availableQty : item.minStockLevel * 2);
