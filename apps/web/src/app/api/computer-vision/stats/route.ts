...
export async function GET(request: NextRequest) {
  try {
    ...
    const varianceCount = scans.filter(
      (scan) =>
        scan.variance !== null &&
        scan.variance !== undefined &&
        scan.variance !== 0,
    ).length;

    const damageCount = scans.filter(
      (scan) => scan.damageDetected === true,
    ).length;

    const totalItems = scans.reduce(
      (sum, scan) => sum + (scan.detectedItems || 0),
      0,
    );

    // Group by mode
    const scansByMode = scans.reduce(
      (acc, scan) => {
    ...
    const scansByDate = scans.reduce(
      (acc, scan) => {
    ...
    const highConfidenceScans = scans.filter(
      (scan) => scan.confidence >= 0.85,
    ).length;
    const accuracyRate =
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        performance: {
          ...
          mediumConfidence: scans.filter(
            (s) => s.confidence >= 0.7 && s.confidence < 0.85,
          ).length,
          lowConfidence: scans.filter((s) => s.confidence < 0.7).length,
        },
        trends: {
