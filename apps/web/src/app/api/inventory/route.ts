...
const createInventorySchema = z.object({
  ...
  sku: z.string()
    ...
    .regex(/^[A-Z0-9\-_]+$/, "SKU must be alphanumeric with hyphens/underscores only")
  ...
  warehouseId: z.string()
    .cuid("Invalid warehouse ID format"),
  categoryId: z.string()
...
function enforceRateLimit(request: NextRequest, operation: 'GET' | 'POST', userRole?: string): boolean {
  const ip = request.headers.get('x-forwarded-for')?.split(',')[0] || 'unknown';
  const key = `${ip}-${operation}`;
  const now = Date.now();
  const limit = userRole === 'ADMIN' ? RATE_LIMITS.ADMIN : RATE_LIMITS[operation];
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!enforceRateLimit(request, 'GET')) {
      ...
      await createSecurityAuditLog('INVENTORY_LIST_BLOCKED', 'anonymous', request, null, 'BLOCKED', blockReason);
    ...
    if (!user || !user.id) {
      securityViolation = true;
      blockReason = 'Authentication required - no anonymous access permitted';
      await createSecurityAuditLog('INVENTORY_LIST_DENIED', 'anonymous', request, null, 'BLOCKED', blockReason);
    ...
    if (!userStatus || !userStatus.isActive) {
      ...
      await createSecurityAuditLog('INVENTORY_ACCESS_DENIED', user.id, request, null, 'BLOCKED', blockReason);
    ...
    if (rawParams.organizationId && !/^[a-zA-Z0-9_-]{25}$/.test(rawParams.organizationId)) {
      securityViolation = true;
      blockReason = 'Invalid organizationId format - potential injection attempt';
    }
    
    if (rawParams.search && (rawParams.search.length > 50 || /[<>"'%;\(\)\&\+]/.test(rawParams.search))) {
      securityViolation = true;
      blockReason = 'Invalid search parameters - potential XSS/injection attempt';
    }

    if (rawParams.status && !['ACTIVE', 'INACTIVE', 'DISCONTINUED'].includes(rawParams.status)) {
      securityViolation = true;
      blockReason = 'Invalid status parameter - potential enumeration attempt';
    }

    if (rawParams.limit && (parseInt(rawParams.limit) > 100 || parseInt(rawParams.limit) < 1)) {
      securityViolation = true;
      blockReason = 'Invalid limit parameter - potential resource exhaustion attempt';
    }

    if (securityViolation) {
      await createSecurityAuditLog('INVENTORY_PARAM_VIOLATION', user.id, request, rawParams, 'BLOCKED', blockReason);
    ...
    if (!rawParams.organizationId) {
      securityViolation = true;
      blockReason = 'Organization ID is mandatory - no global access permitted';
      await createSecurityAuditLog('INVENTORY_NO_ORG', user.id, request, null, 'BLOCKED', blockReason);
    ...
    const orgAccess = await validateOrganizationAccess(user.id, rawParams.organizationId, ['MEMBER', 'MANAGER', 'ADMIN']);
    if (!orgAccess.valid) {
      securityViolation = true;
      blockReason = `Organization access denied: ${orgAccess.reason}`;
      await createSecurityAuditLog('INVENTORY_ORG_ACCESS_DENIED', user.id, request, { orgId: rawParams.organizationId }, 'BLOCKED', blockReason);
    ...
    await createSecurityAuditLog(
      ...
      { 
        ...
        filters: Object.keys(rawParams).filter(k => rawParams[k as keyof typeof rawParams])
      }, 
      'SUCCESS'
    );
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!enforceRateLimit(request, 'POST')) {
      blockReason = 'Create rate limit exceeded - potential abuse or automation detected';
      await createSecurityAuditLog('INVENTORY_CREATE_RATE_LIMITED', 'anonymous', request, null, 'BLOCKED', blockReason);
    ...
    if (!user || !user.id) {
      ...
      await createSecurityAuditLog('INVENTORY_CREATE_AUTH_REQUIRED', 'anonymous', request, null, 'BLOCKED', blockReason);
    ...
    if (!userProfile || !userProfile.isActive) {
      ...
      await createSecurityAuditLog('INVENTORY_CREATE_ACCOUNT_BLOCKED', user.id, request, null, 'BLOCKED', blockReason);
    ...
    if (!userProfile.emailVerified) {
      ...
      await createSecurityAuditLog('INVENTORY_CREATE_EMAIL_UNVERIFIED', user.id, request, null, 'BLOCKED', blockReason);
    ...
    if (userProfile.role === 'USER' && !userProfile.emailVerified) {
      ...
      await createSecurityAuditLog('INVENTORY_CREATE_UNVERIFIED_RISK', user.id, request, null, 'BLOCKED', blockReason);
    ...
    try {
      ...
      if ((rawBody.match(/\{/g) || []).length > 100) {
        blockReason = 'Suspicious JSON structure detected (potential JSON bomb)';
        throw new Error('Malformed request');
      }
      
      body = JSON.parse(rawBody);
    } catch (error) {
      blockReason = blockReason || 'Invalid JSON payload or malicious request structure';
      await createSecurityAuditLog('INVENTORY_CREATE_INVALID_JSON', user.id, request, null, 'BLOCKED', blockReason);
    ...
    try {
      ...
      await createSecurityAuditLog('INVENTORY_CREATE_VALIDATION_FAILED', user.id, request, body, 'BLOCKED', blockReason);
    ...
    if (!orgAccess.valid) {
      ...
      await createSecurityAuditLog('INVENTORY_CREATE_ORG_ACCESS_DENIED', user.id, request, { orgId: validatedData.organizationId }, 'BLOCKED', blockReason);
    ...
    if (!warehouseAccess) {
      blockReason = 'No access to specified warehouse or warehouse inactive';
      await createSecurityAuditLog('INVENTORY_CREATE_WAREHOUSE_ACCESS_DENIED', user.id, request, { warehouseId: validatedData.warehouseId }, 'BLOCKED', blockReason);
    ...
    if (existingSKU) {
      ...
      await createSecurityAuditLog('INVENTORY_CREATE_DUPLICATE_SKU', user.id, request, { sku: validatedData.sku }, 'BLOCKED', blockReason);
    ...
    if (todaysCreations >= dailyLimit) {
      ...
      await createSecurityAuditLog('INVENTORY_CREATE_DAILY_LIMIT', user.id, request, { count: todaysCreations, limit: dailyLimit }, 'BLOCKED', blockReason);
    ...
    const inventoryItem = await prisma.$transaction(async (tx: any) => {
      ...
      const item = await tx.inventoryItem.create({
        data: {
          ...
          status: (validatedData.quantity || 0) > 0 ? 'ACTIVE' : 'OUT_OF_STOCK',
          organizationId: validatedData.organizationId,
