...
const createVerificationSchema = z.object({
  ...
  verificationType: z.enum(['PRE_LOAD', 'POST_LOAD', 'QUALITY_CHECK', 'FINAL_INSPECTION']),
});
...
const reportDiscrepancySchema = z.object({
  verificationId: z.string(),
  itemId: z.string(),
  discrepancyType: z.enum(['QUANTITY_MISMATCH', 'DAMAGED_ITEM', 'MISSING_ITEM', 'WRONG_ITEM', 'PACKAGING_ISSUE']),
  ...
  try {
    ...
    switch (action) {
      ...
      case 'scan_item': {
        ...
        const verification = verifications.find(v => v.id === data.verificationId);
        if (!verification) {
      ...
      case 'report_discrepancy': {
        ...
        if (data.severity === 'CRITICAL') {
          ...
          console.log('CRITICAL DISCREPANCY - Notifying supervisors:', newDiscrepancy);
        }
  ...
  try {
    ...
    switch (action) {
      case 'verification': {
        ...
        const items = verificationItems.filter(i => i.verificationId === verificationId);
        const verificationDiscrepancies = discrepancies.filter(d => d.verificationId === verificationId);
      ...
      case 'quality_metrics': {
        ...
        const avgAccuracy = completedVerifications.length > 0
          ? completedVerifications.reduce((sum, v) => sum + v.verificationScore, 0) / completedVerifications.length
          : 0;
        ...
        // Top issues
        const issueTypeCounts = discrepancies.reduce((acc, d) => {
        ...
        const verifierStats = verifications.reduce((acc, v) => {
        ...
        const verifierPerformance = Object.values(verifierStats).map((stats: any) => {
          const completed = stats.verifications.filter((v: LoadVerification) => v.status === 'COMPLETED');
          const avgAccuracy = completed.length > 0
            ? completed.reduce((sum: number, v: LoadVerification) => sum + v.verificationScore, 0) / completed.length
            : 0;
