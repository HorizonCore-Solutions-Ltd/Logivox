...
const calculateShippingRate = (
  ...
  const baseRates: Record<string, number> = {
    ...
    fedex_ground: 8.0,
    fedex_2day: 14.5,
    fedex_overnight: 24.0,
    usps_priority: 7.5,
    usps_express: 20.0,
    usps_first: 5.5,
  };

  const baseRate = baseRates[service] || 10.0;
  const weightMultiplier = 1 + weight / 10;
  const distanceMultiplier = 1 + distance / 1000;

  return parseFloat(
    (baseRate * weightMultiplier * distanceMultiplier).toFixed(2),
  );
};
    ...
    switch (action) {
      case "generate_label": {
        ...
        const newLabel: ShippingLabel = {
          ...
          carrierName:
            carriers.find((c) => c.id === data.carrierId)?.name || "Unknown",
  ...
  try {
    ...
    switch (action) {
      ...
      case "label_metrics": {
        ...
        // Carrier breakdown
        const carrierStats = activeLabels.reduce(
          (acc, label) => {
        ...
        const carrierBreakdown = Object.values(carrierStats)
          .map((stat: any) => ({
            ...
            percentage:
              totalLabels > 0 ? (stat.labelCount / totalLabels) * 100 : 0,
          }))
          .sort((a, b) => b.labelCount - a.labelCount);

        // Service breakdown
        const serviceStats = activeLabels.reduce(
          (acc, label) => {
        ...
        // Labels by format
        const labelsByFormat = labels.reduce(
          (acc, label) => {
