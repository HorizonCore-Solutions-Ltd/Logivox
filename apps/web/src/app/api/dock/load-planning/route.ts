...
const createLoadPlanSchema = z.object({
  ...
  truckType: z.enum(['DRY_VAN_53', 'DRY_VAN_48', 'REEFER_53', 'FLATBED', 'BOX_TRUCK']),
  ...
  constraints: z.object({
    ...
    weightDistribution: z.enum(['FRONT_HEAVY', 'BALANCED', 'REAR_HEAVY']).optional(),
  }).optional(),
});
...
const optimizeLoadSchema = z.object({
  ...
  optimizationGoal: z.enum(['MAXIMIZE_CUBE', 'MINIMIZE_DAMAGE', 'BALANCE_WEIGHT', 'FASTEST_LOAD']),
});
...
function packItems(
  ...
  for (const item of sortedItems) {
    for (let qty = 0; qty < item.quantity; qty++) {
      ...
      const rotations = [
        { length: item.length, width: item.width, height: item.height, rotation: 0 as const },
        { length: item.width, width: item.length, height: item.height, rotation: 90 as const },
      ];
...
function calculateUtilization(
  ...
  const totalVolume = placedItems.reduce((sum, item) => sum + (item.volume || 0), 0);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'create_load_plan') {
      ...
      const placedItems = packItems(itemsWithVolume, truckSpec, 'MAXIMIZE_CUBE');
      
      // Calculate metrics
      const utilization = calculateUtilization(placedItems, itemsWithVolume, truckSpec);
      const weightDistribution = calculateWeightDistribution(placedItems, truckSpec.length);
      ...
      return NextResponse.json({
        ...
        truckSpec: {
          ...truckSpec,
          remainingCube: truckSpec.maxCube - (utilization.cubePercent / 100) * truckSpec.maxCube,
          remainingWeight: truckSpec.maxWeight - (utilization.weightPercent / 100) * truckSpec.maxWeight,
        },
      });
    }
    ...
    if (action === 'optimize_load') {
      ...
      const newUtilization = calculateUtilization(optimizedPlacement, existingPlan.items, truckSpec);
      const newWeightDist = calculateWeightDistribution(optimizedPlacement, truckSpec.length);
      ...
      const improvement = {
        cubeUtilization: newUtilization.cubePercent - existingPlan.utilization.cubePercent,
        weightBalance: Math.abs(newWeightDist.middle - 60) < Math.abs(existingPlan.weightDistribution.middle - 60),
        itemsPlaced: optimizedPlacement.length - existingPlan.placedItems.length,
      ...
      return NextResponse.json({
        ...
        capacity: {
          cube: {
            ...
            remaining: truckSpec.maxCube - (loadPlan.utilization.cubePercent / 100) * truckSpec.maxCube,
          },
          weight: {
            used: (loadPlan.utilization.weightPercent / 100) * truckSpec.maxWeight,
            available: truckSpec.maxWeight,
            remaining: truckSpec.maxWeight - (loadPlan.utilization.weightPercent / 100) * truckSpec.maxWeight,
          },
        },
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'load_plan') {
      ...
      if (!loadPlanId) {
        return NextResponse.json({ error: 'Load plan ID required' }, { status: 400 });
      }
    ...
    if (action === 'load_summary') {
      const summary = {
        ...
        avgScore: 88.5,
        topPerformers: [
          { id: 'LOAD-001', shipmentId: 'SHP-501', score: 96, cubeUtilization: 94.2 },
          { id: 'LOAD-002', shipmentId: 'SHP-502', score: 94, cubeUtilization: 91.8 },
          { id: 'LOAD-003', shipmentId: 'SHP-503', score: 92, cubeUtilization: 89.5 },
        ],
        needsOptimization: [
          { id: 'LOAD-044', shipmentId: 'SHP-544', score: 68, cubeUtilization: 72.1, issue: 'Low cube utilization' },
          { id: 'LOAD-045', shipmentId: 'SHP-545', score: 71, cubeUtilization: 83.2, issue: 'Poor weight balance' },
        ],
      };
...
function generateRecommendations(
  ...
  if (loadPlan.utilization.cubePercent < 75) {
    recommendations.push('Low cube utilization. Consider consolidating with another shipment.');
  ...
  if (Math.abs(middle - 60) > 15) {
    recommendations.push('Weight distribution is unbalanced. Reposition heavier items toward center.');
  }
  if (front > 35) {
    recommendations.push('Too much weight in front. Risk of poor handling and braking.');
  }
  if (rear > 35) {
    recommendations.push('Too much weight in rear. Risk of reduced traction and stability.');
  }

  // Items placement recommendations
  const placementRate = (loadPlan.placedItems.length / loadPlan.utilization.itemsTotal) * 100;
  if (placementRate < 90) {
    recommendations.push(`Only ${placementRate.toFixed(0)}% of items placed. Consider using larger truck or multiple loads.`);
  }

  // Weight utilization
  if (loadPlan.utilization.weightPercent > 95) {
    recommendations.push('Near maximum weight capacity. Verify axle weight limits.');
  }
