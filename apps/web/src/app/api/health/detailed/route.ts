...
export async function GET() {
  try {
    ...
    const detailedHealth = {
      ...
      database: {
        status: databaseStatus.status === 'fulfilled' ? 'connected' : 'disconnected',
        error: databaseStatus.status === 'rejected' ? databaseStatus.reason?.message : null
      },
      ...
      system: systemInfo.status === 'fulfilled' ? {
        ...
        totalMemory: Math.round(systemInfo.value.totalMemory / 1024 / 1024 / 1024), // GB
        freeMemory: Math.round(systemInfo.value.freeMemory / 1024 / 1024 / 1024), // GB
        loadAverage: systemInfo.value.loadAverage,
      } : null,

      services: {
        prisma: databaseStatus.status === 'fulfilled' ? 'operational' : 'degraded',
        nextauth: process.env.NEXTAUTH_URL ? 'configured' : 'not_configured',
        uploads: process.env.S3_BUCKET_NAME || process.env.UPLOAD_DIR ? 'configured' : 'local_only',
      }
    }
