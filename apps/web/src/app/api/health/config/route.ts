...
function validateAuthentication(errors: string[], warnings: string[]): ConfigCheck {
  const issues: string[] = [];

  if (!process.env.NEXTAUTH_SECRET) {
    errors.push('NEXTAUTH_SECRET is required');
    issues.push('NextAuth secret missing');
  } else if (process.env.NEXTAUTH_SECRET.includes('test') || 
  ...
  if (issues.length > 0) {
    return {
      status: errors.some(e => e.includes('NEXTAUTH')) ? 'invalid' : 'warning',
...
function validateIntegrations(errors: string[], warnings: string[]): ConfigCheck {
  const issues: string[] = [];
