  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (pickList.status === "PICKED" || pickList.status === "CANCELLED") {
        throw new Error(`Cannot update pick list with status: ${pickList.status}`);
      }
      ...
      // Validate quantity
      const newTotalPicked = pickListItem.quantityPicked + validatedData.quantityPicked;
      ...
      const updatedItem = await tx.pickListItem.update({
        ...
        data: {
          ...
          pickedAt: newTotalPicked === pickListItem.quantityToPick ? new Date() : undefined,
          ...(validatedData.binLocation && { binLocation: validatedData.binLocation }),
          ...(validatedData.batchNumber && { batchNumber: validatedData.batchNumber }),
