...
const slottingRuleSchema = z.object({
  ...
  targetZoneType: z.enum(['PICKING', 'RESERVE', 'BULK', 'FAST_PICK', 'SLOW_PICK']),
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch slotting rules' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create slotting rule' }, { status: 500 });
  }
}
