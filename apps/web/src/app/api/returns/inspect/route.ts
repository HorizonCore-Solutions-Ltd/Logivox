...
export async function POST(req: NextRequest) {
  try {
    ...
    const { rmaId, itemId, condition, action, notes, quantity, photos } = body;
    ...
    if (rmaItem.rmaId !== rmaId) {
      return NextResponse.json(
        { error: "Item does not belong to RMA" },
        { status: 400 },
      );
    }
    ...
    if (quantity > availableQty) {
      return NextResponse.json(
        {
          error: `Cannot inspect more than received quantity (${availableQty})`,
        },
        { status: 400 },
      );
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (rmaItem.isInspected && quantity !== rmaItem.quantityReceived) {
        throw new Error(
          "Cannot partially inspect an already inspected item. Re-inspect full line only.",
        );
      }
      ...
      if (quantity === availableQty) {
        ...
        await tx.rMAItem.update({
          ...
          data: {
            ...
            quantityAccepted:
              action !== "DISPOSE" && action !== "REJECTED" ? quantity : 0,
            quantityRejected:
              action === "DISPOSE" || action === "REJECTED" ? quantity : 0,
          },
        });
      ...
      // CASE B: Partial Inspection (Split)
        ...
        const quantityRequestedMove = Math.min(
          quantity,
          rmaItem.quantityRequested,
        );
        ...
        await tx.rMAItem.update({
          ...
          data: {
            ...
            quantityRequested:
              rmaItem.quantityRequested - quantityRequestedMove,
          },
        });
      }
      ...
      const meaningfulItems = allItems.filter(
        (i) => (i.quantityReceived ?? 0) > 0,
      );
