  ...
  try {
    ...
    if (items.length === 0) {
      return NextResponse.json({
        message: "No eligible items found for disposition.",
      });
    }
    ...
    for (const item of items) {
      ...
      if (item.action === "DISPOSE") {
        ...
        if (["NEW", "GOOD"].includes(item.condition || "")) {
          ...
          ["DAMAGED", "DESTROYED", "DEFECTIVE"].includes(item.condition || "")
        ) {
    ...
    const allRmaItems = await prisma.rMAItem.findMany({
      where: { rmaId: rmaId },
    });
    ...
    const meaningfulItems = allRmaItems.filter(
      (i) => (i.quantityReceived ?? 0) > 0,
    );
    const allCovered = meaningfulItems.every((i) =>
      allTasks.some((t) => t.sourceId === i.id),
    );
