...
const DOCK_DOOR_TYPES = {
  ...
  REFRIGERATED: { name: 'Temperature Controlled', capacity: 2, slotDuration: 45 },
...
const BookAppointmentSchema = z.object({
  ...
  carrierTier: z.enum(['PLATINUM', 'GOLD', 'SILVER', 'BRONZE', 'STANDARD']).optional(),
  contactName: z.string().min(1),
  contactPhone: z.string().min(10),
  contactEmail: z.string().email(),
  appointmentDate: z.string(), // ISO date
  appointmentTime: z.string(), // HH:mm format
  dockDoorType: z.enum(['STANDARD', 'EXPRESS', 'OVERSIZED', 'REFRIGERATED', 'HAZMAT']),
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'stats') {
      ...
      const averageWaitTime = completedAppointments > 0 
        ? Math.round(totalWaitTime / completedAppointments) 
        : 0;

      const onTimeRate = (onTimeArrivals + lateArrivals) > 0 
      ...
      const timeSaved = completedAppointments * (manualWaitTime - appointmentWaitTime);
  ...
  try {
    ...
    switch (validatedData.action) {
      case 'book_appointment': {
        ...
        const appointmentDateTime = new Date(`${validatedData.appointmentDate}T${validatedData.appointmentTime}`);
        ...
        const expectedDuration = validatedData.expectedDuration || dockConfig.slotDuration;
        ...
        return NextResponse.json({
          ...
          appointment: {
            ...
            expectedDuration,
            status: 'CONFIRMED',
            message: 'Appointment confirmed! Confirmation sent to ' + validatedData.contactEmail
          }
        });
      }
      ...
      case 'check_in': {
        ...
        const diffMinutes = (actualArrivalTime.getTime() - scheduledTime.getTime()) / 60000;
        ...
        return NextResponse.json({
          ...
          checkIn: {
            ...
            message: isOnTime 
              ...
              : 'Checked in late. Please proceed to dock door ' + metadata.dockDoor
          }
        });
      }
      ...
      case 'check_availability': {
        ...
        const utilizationRate = ((totalSlots - availableCount) / totalSlots) * 100;
