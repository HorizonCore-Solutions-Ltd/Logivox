...
async function identifyOpportunity(
  ...
  for (const order of matchingOrders) {
    ...
    const matchedSKUs = receivingItems.filter((sku) =>
      orderItems.includes(sku),
    );
    ...
    if (matchPercent >= CROSS_DOCK_CRITERIA.MIN_ORDER_MATCH_PERCENT) {
      const daysUntilDue = Math.ceil(
        (new Date(order.shipByDate).getTime() - Date.now()) /
          (1000 * 60 * 60 * 24),
      );
...
function calculateSavings(type: CrossDockType, quantity: number): number {
  const COST_PER_PUTAWAY = 2.5;
  const COST_PER_PICK = 1.75;
  const STORAGE_COST_PER_DAY = 0.5;
...
async function stageItem(session: any, data: z.infer<typeof stageItemSchema>) {
  ...
  await prisma.activityLog.create({
    data: {
      ...
      metadata: {
        ...
        stagedAt: new Date().toISOString(),
        maxStagingTime: new Date(
          Date.now() + CROSS_DOCK_CRITERIA.STAGING_DURATION_MINUTES * 60000,
        ).toISOString(),
      },
    },
  });
...
async function completeTransfer(
  ...
  const cycleTimeHours =
    Math.round(
      ((new Date().getTime() - crossDock.expectedArrival.getTime()) /
        (1000 * 60 * 60)) *
        10,
    ) / 10;
  ...
  try {
    ...
    if (action === "active-opportunities") {
      const opportunities = await prisma.crossDockOpportunity.findMany({
        ...
        orderBy: [{ priorityScore: "desc" }, { targetShipDate: "asc" }],
        take: 50,
      });
