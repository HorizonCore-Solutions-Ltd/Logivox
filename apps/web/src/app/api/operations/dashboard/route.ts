...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      prisma.taskExecution.count({ where: { organizationId, type: "REPLENISHMENT", status: "IN_PROGRESS" } }),
      prisma.taskExecution.count({ where: { organizationId, type: "PUTAWAY", status: "IN_PROGRESS" } }),
      prisma.wavePick.count({ where: { organizationId, status: "RELEASED" } }),
      prisma.wavePick.count({ where: { organizationId, status: { in: ["PLANNED", "READY"] } } }),
      prisma.loadSheet.count({ where: { organizationId, status: "APPROVED" } }),
      prisma.bayDoor.findMany({ where: { organizationId, doorType: "INBOUND" } }),
      prisma.bayDoor.findMany({ where: { organizationId, doorType: "OUTBOUND" } }),
    ...
    const inboundOccupied = inboundDoors.filter(d => d.status === 'OCCUPIED').length;
    const outboundOccupied = outboundDoors.filter(d => d.status === 'OCCUPIED').length;
    ...
    const dockStatus = {
      ...
      inboundOccupied: inboundDoors.filter(d => d.status === 'OCCUPIED').length,
      outboundTotal: outboundDoors.length,
      outboundOccupied: outboundDoors.filter(d => d.status === 'OCCUPIED').length,
    };
    ...
    const availableZones = ["ZONE-A", "ZONE-B", "PACKING", "RECEIVING", "MARSHALLING"];
    ...
    const wavesProgressRaw = await prisma.wavePick.findMany({
      ...
      select: { id: true, waveNumber: true, status: true, progress: true, totalLines: true, pickedLines: true },
      take: 5
    });
    ...
    const alerts = [];
    if (wavesPending > 10) {
        alerts.push({ id: "alert-1", type: "SLA", message: "High pending wave count", severity: "MEDIUM", timestamp: new Date().toISOString() });
    }
    
    // Simulate "Live Issues" (Andon) from the picker floor
    const andonEvents = [
        { id: "ev-1", type: "LABOR", message: "Picker #42 blocked in Aisle 4 (Dropped Pallet)", severity: "HIGH", timestamp: new Date(Date.now() - 1000 * 60 * 5).toISOString() },
        { id: "ev-2", type: "EQUIPMENT", message: "Scanner #08 battery low", severity: "LOW", timestamp: new Date(Date.now() - 1000 * 60 * 15).toISOString() },
        { id: "ev-3", type: "INVENTORY", message: "SKU-992 marked missing during pick", severity: "MEDIUM", timestamp: new Date(Date.now() - 1000 * 60 * 2).toISOString() },
    ];
    ...
    const leaderboard = activeUsers.slice(0, 5).map((u, i) => ({
        id: u.id,
