...
const refundSchema = z.object({
  reason: z.enum(["ITEM_NOT_ARRIVED", "DAMAGED_ITEM", "JOB_CANCELLED", "CUSTOMER_REQUEST", "OTHER"]),
...
export async function POST(
  ...
  try {
    ...
    const alreadyRefunded = order.refunds.reduce((acc, refund) => acc + Number(refund.amount), 0);
    ...
    if (totalRefunded >= Number(order.paidAmount) && Number(order.paidAmount) > 0) {
