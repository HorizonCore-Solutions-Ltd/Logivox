...
export async function POST(request: NextRequest) {
  try {
    ...
    const { template, fields, filters, dateRange, chartConfig } = await request.json();
    ...
    switch (template) {
      case 'inventory-valuation':
        rows = await generateInventoryValuation(session.user.organizationId, fields, filters, dateRange);
        break;

      case 'inventory-turnover':
        rows = await generateInventoryTurnover(session.user.organizationId, fields, filters, dateRange);
        break;

      case 'stock-movement':
        rows = await generateStockMovement(session.user.organizationId, fields, filters, dateRange);
        break;

      case 'order-summary':
        rows = await generateOrderSummary(session.user.organizationId, fields, filters, dateRange);
        break;

      case 'order-fulfillment-time':
        rows = await generateFulfillmentTime(session.user.organizationId, fields, filters, dateRange);
        break;

      case 'picking-efficiency':
        rows = await generatePickingEfficiency(session.user.organizationId, fields, filters, dateRange);
        break;

      case 'warehouse-utilization':
        rows = await generateWarehouseUtilization(session.user.organizationId, fields, filters);
        break;

      case 'revenue-by-product':
        rows = await generateRevenueByProduct(session.user.organizationId, fields, filters, dateRange);
        break;

      case 'kpi-dashboard':
        rows = await generateKPIDashboard(session.user.organizationId, dateRange);
        break;
...
async function generateStockMovement(orgId: string, fields: string[], filters: any[], dateRange: any) {
...
async function generateOrderSummary(orgId: string, fields: string[], filters: any[], dateRange: any) {
  const orders = await prisma.order.findMany({
    where: {
      organizationId: orgId,
      createdAt: {
...
async function generateFulfillmentTime(orgId: string, fields: string[], filters: any[], dateRange: any) {
  const orders = await prisma.order.findMany({
    where: {
      organizationId: orgId,
      createdAt: {
  ...
  return orders.map(order => {
    const fulfillmentTime = order.shippedAt 
      ? (order.shippedAt.getTime() - order.createdAt.getTime()) / (1000 * 60 * 60)
      : 0;
...
async function generateWarehouseUtilization(orgId: string, fields: string[], filters: any[]) {
  ...
  warehouses.forEach(warehouse => {
    warehouse.zones.forEach(zone => {
      ...
      const occupiedLocations = zone.locations.filter(loc => loc.currentQuantity > 0).length;
      const utilization = totalLocations > 0 ? (occupiedLocations / totalLocations) * 100 : 0;
...
async function generateRevenueByProduct(orgId: string, fields: string[], filters: any[], dateRange: any) {
  const orders = await prisma.order.findMany({
    where: {
      organizationId: orgId,
      createdAt: {
...
async function generateKPIDashboard(orgId: string, dateRange: any) {
  ...
  const totalRevenue = revenue.reduce((sum, order) => {
    return sum + order.lineItems.reduce((orderSum, item) => 
      orderSum + (item.quantity * (item.product.price || 0)), 0
    );
  }, 0);
