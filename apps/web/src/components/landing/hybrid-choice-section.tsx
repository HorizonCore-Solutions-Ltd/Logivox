  ...
  const modes = [
    {
      ...
      description:
        "Perfect for onboarding. Screen shows product images, maps, and large text instructions to build confidence.",
    ...
    },
    {
      ...
      description:
        "Screen dims to OLED black to save battery. Wakes only for exceptions (short picks) or when you ask.",
    ...
    },
    {
      ...
      description:
        "Screen off or pocketed. 100% audio interaction for maximum throughput and hands-free operation.",
      ...
      <div className="container px-4 md:px-6 mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-xl text-muted-foreground">
            LogiVox doesn't force a philosophy. We provide a toolset. Select the
            mode that fits your workforce proficiency.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {modes.map((mode, index) => (
            ...
            >
              <Card
                className={`h-full border-2 ${mode.borderColor} hover:shadow-lg transition-all`}
              >
                <CardHeader
                  className={`${mode.bgColor} border-b ${mode.borderColor} py-8`}
                >
                  <div
                    className={`w-12 h-12 rounded-lg bg-white flex items-center justify-center mb-4 shadow-sm mx-auto`}
                  >
                  ...
                  <div className="space-y-4">
                    ...
                    <ul className="space-y-2 text-left px-4">
                      {mode.features.map((feature) => (
                        ...
                        >
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${mode.color.replace("text-", "bg-")}`}
                          />
                          {feature}
                        </li>
                      ))}
