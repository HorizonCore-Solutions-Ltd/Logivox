...
export const InvoiceService = {
     ...
     */
    generateInvoices: async (organizationId: string, clientId?: string, periodEnd: Date = new Date()) => {
        ...
        // 2. Group by Client
        const txnsByClient = transactions.reduce((acc, txn) => {
        ...
        for (const [cId, txns] of Object.entries(txnsByClient)) {
            ...
            const earliestDate = txns.length > 0 
                ? txns.reduce((min, t) => t.transactionDate < min ? t.transactionDate : min, txns[0].transactionDate)
                : new Date();
