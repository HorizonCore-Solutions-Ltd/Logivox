...
export const SimulationEngine = {
    ...
    async runSimulation(
        ...
        for (let time = 0; time < params.durationMinutes; time += params.timeStepMinutes) {
...
function applyScenarioOverrides(state: TwinState, params: SimulationParameters): TwinState {
...
function processTimeStep(state: TwinState, stepMinutes: number, metrics: SimulationMetrics) {
    ...
    const idleResources = Array.from(state.resources.values()).filter(r => r.status === "IDLE");
    ...
    if (pendingTasks.length > 50) { // arbitrary threshold for demo
