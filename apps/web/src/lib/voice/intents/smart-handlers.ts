...
export async function handleCrateLookup(
  ...
  const itemCount = crate.containerItems.reduce((sum, item) => sum + item.quantity, 0);
...
export async function handleExplainItem(sku: string): Promise<string> {
    const item = await prisma.inventoryItem.findFirst({
        ...
        select: { description: true, name: true, category: { select: { name: true } } }
    });
    ...
    if (item.category?.name) desc += ` It should be in the ${item.category.name} section.`;
...
export async function handlePickRequest(
   ...
   if (sku && task.inventoryItem?.sku && task.inventoryItem.sku.toLowerCase() !== sku.toLowerCase()) {
...
export async function handleScan(userId: string, barcode: string): Promise<string> {
    const task = await prisma.pickingTask.findFirst({
        where: { assignedToId: userId, status: "IN_PROGRESS" },
        include: { inventoryItem: true }
    });
    ...
    if (task.inventoryItem?.sku === barcode || task.inventoryItem?.id === barcode) {
