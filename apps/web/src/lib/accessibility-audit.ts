  ...
  buttons.forEach((button, index) => {
    ...
    if (!hasText && !hasAriaLabel && !hasAriaLabelledBy && !hasTitle) {
      issues.push({
        ...
        recommendation: 'Add text content, aria-label, aria-labelledby, or title attribute',
  ...
  inputs.forEach((input, index) => {
    ...
    if (!hasLabel && !hasAriaLabel && !hasAriaLabelledBy) {
      issues.push({
        ...
        recommendation: 'Add a <label> element or aria-label/aria-labelledby attribute',
  ...
  images.forEach((img, index) => {
    ...
    if (!isDecorative && alt === null && !ariaLabel) {
      issues.push({
        ...
        recommendation: 'Add alt text describing the image, or alt="" if decorative',
      ...
      issues.push({
        ...
        recommendation: 'Add descriptive alt text or mark as aria-hidden="true" if decorative',
        wcagCriterion: '1.1.1 Non-text Content',
      });
    }
  });
...
function auditLandmarks(): AccessibilityIssue[] {
  ...
  if (!navLandmark) {
    issues.push({
      ...
      recommendation: 'Add <nav> element or role="navigation" for main navigation',
      wcagCriterion: '1.3.1 Info and Relationships',
    });
  }
    ...
    mainLandmarks.forEach((landmark, index) => {
      ...
      if (!ariaLabel && !ariaLabelledBy) {
        issues.push({
          ...
          recommendation: 'Add aria-label to distinguish between multiple main landmarks',
  ...
  tables.forEach((table, index) => {
    ...
    if (!caption && !ariaLabel && !ariaLabelledBy) {
      issues.push({
        ...
        recommendation: 'Add <caption> element or aria-label to describe table content',
        wcagCriterion: '1.3.1 Info and Relationships',
      });
    }
...
export function enableLiveAccessibilityMonitoring(): void {
  if (process.env.NODE_ENV !== 'development') {
    console.warn('Live accessibility monitoring is only available in development mode');
    return;
  }
