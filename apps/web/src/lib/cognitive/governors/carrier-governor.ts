...
export const CarrierGovernor: GovernorInterface = {
  ...
  async evaluate(context: DecisionContext): Promise<ProposedAction[]> {
    if (context.triggerEvent !== "ORDER_PLACED" || !context.metadata?.orderId)
      return [];
    ...
    return [
      {
        ...
        predictedOutcomes: [
          ...
          },
          {
            ...
            value: bestOption.cost,
            confidence: 1.0,
            impact: goal === "MINIMIZE_COST" ? "POSITIVE" : "NEGATIVE",
          },
        ],
        confidence: 0.98,
  ...
  async execute(action: ProposedAction): Promise<boolean> {
    console.log(
      `[CarrierGovernor] Assigning Carrier: ${action.parameters.carrier}`,
    );
