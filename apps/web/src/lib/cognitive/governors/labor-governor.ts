...
export const LaborGovernor: GovernorInterface = {
  ...
  async evaluate(context: DecisionContext): Promise<ProposedAction[]> {
    ...
    if (requiredHours > availableCapacity) {
      ...
      return [{
        ...
        parameters: { warehouseId: context.warehouseId, hoursNeeded: requiredHours - availableCapacity },
        reasoning: `Predicted capacity shortfall: ${requiredHours.toFixed(1)}h needed vs ${availableCapacity}h avail.`,
        predictedOutcomes: [
          { metric: "SLA_BREACH_PROBABILITY", value: 0.1, confidence: 0.9, impact: "POSITIVE" }, // Reduced probability
          { metric: "COST_IMP", value: (requiredHours - availableCapacity) * 30, confidence: 1.0, impact: "NEGATIVE" } // Cost increase
        ],
        confidence: 0.95,
      ...
      return [{
        ...
        parameters: { warehouseId: context.warehouseId, excessHours: availableCapacity - requiredHours },
        reasoning: "Excess capacity detected. Reallocating to cycle counting or training.",
        predictedOutcomes: [
          { metric: "EFFICIENCY", value: 15, confidence: 0.8, impact: "POSITIVE" }
        ],
        confidence: 0.85,
        estimatedCost: 0,
        estimatedTimeRaw: 0
  ...
  async execute(action: ProposedAction): Promise<boolean> {
    console.log(`[LaborGovernor] Executing: ${action.actionType}`, action.parameters);
