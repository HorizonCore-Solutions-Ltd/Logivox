...
export const VOICE_COMMANDS: VoiceCommand[] = [
  // Navigation Commands
  {
    patterns: ['go to inventory', 'open inventory', 'show inventory', 'navigate to inventory'],
  ...
  {
    patterns: ['go to reports', 'open reports', 'show reports', 'navigate to reports'],
  ...
  },
  {
    ...
    action: () => { window.location.href = '/dashboard/alerts'; },
    category: 'navigation',
    requiresAuth: true,
  },
  {
  ...
  },
  {
    ...
    action: () => { window.location.href = '/dashboard/bookings'; },
    category: 'navigation',
    requiresAuth: true,
  },
  {
  ...
  },
  {
    ...
    action: () => { window.location.href = '/dashboard/suppliers'; },
    category: 'navigation',
  },
  {
  ...
  {
    patterns: ['find customer {customer}', 'search customer {customer}', 'look for customer {customer}'],
  ...
  {
    patterns: ['find supplier {supplier}', 'search supplier {supplier}', 'look for supplier {supplier}'],
  ...
  },
  {
    ...
    action: (params) => {
      ...
      console.log(`Booking ${params.quantity} ${params.product} for ${params.customer}`);
...
export class VoiceControlEngine {
  ...
  private initializeSpeechRecognition(): void {
    ...
    if (!SpeechRecognition) {
      this.updateState({ supported: false, error: 'Speech recognition not supported' });
      return;
    }
    ...
    if (!this.recognition) {
      this.updateState({ supported: false, error: 'Failed to create speech recognition' });
      return;
    }
  ...
  private processCommand(transcript: string, confidence: number): void {
    console.log(`[VoiceControl] Processing: "${transcript}" (confidence: ${confidence})`);
  ...
  private matchCommand(transcript: string, command: VoiceCommand): { parameters: Record<string, string> } | null {
  ...
  private matchPattern(transcript: string, pattern: string): Record<string, string> | null {
