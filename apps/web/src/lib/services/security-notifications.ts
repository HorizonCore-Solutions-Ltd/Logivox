...
export class SecurityNotificationService {
  ...
  private static async dispatch(notification: any) {
    ...
    switch (notification.deliveryMethod) {
      ...
      default:
        throw new Error(
          `Unsupported delivery method: ${notification.deliveryMethod}`,
        );
    }
  }
  ...
  private static async sendSMS(notification: any) {
    ...
    console.log(
      `SMS would be sent to ${notification.recipientPhone}: ${notification.message}`,
    );
    await this.markDelivered(notification.id);
  }
  ...
  private static async sendWebhook(notification: any) {
    const webhookUrl =
      notification.metadata?.webhookUrl || process.env.SECURITY_WEBHOOK_URL;
    if (!webhookUrl) {
  ...
  static async notifyVisitorOverdue(organizationId: string, visitor: any) {
    await this.sendNotification({
      organizationId,
