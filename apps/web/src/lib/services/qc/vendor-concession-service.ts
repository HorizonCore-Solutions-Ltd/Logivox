...
export class VendorConcessionService {
  ...
  async applyConcession(params: {
    ...
    const newUtilizationAmount =
      Number(concession.utilizationAmount) + params.appliedAmount;
    const newRemainingValue =
      Number(concession.concessionValue) - newUtilizationAmount;
   ...
   * Cancel concession
   */
  async cancelConcession(params: { concessionId: string; reason: string }) {
    return await prisma.vendorConcession.update({
      where: { id: params.concessionId },
      data: {
        status: "CANCELLED",
  ...
  async getStatistics(params: {
    ...
    const totalConcessionValue = concessions.reduce(
      (sum: number, c: any) => sum + Number(c.concessionValue),
      0,
    );
    const totalUtilizedAmount = concessions.reduce(
      (sum: number, c: any) => sum + Number(c.utilizationAmount),
      0,
    );
    const totalRemainingValue = concessions.reduce(
      (sum: number, c: any) => sum + Number(c.remainingValue),
      0,
    );

    const byType = concessions.reduce(
    ...
    const byStatus = concessions.reduce(
      (acc: Record<string, number>, c: any) => {
    ...
    return {
      ...
      totalRemainingValue,
      utilizationRate:
        totalConcessionValue > 0
          ? (totalUtilizedAmount / totalConcessionValue) * 100
          : 0,
  ...
  private async generateConcessionNumber(
    organizationId: string,
  ): Promise<string> {
    const date = new Date();
