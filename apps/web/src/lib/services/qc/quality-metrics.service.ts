...
export class QualityMetricsService {
  ...
  static async getRealTimeMetrics(params: {
    ...
    return {
      ...
      correctiveActions: {
        ...
        onTimePerformance:
          ((capaStats._count - overdueCapas) / (capaStats._count || 1)) * 100,
      },
      qualityHolds: {
        active: activeHolds,
      ...
      customerComplaints: {
        ...
        open: openComplaints,
        responseRate:
          ((complaints._count - openComplaints) / (complaints._count || 1)) *
          100,
      },
      materialReview: {
  ...
  static async getTrendAnalysis(params: {
    ...
    const olderAvg =
      trends.slice(0, 3).reduce((sum, t) => sum + t.count, 0) / 3;
   ...
   */
  static async getQualityScore(params: { organizationId: string }) {
