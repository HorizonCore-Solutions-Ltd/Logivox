 ...
 */
export async function generateLoadSheetNumber(organizationId: string): Promise<string> {
...
export function calculateLoadSheetStats(loadSheet: any) {
  ...
  for (const container of containers) {
    ...
    // Status
    stats.statusBreakdown[container.status] = (stats.statusBreakdown[container.status] || 0) + 1;
  ...
  if (loadSheet.containers) {
    for (const container of loadSheet.containers) {
      ...
      if (container.maxWeight && container.weight > container.maxWeight) {
        errors.push(`Container ${container.containerNumber} exceeds weight limit`);
      }
      
      if (container.status !== 'READY' && container.status !== 'IN_PROGRESS') {
        warnings.push(`Container ${container.containerNumber} status is ${container.status}`);
      }
    }
  }
...
export async function suggestContainer(
  ...
  for (const container of matchingContainers) {
    const availableWeight = (container.maxWeight || Infinity) - (container.weight || 0);
    const availableVolume = (container.maxVolume || Infinity) - (container.volume || 0);
