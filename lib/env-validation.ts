  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      for (const secret of secrets) {
        if (dangerousDefaults.some(def => secret.value.toLowerCase().includes(def.toLowerCase()))) {
          errors.push(`${secret.name} appears to be using a default/development value in production`)
        }
      }

      // Check OAuth configuration
      if (!requiredEnv.GOOGLE_CLIENT_ID && !requiredEnv.GITHUB_ID) {
        warnings.push('No OAuth providers configured - users can only use credentials login')
      }
      ...
      if (!optionalEnv.S3_BUCKET_NAME && !process.env.UPLOAD_DIR) {
        warnings.push('No file storage configured - file uploads will use local filesystem')
      }
    ...
    if (requiredEnv.DATABASE_URL.includes('localhost') && process.env.NODE_ENV === 'production') {
      errors.push('DATABASE_URL should not use localhost in production')
    }

    if (requiredEnv.NEXTAUTH_URL.includes('localhost') && process.env.NODE_ENV === 'production') {
    ...
    return {
      ...
      environment: {
        ...
        GOOGLE_CLIENT_SECRET: requiredEnv.GOOGLE_CLIENT_SECRET ? '***REDACTED***' : undefined,
        GITHUB_SECRET: requiredEnv.GITHUB_SECRET ? '***REDACTED***' : undefined,
      }
    }
  } catch (error) {
    if (error instanceof z.ZodError) {
      const envErrors = error.errors.map(err => 
...
export function validateStartupEnvironment() {
  ...
  console.log(`  Database: ${result.environment.DATABASE_URL ? 'Configured' : 'Not configured'}`)
  console.log(`  Authentication: ${result.environment.NEXTAUTH_SECRET ? 'Configured' : 'Not configured'}`)
  console.log(`  OAuth Providers: ${result.environment.GOOGLE_CLIENT_ID || result.environment.GITHUB_ID ? 'Configured' : 'Not configured'}`)
  console.log(`  Security Features: ${result.environment.SECURITY_HEADERS_ENABLED === 'true' ? 'Enabled' : 'Disabled'}`)
