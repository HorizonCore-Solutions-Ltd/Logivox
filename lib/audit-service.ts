...
export async function logAudit(params: {
  ...
  try {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const currentHash = createHash('sha256').update(hashContent).digest('hex');
 ...
 */
export async function verifyAuditChain(): Promise<{ valid: boolean; brokenAtRecordId?: string }> {
  ...
  for (const log of allLogs) {
    ...
    const calculatedHash = createHash('sha256').update(hashContent).digest('hex');
