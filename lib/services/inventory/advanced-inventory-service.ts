...
export class AdvancedInventoryService {
  ...
  async generateAdvancedForecast(
    ...
    const historicalSales = this.extractHistoricalSales(
      product.salesOrderItems,
    );

    // Run ML models
    const forecast = await this.runEnsembleForecast(
      historicalSales,
      horizonDays,
    );
    ...
    const velocityAnalysis =
      this.calculateVelocityClassification(historicalSales);
  ...
  private exponentialMovingAverage(data: number[], period: number): number[] {
    ...
    for (let i = period; i < data.length; i++) {
      const currentEMA =
        (data[i] - ema[i - period]) * multiplier + ema[i - period];
      ema.push(currentEMA);
    }
  ...
  async performABCAnalysis(
    ...
    const productRevenues = products.map((product) => {
      const annualRevenue = product.salesOrderItems.reduce(
    ...
    return productRevenues.map((product, index) => {
      ...
      const recommendedSafetyStock =
        classification === "A" ? 30 : classification === "B" ? 20 : 10;
      const recommendedReorderPoint =
        classification === "A" ? 50 : classification === "B" ? 30 : 15;
  ...
  private calculateVelocityClassification(historicalSales: number[]): {
    ...
    const recentSales =
      historicalSales.slice(-30).reduce((a, b) => a + b, 0) / 30;
  ...
  private calculateFinancialMetrics(
    ...
    const stockoutCost =
      shortageStock * (product.sellingPrice || product.costPrice || 0) * 0.2;
    const potentialSavings =
      excessStock * (product.costPrice || 0) * carryingCostRate + stockoutCost;
  ...
  private async getLastAutomatedAction(
    productId: string,
  ): Promise<AutomatedAction | undefined> {
