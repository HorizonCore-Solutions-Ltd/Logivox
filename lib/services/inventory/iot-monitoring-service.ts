...
export class IoTMonitoringService {
  ...
  async processRFIDScan(reading: RFIDReading): Promise<{
    ...
    console.log(
      `[IOT] Processing RFID scan from ${reading.readerId}: ${reading.tags.length} tags`,
    );
    ...
    reading.tags.forEach((tag) => {
      productCounts.set(
        tag.productId,
        (productCounts.get(tag.productId) || 0) + 1,
      );
    });
    ...
    for (const [productId, count] of productCounts) {
      try {
        ...
        if (Math.abs(discrepancy) > 0) {
          ...
          if (Math.abs(discrepancy) > product.quantity * 0.1) {
            // >10% discrepancy
            await this.generateAlert({
              ...
              estimatedImpact:
                Math.abs(discrepancy) * (product.costPrice?.toNumber() || 0),
            });
          ...
          if (reading.accuracy >= 95 && Math.abs(discrepancy) <= 5) {
            ...
            console.log(
              `[IOT] ✅ Auto-adjusted ${product.sku}: ${product.quantity} → ${count}`,
            );
          }
        }
    ...
    console.log(
      `[IOT] RFID scan complete: ${results.inventoryUpdated ? "Updated" : "No updates"}, ${results.discrepanciesFound} discrepancies, ${results.alertsGenerated} alerts`,
    );
   ...
   */
  async processWeightSensorReading(
    reading: WeightSensorReading,
  ): Promise<void> {
    console.log(
      `[IOT] Processing weight sensor ${reading.sensorId}: ${reading.weight} kg`,
    );
    ...
    if (reading.confidence >= 90 && reading.discrepancyPercent < 15) {
      ...
      if (product && reading.discrepancy !== 0) {
        ...
        console.log(
          `[IOT] ✅ Updated ${product.sku} from weight sensor: ${product.quantity} → ${reading.estimatedQuantity}`,
        );
      }
    }
  }
   ...
   */
  async processEnvironmentalReading(
    reading: EnvironmentalReading,
  ): Promise<void> {
    console.log(
      `[IOT] Environmental: ${reading.temperature}°C, ${reading.humidity}% RH at ${reading.location}`,
    );
    ...
    if (!reading.withinThresholds) {
      ...
      for (const violation of reading.violations) {
        await this.generateAlert({
          ...
          estimatedImpact: reading.affectedProducts.reduce(
            (sum, p) => sum + p.estimatedDamage,
            0,
          ),
        });
      }
      ...
      for (const affected of reading.affectedProducts) {
        if (
          affected.riskLevel === "CRITICAL" ||
          affected.riskLevel === "HIGH"
        ) {
          await this.flagProductForInspection(
            affected.productId,
            `Environmental exposure: ${reading.violations.join(", ")}`,
          );
        }
      }
    }
  ...
  async synchronizeDigitalTwin(productId: string): Promise<DigitalTwinState> {
    ...
    if (physicalState.quantity !== product.quantity) {
      discrepancies.push({
        ...
        severity:
          Math.abs(physicalState.quantity - product.quantity) > 10
            ? "HIGH"
            : "MEDIUM",
      });
    }
   ...
   */
  private async autoSyncDiscrepancies(
    productId: string,
    discrepancies: any[],
  ): Promise<void> {
    console.log(
      `[IOT] Auto-syncing ${discrepancies.length} discrepancies for product ${productId}`,
    );
  ...
  async predictDeviceMaintenance(deviceId: string): Promise<{
    ...
    if (device.batteryLevel && device.batteryLevel < 20) {
      ...
      predictedFailureDate.setDate(
        predictedFailureDate.getDate() + daysRemaining,
      );
    }
    ...
    const daysSinceCalibration = device.lastCalibration
      ? Math.floor(
          (Date.now() - new Date(device.lastCalibration).getTime()) /
            (1000 * 60 * 60 * 24),
        )
      : 999;
  ...
  private async generateAlert(alert: Omit<IoTAlert, "alertId">): Promise<void> {
    ...
    console.log(
      `[IOT] 🚨 Alert generated: ${alert.severity} - ${alert.message}`,
    );
  ...
  private async flagProductForInspection(
    productId: string,
    reason: string,
  ): Promise<void> {
  ...
  private async storeEnvironmentalReading(
    reading: EnvironmentalReading,
  ): Promise<void> {
