...
export class SerialTrackingService {
   ...
   */
  async validateReturnSerial(
    request: SerialValidationRequest,
    ...
    const checks: any = {
      ...
      serialExists: {
        valid: !!serialInfo,
        message: serialInfo
          ? "Serial number found in system"
          : "Serial number not found in system",
      },
      soldToCustomer: { valid: false, message: "Not sold to this customer" },
      alreadyReturned: {
        returned: false,
        returnCount: 0,
        message: "No previous returns",
      },
      counterfeitRisk: {
        risk: "LOW",
        confidence: 0,
        evidence: [],
        message: "No counterfeit indicators detected",
      },
      serialMismatch: {
        matches: true,
        message: "Serial matches expected value",
      },
    ...
    if (!serialInfo) {
      ...
      if (
        counterfeitCheck.risk === "HIGH" ||
        counterfeitCheck.risk === "CRITICAL"
      ) {
        valid = false;
        recommendation = "REJECT";
        requiresPhotos = true;
      ...
      if (serialInfo.warranty) {
        const warrantyCheck = await this.checkWarrantyStatus(
          serialInfo.warranty,
        );
  ...
  async detectSerialSwap(params: {
    ...
    if (returnedInfo && expectedInfo && returnedInfo.sku !== expectedInfo.sku) {
      ...
      discrepancies.push(
        `Returned serial belongs to ${returnedInfo.sku}, expected ${expectedInfo.sku}`,
      );
    }
    ...
    if (returnedInfo && expectedInfo) {
      if (
        returnedInfo.condition === "DEFECTIVE" &&
        expectedInfo.condition === "NEW"
      ) {
    ...
    if (returnedInfo) {
      if (returnedInfo.returnHistory.timesReturned > 2) {
        ...
        discrepancies.push(
          `Returned serial has been returned ${returnedInfo.returnHistory.timesReturned} times`,
        );
      }
    }
  ...
  async trackSerial(serialNumber: string): Promise<{
    ...
    const daysInSystem = Math.floor(
      (Date.now() - serialInfo.createdAt.getTime()) / 86400000,
    );
    const timesShipped = serialInfo.lifecycle.filter(
      (e) => e.event === "SHIPPED",
    ).length;
