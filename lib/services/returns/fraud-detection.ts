...
export interface FraudRule {
  ...
  // Conditions
  conditions: {
    type: 'CUSTOMER_RETURN_RATE' | 'RETURN_VALUE' | 'SERIAL_VALIDATION' | 'TIME_WINDOW' | 'CUSTOM';
...
export class FraudDetectionService {
  ...
  async analyze(request: FraudAnalysisRequest): Promise<FraudAnalysisResult> {
    ...
    if (request.items.some(item => item.serial && item.originalSerial && item.serial !== item.originalSerial)) {
      const signal = {
    ...
    if (request.customerHistory) {
      ...
      const returnsPerMonth = request.customerHistory.totalReturns / (request.customerHistory.daysAsCustomer / 30);
    ...
    if (request.returnReason === 'UNWANTED' && request.hasOriginalPackaging === false) {
      const signal = {
    ...
    const customerRiskProfile = request.customerHistory ? {
      returnFrequency: request.customerHistory.totalReturns / (request.customerHistory.daysAsCustomer / 30),
      returnValueAvg: request.returnValue,
      suspiciousPatterns: signals.filter(s => s.severity === 'HIGH' || s.severity === 'CRITICAL').map(s => s.signal),
   ...
   */
  private async checkDuplicateReturns(items: any[]): Promise<{ found: boolean; count: number }> {
   ...
   */
  private async runMLFraudModel(request: FraudAnalysisRequest): Promise<number> {
    ...
    if (request.customerHistory) {
      ...
      if (request.customerHistory.daysAsCustomer < 30 && request.returnValue > 500) score += 25;
      
      // Return value higher than typical
      if (request.returnValue > request.customerHistory.avgOrderValue * 2) score += 20;
    }
   ...
   */
  private generateReasoning(signals: any[], fraudScore: number, riskLevel: string): string {
    if (signals.length === 0) {
   ...
   */
  async evaluateRules(request: FraudAnalysisRequest, rules: FraudRule[]): Promise<FraudRule[]> {
   ...
   */
  private async evaluateCondition(condition: any, request: FraudAnalysisRequest): Promise<boolean> {
    switch (condition.type) {
      case 'CUSTOMER_RETURN_RATE':
        if (!request.customerHistory) return false;
        return this.compare(request.customerHistory.returnRate, condition.operator, condition.value);

      case 'RETURN_VALUE':
        return this.compare(request.returnValue, condition.operator, condition.value);

      case 'SERIAL_VALIDATION':
        const hasSerialMismatch = request.items.some(
          item => item.serial && item.originalSerial && item.serial !== item.originalSerial
        );
   ...
   */
  async logFraudAttempt(result: FraudAnalysisResult, blocked: boolean): Promise<void> {
    // Log to analytics/monitoring system
    console.log(`Fraud attempt logged: RMA ${result.rmaId}, Score: ${result.fraudScore}, Blocked: ${blocked}`);
  }
}
...
export class FraudMonitoringService {
   ...
   */
  async getStatistics(startDate: Date, endDate: Date): Promise<FraudStatistics> {
    ...
    return {
      ...
      estimatedFraudPrevented: 12580.50,
      currency: 'USD',
    };
  }
