...
export class VendorComplianceService {
  ...
  async createCheck(params: {
    ...
    // Calculate metrics
    const totalItems = params.checklistItems.filter(item => item.result !== 'NA').length;
    const passedItems = params.checklistItems.filter(item => item.result === 'PASS').length;
    const failedItems = params.checklistItems.filter(item => item.result === 'FAIL').length;
    const complianceRate = totalItems > 0 ? (passedItems / totalItems) * 100 : 0;
    ...
    const criticalViolations = violations.filter(v => v.criticality === 'CRITICAL').length;
    const majorViolations = violations.filter(v => v.criticality === 'MAJOR').length;
    const minorViolations = violations.filter(v => v.criticality === 'MINOR').length;
    ...
    if (check.followUpRequired) {
      ...
      followUpDate.setDate(followUpDate.getDate() + (criticalViolations > 0 ? 7 : 30));
  ...
  async getStatistics(params: {
    ...
    const avgComplianceRate = checks.length > 0
      ? checks.reduce((sum, c) => sum + Number(c.complianceRate), 0) / checks.length
      : 0;
    
    const totalPenalties = checks.reduce((sum, c) => sum + Number(c.penaltyAmount || 0), 0);
    const totalViolations = checks.reduce((sum, c) => sum + c.violationCount, 0);
    
    const byResult = checks.reduce((acc, c) => {
    ...
    return {
      ...
      totalViolations,
      criticalViolations: checks.reduce((sum, c) => sum + c.criticalViolations, 0),
