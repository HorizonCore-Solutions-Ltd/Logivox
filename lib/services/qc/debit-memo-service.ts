...
export class DebitMemoService {
  ...
  async createDebitMemo(params: {
    ...
    const debitMemoNumber = await this.generateDebitMemoNumber(params.organizationId);
  ...
  private calculateDebitAmount(calculation: DebitMemoCalculation): number {
    ...
    switch (calculation.method) {
      ...
      case 'PERCENTAGE':
        amount = (calculation.baseAmount || 0) * ((calculation.percentage || 0) / 100);
        break;
  ...
  async getStatistics(params: {
    ...
    const totalAmount = debitMemos.reduce((sum: number, dm: any) => sum + Number(dm.debitAmount), 0);
    const collectedAmount = debitMemos.reduce((sum: number, dm: any) => sum + Number(dm.collectedAmount || 0), 0);
    ...
    const byReason = debitMemos.reduce((acc: Record<string, number>, dm: any) => {
      acc[dm.reason] = (acc[dm.reason] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    
    const byStatus = debitMemos.reduce((acc: Record<string, number>, dm: any) => {
  ...
  private async generateDebitMemoNumber(organizationId: string): Promise<string> {
    const date = new Date();
  ...
  private async getVendorNextPayment(vendorId: string): Promise<{ date: Date } | null> {
