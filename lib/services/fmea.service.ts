...
export class FMEAService {
  ...
  static calculateRPN(
    ...
    if (occurrence < 1 || occurrence > 10)
      throw new Error("Occurrence must be 1-10");
    if (detection < 1 || detection > 10)
      throw new Error("Detection must be 1-10");
  ...
  static async getFMEAStatus(fmeaId: string) {
    ...
    const openModes = fmea.failureModes.filter(
      (fm) => fm.status === "OPEN",
    ).length;
    const mitigatedModes = fmea.failureModes.filter(
      (fm) => fm.residualRPN !== null,
    ).length;
    const closedModes = fmea.failureModes.filter(
      (fm) => fm.status === "CLOSED",
    ).length;
    ...
    const modesWithResidualRPN = fmea.failureModes.filter(
      (fm) => fm.residualRPN !== null,
    );
    const averageResidualRPN =
      modesWithResidualRPN.length > 0
        ? modesWithResidualRPN.reduce(
            (sum, fm) => sum + (fm.residualRPN || 0),
            0,
          ) / modesWithResidualRPN.length
        : null;

    const completionPercentage =
      totalModes > 0 ? ((mitigatedModes + closedModes) / totalModes) * 100 : 0;
    ...
    return {
      ...
      averageResidualRPN: averageResidualRPN
        ? Math.round(averageResidualRPN)
        : null,
  ...
  static async getFailureModesDueForAction() {
    ...
    return overdueFailureModes.map((fm) => ({
      ...
      daysOverdue: Math.floor(
        (now.getTime() - fm.targetDate!.getTime()) / (1000 * 60 * 60 * 24),
      ),
      riskLevel: this.getRPNLevel(fm.rpn),
    }));
  }
  ...
  static async getFMEAAnalytics(organizationId: string) {
    ...
    const highModes = allFailureModes.filter(
      (fm) => fm.rpn >= 125 && fm.rpn < 200,
    ).length;
    const mediumModes = allFailureModes.filter(
      (fm) => fm.rpn >= 50 && fm.rpn < 125,
    ).length;
    const lowModes = allFailureModes.filter((fm) => fm.rpn < 50).length;

    const mitigatedModes = allFailureModes.filter(
      (fm) => fm.residualRPN !== null,
    );
    ...
    const averageRPNReduction =
      mitigatedModes.length > 0 ? totalRPNReduction / mitigatedModes.length : 0;
   ...
   */
  static async linkFailureModeToCAPAAsync(
    ...
    if (failureMode.linkedCAPAIds) {
      try {
        linkedCAPAIds =
          typeof failureMode.linkedCAPAIds === "string"
  ...
  static getSeverityGuidance(): { rating: number; description: string }[] {
    return [
      {
        rating: 10,
        description: "Hazardous without warning - May endanger operator/user",
      },
      {
        rating: 9,
        description: "Hazardous with warning - Serious consequences",
      },
      {
        rating: 8,
        description:
      ...
      },
      {
        rating: 7,
        description:
          "High - Product/item operable with significant degradation",
      },
      {
        rating: 6,
        description: "Moderate - Product/item operable with some degradation",
      },
      {
        rating: 5,
        description:
          "Low - Fit and finish/squeak and rattle item does not conform",
      },
      {
        rating: 4,
        description:
          "Very low - Fit and finish/squeak and rattle item hardly noticeable",
      },
      {
        rating: 3,
        description: "Minor - Fit and finish/squeak and rattle item noticeable",
      },
      {
        rating: 2,
        description:
          "Very minor - Fit and finish/squeak and rattle item noticeable to discriminating customers",
      },
  ...
  static getOccurrenceGuidance(): {
    ...
    return [
      {
        rating: 10,
        description: "Very high: Failure is almost inevitable",
        probability: "≥ 1 in 2",
      },
      {
        rating: 9,
        description: "Very high: Repeated failures",
        probability: "1 in 3",
      },
      {
        rating: 8,
        description: "High: Frequent failures",
        probability: "1 in 8",
      },
      {
        rating: 7,
      ...
      {
        rating: 6,
        description: "Moderate: Occasional failures",
        probability: "1 in 80",
      },
      {
        rating: 5,
        description: "Moderate: Occasional failures",
        probability: "1 in 400",
      },
      {
        rating: 4,
        description: "Low: Relatively few failures",
        probability: "1 in 2,000",
      },
      {
        rating: 3,
        description: "Low: Relatively few failures",
        probability: "1 in 15,000",
      },
      {
        rating: 2,
        description: "Remote: Failure is unlikely",
        probability: "1 in 150,000",
      },
      {
        rating: 1,
        description: "Remote: Failure is unlikely",
        probability: "< 1 in 1,500,000",
      },
    ];
  }
  ...
  static getDetectionGuidance(): { rating: number; description: string }[] {
    return [
      {
        rating: 10,
        description: "Absolute uncertainty - Cannot detect or is not checked",
      },
      {
        rating: 9,
        description: "Very remote - Controls probably will not detect",
      },
      {
        rating: 8,
        description: "Remote - Controls have poor chance of detection",
      },
      {
        rating: 7,
        description: "Very low - Controls have low chance of detection",
      },
      { rating: 6, description: "Low - Controls may detect" },
      { rating: 5, description: "Moderate - Controls may detect" },
      {
        rating: 4,
        description: "Moderately high - Controls have good chance to detect",
      },
      { rating: 3, description: "High - Controls have high chance to detect" },
      {
        rating: 2,
        description: "Very high - Controls almost certain to detect",
      },
